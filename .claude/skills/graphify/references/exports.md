...
```bash
python ".claude/skills/graphify/run.py" export wiki
```
...
```bash
python ".claude/skills/graphify/run.py" export neo4j
```

**If `--neo4j-push <uri>`** - push directly to a running Neo4j instance. Ask the user for credentials if not provided:

```bash
python ".claude/skills/graphify/run.py" export neo4j --push bolt://localhost:7687 --user neo4j --password PASSWORD
```
...
```bash
python ".claude/skills/graphify/run.py" export falkordb
```
...
```bash
python ".claude/skills/graphify/run.py" export falkordb --push falkordb://localhost:6379
```
...
```bash
python ".claude/skills/graphify/run.py" export svg
```

### Step 7c - GraphML export (only if --graphml flag)

```bash
python ".claude/skills/graphify/run.py" export graphml
```
...
To configure in Claude Desktop, add to `claude_desktop_config.json`. Use the project-local interpreter printed by `cat graphify-out/.graphify_python`; it is the only interpreter selected by this project:
```json
{
  "mcpServers": {
    "graphify": {
      "command": "<project path from: cat graphify-out/.graphify_python>",
...
```bash
python ".claude/skills/graphify/run.py" benchmark
```
