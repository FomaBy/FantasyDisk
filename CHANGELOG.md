...
- Текущий канал обновлений macOS переключён на Developer ID-подпись и Apple
  notarization. Сборка остаётся fail-closed: без действительных Apple credentials
  или при отказе trust-проверок публикация не продолжится и не перейдёт молча на
  неподписанный fallback.
