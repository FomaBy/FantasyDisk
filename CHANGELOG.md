...
- Будущая macOS-сборка получает целостную ad-hoc подпись без Apple Developer ID:
  Finder больше не видит повреждённую унаследованную подпись export template.
  Подтверждение Gatekeeper «Всё равно открыть» для неподписанной поставки
