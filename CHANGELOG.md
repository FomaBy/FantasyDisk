...
- SCRUM-834a: Мета 4.1 — условные keystone на существующих гейтах с не-урон
  ...
  rush×crit=0.50). Поведенческие смоки (сценарии 5/6) проверяют, что стат растёт
  ЛИШЬ при выполнении условия. Немапящиеся пары остаются generic (→ SCRUM-835/
...
- SCRUM-834: Мета 4.1 — «сердце уникального геймплея» созвездий. Первые два
  keystone каждого из 17 классов (`k0`/`k1`) переведены со статических
  стат-трейдов на УСЛОВНЫЕ механики: бонус урона, активный лишь при выполнении
  условия одного из четырёх типов (гейты в `player.gd`, консумит
  `derived_parameters`): «пока ранен» (HP<50%, `hurt_damage_bonus`), «в стойке»
  (неподвижность ≥0.8с, `stance_damage_bonus`), «в рывке» (2с после уклонения,
  `rush_damage_bonus`), «в гуще боя» (доля врагов рядом от кэпа 8,
  `swarm_damage_bonus`). Крупный заголовочный процент (напр. +34% «в рывке»)
  сбалансирован через budget-веса = аптайм × вес урона, поэтому коридор силы
  §6 (+18..25%) и кросс-классовый спред ≤1.25 сохранены точно (конверсия
  budget-нейтральна). Третий keystone (`k2`) — прежний статический трейд.
  Скрытые звёзды: 17 классов получили уникальные per-class подвиги вместо
  однотипных (метрики `weapon_diversity`/`best_ascension`/`no_shop_wins` +
  новая `class_wins` из `class_boss_wins`, разные пороги). Минорные звёзды
  разведены на два номинала (+1/+2, пара сохраняет budget-сумму). Приложения
  A/B дизайн-дока и поведенческие гейты обновлены
  (`_test_conditional_keystones` — по сценарию на каждый тип условия).
