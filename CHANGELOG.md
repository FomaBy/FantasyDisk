...
### Fixed
- Исправлена визуальная привязка всех 17 героев к их боевой позиции: стопы каждого idle/move/walk-кадра теперь остаются на площадке получения урона, включая ранее зависавших выше неё Инженера и Гитариста.
- Исправлена компиляция главной сцены после выделения лор-модуля: на холодных чекаутах пропадали Кодекс, «Летопись» и экраны исхода забега; фокусные лор/Кодекс-тесты теперь падают с ненулевым кодом при ошибках компиляции Main вместо ложно-зелёного результата.
