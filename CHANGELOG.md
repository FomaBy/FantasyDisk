...
### Added
- Начиная с версии 0.2.2 игра проверяет публичные GitHub Releases при запуске и
...
### Fixed
- Ретроспективный массовый backfill story points на закрытой истории задач
  (FAN-1135) снят с 91 закрытой live-задачи с сохранением статусов, дат и
  description; Jira Archive не изменялся. Отчёт закрытых story points теперь
  разделяет принятые pre-work оценки и непринятую ретроспективную аналитику
  (`tools/story_points_remediation.py`), правило закреплено в
  `docs/process/story_points.md`.
- АОЕ молота Берсерка снова круглая, а не овальная: убран вертикальный растяг
  зоны удара — и область урона, и slam-VFX теперь ровный круг радиусом
  `aoe_radius` вокруг точки удара у ног. Урон, радиус и кулдаун не изменились.
- Релизная macOS-сборка больше не допускает тихий ad-hoc/unnotarized артефакт:
  строгий signed-канал (default) требует Developer ID, app+DMG notarization,
  stapling и Gatekeeper `spctl`, иначе публикация останавливается. Одобренный
  владельцем канал без Apple credentials включается только явным
  `FANTASYDISK_MACOS_CHANNEL=unsigned`, отказывается работать при заданных
  credentials и сохраняет все остальные проверки (точный тег, layout DMG,
  secret scan, SHA-256, манифест).
