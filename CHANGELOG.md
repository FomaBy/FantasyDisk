...
- Все 51 оружейный ультимейт теперь несут свою авторскую сцену в самом
  заспавненном эффекте: 22 недоподключённые презентации (солдат, робот,
  инженер, гитарист, доктор, тёмный маг, химик — по три; копьё рыцаря)
  получили мост `scripts/ultimates/classes/<класс>/<оружие>.tscn` с узлом
  `Presentation` по образцу берсерка. Урон, тайминги и заряд не менялись.
  Новый живой контракт-тест
  `tests/ultimates/presentation/weapon_ultimate_presentation_wiring_live_test.gd`
  активирует все 51 через реального игрока и краснеет при любом отключённом
  узле `Presentation` (FAN-2985).
