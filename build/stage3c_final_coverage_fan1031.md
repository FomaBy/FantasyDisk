...
> **Update (после peer review, коммит `8dd7e4fb`):** координатор ПОЧИНИЛ харнес — знаменатель
> `_measure_dps` теперь = фактическая сумма process-delta окна (вариант **B** выше). Значит
> committed v5-артефакты (`build/character_balance_dps.csv`, `character_balance_band.md`,
> `ascension_viability_report.md`) сняты СТАРЫМ инфлированным знаменателем и **невоспроизводимы**
> после фикса — приёмка Stage 4 идёт по **v6** (среднее двух чистых прогонов). Запись методики —
> в `progression_balance.md` (коммит `3a3b3442`). Абзац «я НЕ трогал» относится к моему коммиту
> `375aa03d`; фикс методики — отдельный коммит координатора поверх.
