...
| `plague_prophet` | Чумной Пророк | `scenes/ElitePoisoned.tscn` | Зональная элитка | `assets/sprites/elites/plague_prophet.png`; explicit eight-direction full-frame `assets/sprites/elites/full_frame/plague_prophet_spriteframes.tres` from `assets/sprites/elites/pixellab/plague_prophet/` | Пассив: ядовитые зоны. Уникальная атака `poison_volley`: 3 lob-снаряда по дуге в telegraph-метки, в точках падения лужи на 3с (тик 0.6с), кулдаун 8с. Визуально: `idle` 1f, `move` 8f loop, `skill_poison_volley` 10f one-shot, `hit` 5f one-shot, `death` 7f one-shot по каждому из 8 направлений; `attack`/`attack_primary` aliases отсутствуют в directional pack | Реализовано |
