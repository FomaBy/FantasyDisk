...
Data-driven ростер `scripts/progression_data_enemies.gd::MINI_ELITE_KINDS` содержит 10 видов mini-elite. FAN-3606 заменил donor-derived visual fallback для восьми целевых видов (`mini_scavenger_reaper`, `mini_plague_bellringer`, `mini_bone_warden`, `mini_spark_wight`, `mini_shadow_devourer`, `mini_siege_rammer`, `mini_swarm_sniper`, `mini_void_phantom`) на dedicated PixelLab character packs. Для каждого целевого вида `assets/sprites/elites/full_frame/mini_<id>_spriteframes.tres` содержит `idle`/`move`/`walk`/`attack`/`attack_primary`/`hit`/`death` и две `skill_*` строки с matching `attack_*` aliases по восьми направлениям; реестр устанавливает `explicit_eight_directions: true`, поэтому runtime не применяет `flip_h`. Исходные кадры, source-state mapping, immutable PixelLab character ID и SHA-256 alpha/frame отчёт хранятся в `assets/sprites/elites/pixellab/mini_<id>/manifest.json`. `mini_plague_berserker` намеренно оставлен на явном fallback к `plague_prophet` как clean control до отдельной задачи. Изменение визуальное: существующие kind, сцены, поведение и параметры боя не меняются.
