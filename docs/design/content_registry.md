...
| --- | --- | --- | --- | --- | --- | --- |
| `iron_bastion` | Железный Оплот | `scenes/EliteArmored.tscn` | Танкующая элитка | `assets/sprites/elites/iron_bastion.png`; full-frame `assets/sprites/elites/full_frame/iron_bastion_spriteframes.tres` (FAN-2620, explicit 8-direction PixelLab pack) | Пассив: периодический щит. Уникальная атака `slam_wave`: замах 0.6с с telegraph-кругом, затем кольцевая ударная волна (радиус 260, урон + отбрасывание), кулдаун 6с. Визуально: `idle`/`move`/`attack`/`hit`/`death`/`skill_shield_block`/`skill_slam_wave` × 8 явных направлений (`explicit_eight_directions: true`), без flip и без `attack_primary`/`attack_*` алиасов | Реализовано |
