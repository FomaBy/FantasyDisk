...
- `BaseStatsGrid` contains exactly eight real `BaseStatRow_<id>` controls: one column at 720p and two columns at 1080p/2K. The 1080p two-column name lane remains at least wide enough for the rendered short label `Сила`; longer localized names use meaningful visible prefixes plus the complete tooltip. `DerivedStatsGroups` is a real 2×2 grid for physical, magic, support/control and periodic-damage groups. Compact values use localized units (`/с`, `%`, `×`).
- Every base, survival and derived stat control is `FOCUS_ALL`. Hover uses the shared global tooltip, focus shows `DossierFocusTooltip` (max 430×288), and both read name/current value/description/formula/source/influences directly from `scripts/stat_formulas.gd`. Geometric D-pad neighbors preserve rows/columns and transfer boundary focus to the closest footer action.
