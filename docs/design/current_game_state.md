...
Settings controls получили системные fantasy assets из `assets/sprites/ui/icons/system/`: checkbox checked/unchecked, slider track/grabber, arrows/back/settings/close icons. Иконки зарегистрированы в `scripts/ui_icon_registry.gd` как `system_*`. SCRUM-334 подключил design-ready tab switcher frame `assets/sprites/ui/frames/settings/ui_frame_settings_tab_switcher.png` к экрану настроек как proportional strip с runtime hit areas внутри recorded safe rects.
...
- Кнопка «Повышение уровня (N)» появляется при непотраченных level-up выборах в бою и на небоевых экранах; она открывает тот же зафиксированный набор из 3 карт и показывает pending-бейдж. В бою кнопка закреплена в правом нижнем углу, полностью непрозрачна и использует красивую Red & Gold рамку `main_menu` с нейтральным hover/focus без желтого свечения, чтобы не закрывать центр арены и оставаться читаемой во время движения мышью. При `pending_level_ups > 0` это единственная точка входа в level-up: `UpgradeFabButton` скрывается, чтобы не дублировать действие. Когда pending-уровней нет, FAB остается доступен для докачки характеристик за золото. На событии докачка отключена (повторный вход перегенерировал бы выборы события). Кнопка «Позже» на самом level-up окне использует необрезанный 260x104 `back_m` frame, чтобы текст и орнамент оставались внутри content-зоны.
...
- Экран выбора героя — fullscreen `HeroSelectScreen` master layout: `HeroSelectContent` делится на две главные области с 1:2 stretch, где `HeroSelectPortraitPanel` занимает левую треть с full-art портретом в dedicated `ui_frame_hero_select_portrait`, а `HeroSelectRightRegion` занимает правые две трети с `HeroSelectDossierPanel` и невидимым `HeroSelectRadarReserve`. После SCRUM-321 сама портретная рамка находится во внутреннем `HeroSelectPortraitFrame`: колонка может быть шире, но декоративный frame art не растягивается по X отдельно от Y и всегда масштабируется цельной картинкой; портрет сидит в safe content-zone `Vector4(128, 230, 128, 330)`. После SCRUM-323 досье (`ui_frame_hero_select_dossier`) тоже стало цельным proportional layer: `HeroSelectDossierPanel` центрирует `HeroSelectDossierFrame`, frame art имеет source `1120x1140` и runtime rects `387x394` / `581x591` / `774x788` на 720p/1080p/1440p; единственное имя героя, описание, сильные/слабые стороны, список 3 оружий, селектор Возвышения и compact кнопку «Выбрать» лежат в `HeroSelectDossierContent` с base margins `Vector4(96, 66, 96, 54)`. SCRUM-355 заменил live dossier/thumbnail-strip PNG на более тонкие версии и установил strict Design safe margins dossier `Vector4(126, 160, 126, 172)` / thumbnail strip `Vector4(132, 62, 132, 62)`; runtime margins ещё должен обновить Back-end SCRUM-354, потому что контент нельзя оставлять на гребнях, металле и самоцветах. Программная радар-диаграмма по 8 базовым характеристикам остается плавающим `HeroSelectRadarPanel` с `ui_frame_hero_select_radar` в правом верхнем углу; SCRUM-322 сделал этот radar frame квадратным windrose `TextureRect`, который растет пропорционально с разрешением и не превращается в овал, while reserve inside right region keeps the dossier left of the radar gap without placing content on frame ornament. После SCRUM-347 заголовок «Характеристики» внутри compass удален: radar-control центрируется в рамке, полигон увеличен на 20% (`HeroStatRadar.HERO_RADAR_RADIUS_FACTOR = 0.36`), подписи остаются в safe field. Селектор Возвышения использует dedicated 54x62 +/- frames и показывает только дельту выбранного уровня (`Уровень N: ...`). Радар нормируется по общему максимуму каждой характеристики среди всех игровых героев. Runtime smoke проверяет фактическое 1/3·2/3 соотношение, что правый край описания/досье остается левее радара с зазором, что радар parented to `HeroSelectScreen`/anchored top-right, square aspect у radar panel, центрирование графика и что нет overlap на 1280x720, 1600x900 и 2560x1440; dump: `build/qa/hero_select_radar_rects.md`. Внизу находится адаптивная лента кликабельных миниатюр-картинок в `ui_frame_hero_select_thumbnail_strip`: Carusel frame рисуется цельной картинкой без 9-slice/one-axis stretch и масштабируется пропорционально (`1024x170`, `1536x255`, `2048x340` на 720p/1080p/1440p), а миниатюры центрируются внутри отдельной content-зоны `72/36/72/36` с 2px separation и адаптируются до более читаемых вертикальных слотов (`49x66`, `75x101`, `101x136` на 720p/1080p/1440p), не перекрывая орнамент. QA screenshots: `build/qa/scrum281/hero_select_1280x720.png`, `hero_select_1920x1080.png`, `hero_select_2560x1440.png`; SCRUM-320 copies: `build/qa/scrum320/hero_select_carousel_*.png`; SCRUM-321 rect dump: `build/qa/scrum321/hero_select_portrait_rects.md`; SCRUM-323 rect dump: `build/qa/scrum323/hero_select_dossier_rects.md`; SCRUM-355 preview/QA: `docs/design/previews/hero_select_thin_frames_content_zones.png`, `build/qa/scrum355/hero_select_thin_frames_qa.md`; SCRUM-322/SCRUM-347/SCRUM-342 rect dump: `build/qa/hero_select_radar_rects.md`. Все кнопки игры используют pointer-курсор.
