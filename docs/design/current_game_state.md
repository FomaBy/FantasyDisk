...
Обновлено: 2026-06-12 (release 0.1.2 / dev snapshot)
...
- Текущий релизный срез: `v0.1.2` от 2026-06-12; версия проекта в `project.godot` — `0.1.2`.
...
Contextual UI frame kits приняты Design review в `assets/sprites/ui/frames/contextual/`: Wild Start, Grave Defeat, Laurel Reward и Parchment/Codex/Map. После пользовательского фидбэка первый плоский вариант заменён на более реалистичный D&D/tabletop raster pass с текстурными материалами из текущих ассетов проекта. Они пока не являются активным runtime theme selection; подключение экранов должно идти через `docs/tasks/backend_contextual_ui_frame_theme_integration_task.md` / SCRUM-118 в backlog `0.1.4`.
...
| --- | --- |
| `battle` | Обычный бой |
| `elite_battle` | Бой с элитным врагом (победа открывает окно награды: выбор 1 из 3 артефактов крупными карточками по центру экрана, иконка 112px + тир цветом + эффект + классовая интерпретация, выбор обязателен, навигация мышью/клавиатурой/геймпадом; показывается до экрана докачки) |
| `shop` | Магазин |
...
- Нижняя кнопка «Повышение уровня (N)» появляется при непотраченных level-up выборах в бою и на небоевых экранах; она открывает тот же зафиксированный набор из 5 карт и показывает pending-бейдж. При `pending_level_ups > 0` это единственная точка входа в level-up: `UpgradeFabButton` скрывается, чтобы не дублировать действие. Когда pending-уровней нет, FAB остается доступен для докачки характеристик за золото. На событии докачка отключена (повторный вход перегенерировал бы выборы события).
...
- Экран выбора героя — fullscreen `HeroSelectScreen` v3: слева крупный full-art портрет выбранного героя, справа досье с описанием, сильными/слабыми сторонами, списком 3 оружий, селектором Возвышения, программной радар-диаграммой по 8 базовым характеристикам и кнопкой «Выбрать». Внизу находится лента из 9 кликабельных миниатюр; выбор миниатюры мгновенно обновляет портрет, досье и радар, а переход к оружию происходит только через кнопку «Выбрать». Радар нормируется по общему максимуму каждой характеристики среди всех 9 героев, поэтому силуэты классов сравнимы между собой. Все кнопки игры используют pointer-курсор.
...
| Выбор персонажа | Fullscreen v3: большой портрет слева, досье/оружие/Возвышение/радар справа, лента 9 героев снизу |
...
| Level-up | Геройский экран выбора 1 из 5 наград с портретом персонажа, частицами, редкими main stat карточками и отложенным выбором через нижнюю кнопку |
