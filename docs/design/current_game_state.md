...
Обновлено: 2026-07-04 (0.2.1 active sprint snapshot)
...
`18px` anchor gap and `16px` viewport clamp, but its runtime frame
...
minimal-metal tooltip frame for standard controls, preserves the 2K
glossary/stat tooltip frames for their specialist paths, wraps copy inside
content margins, ignores mouse input, clamps to the viewport, and keeps an 18px
cursor/anchor gap with side flip when space allows. The runtime spec mirror
lives at `docs/design/mockups/scrum840_global_tooltips/spec.md`.
...
Видимая VFX-зона каждой атаки совпадает с фактической зоной урона: художественный VFX (AttackVfx) дополняется полупрозрачным оверлеем точной геометрии зоны (`_show_exact_zone_overlay`). SCRUM-854/864 сделал точный `sweep`-оверлей Берсерка читаемой outward wedge-зоной: вершина остается на персонаже, arc уходит по направлению удара, alpha оверлея 0.60.
...
| Инженер | Минная Сетка | `engineer_pressure_mines` | `engineer_pressure_mines` | Три мины веером лежат свой duration и тикают урон по врагам внутри, а не исчезают после первого касания |
...
SCRUM-854: `AttackVfx.weapon_signature()` теперь рисует главный силуэт оружия отдельным non-additive `WeaponSignatureBody` с alpha 0.60; shadow/rim остаются ниже по читаемости. Это делает оружие видимым при каждом ударе, не меняя зоны урона и не поднимая additive-alpha cap для остальных VFX.
...
Актуальная философия прокачки: все базовые и производные атрибуты полезны каждому классу. Старая фильтрация «нерелевантных» статов отключена; `STAT_CLASS_RELEVANCE` оставлен пустым для совместимости. Исключение SCRUM-854: Доктор не получает внешние regeneration/vampirism/lifesteal rewards из level-up, artifact pool, shop, elite reward и start boon rolls; его sustain идет только через собственные weapon mechanics. Для остальных тематически чужих эффектов UI показывает классовую интерпретацию: магический урон становится зачарованием оружия, Лидерство — эхо-оружием/фантомом/соколом/фамильяром, звуковой урон — боевым кличем, DoT — малым bleed/burn/poison, Energy — ускорением уникальной механики.
...
- SCRUM-854 дополнительно срезал runaway Доктора: общие regen/vampirism/lifesteal rolls запрещены для `doctor`, поэтому `restore_potion`, `plague_syringe` и `bone_saw` остаются единственным источником sustain и по-прежнему проходят `doctor_drain_softcap_test.gd`.
...
- Баланс классов и оружия теперь имеет измерительный слой: `tools/balance_harness.gd` генерирует `build/balance_report.md` для 51 пары класс+оружие (solo DPS, 5-target DPS, crowd-clear 5/10/20, EHP) и финальный отчет `build/balance_final_audit_0_1_5.md`. `tests/global_damage_balance_smoke_test.gd` проверяет combined DPS ±25%, solo DPS ±20% и 5/10/20 crowd-clear time ±30%; текущий SCRUM-857/SCRUM-858 recheck PASS, худшее CCT +22.0% у `doctor/restore_potion` на 20 целях. SCRUM-856 добавляет class-trio identity audit в `docs/design/reports/full_class_rebalance_identity_audit.md`: волна SCRUM-857..860 должна чинить playfeel механикой оружий/китов, а не только числовыми множителями. SCRUM-857 добавил focused gate `tests/projectile_chain_pierce_identity_test.gd` для delayed grenade, long-cast meteor, ricochet vs fan split, prayer sustain arc и dark pierce/curse decay; SCRUM-858 закрыл первый tank/melee slice для Knight counter identity через focused gate `tests/melee_unique_mechanics_test.gd`. Playtest все еще нужен для ощущения темпа, но базовая численная сетка больше не держится только на ручной оценке.
