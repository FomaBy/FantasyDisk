...
Актуальная философия прокачки (FAN-1887): игрок видит только реально работающие для его класса/оружия варианты. Канонический player-facing реестр — 16 осей (`CharacterData.ATTRIBUTE_REGISTRY`); optional-ось матрицы означает «нет настоящего потребителя» и никогда не предлагается; «Сила призыва» и редкое Лидерство доступны только config-derived summon/deploy-способным классам (гитарист/химик/друид/инженер); карты с нулевой фактической дельтой (капы) отсеиваются до показа. Искусственная текстовая интерпретация потребителем не считается. `STAT_CLASS_RELEVANCE` оставлен пустым для совместимости. Исключение SCRUM-854/SCRUM-862: Доктор не получает внешние regeneration/vampirism/lifesteal rewards из level-up, artifact pool, shop, elite reward, boss completion reward и start boon rolls; его sustain идет только через собственные weapon mechanics. Классовые интерпретации остаются пояснительным текстом для базовых характеристик (Attribute Shop/pause tooltips), но не открывают выдачу осей без настоящего потребителя (FAN-1887). SCRUM-898 удалил отдельный звуковой урон и прежний универсальный battle-shout hook: все три оружия Гитариста и природная магия Друида используют `magic_damage`, а звуковая тема остаётся геометрией/VFX, не отдельной осью урона.
