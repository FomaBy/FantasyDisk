...
- Every base, survival and derived stat control is `FOCUS_ALL`. Hover uses the shared global tooltip, focus shows `DossierFocusTooltip` as a clipped vertical-scroll viewport (actual panel max 430×288), and both read name/current value/description/formula/source/influences directly from `scripts/stat_formulas.gd`. Geometric D-pad neighbors preserve rows/columns; footer Up/Down resolves only to stat rows that are actually visible after ScrollContainer clipping.
