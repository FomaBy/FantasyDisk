...
Обновлено: 2026-07-10 (0.2.1 active sprint snapshot)
...
- **Персистенс**: дисплей, звук, `aim_mode`, `debug_mode`, `combat_feedback`, `input_bindings`, а также геймпад-ключи (`input_mode`, `gamepad_bindings`, `gamepad_deadzone`, `gamepad_vibration`) сохраняются в `user://settings.cfg` (`scripts/game_settings.gd`) и применяются при старте. SCRUM-816 добавил запись этих геймпад-ключей в `main.save_game_settings()` (раньше их писало только ядро при старте, и любое сохранение настроек перезаписывало их дефолтами) + зеркалит `gamepad_deadzone`/`gamepad_vibration` в root-мету для `player.gd`. Активный забег сохраняется отдельно через `scripts/run_autosave.gd` в `user://fantasydisk_autosave.cfg` после безопасных route checkpoints, включая `current_act` для Act 2/3; главное меню предлагает «Продолжить»/«Новая игра» через widened SCRUM-842 continue-run dialog (840×380 panel, 420×72 primary continue button), а смерть/финальная победа очищают autosave.
...
- SFX (15 ogg-id, SCRUM-967/968): `hit`/`hit_magic`/`hit_dot` (типизированные попадания по `feedback.damage_type` в `enemy.take_damage`), `player_hit`, `dodge`, `pickup_xp`, `pickup_money`, `level_up`, `boss_phase` (смена фазы босса), `low_hp_pulse` (луп-канал `set_sfx_loop`, гистерезис HP ВКЛ<30%/ВЫКЛ≥34% — зеркалит виньетку); `purchase`/`ui_click`/`ui_back`/`ui_error`/`artifact_reveal` — ассеты готовы, вызовы в ui_screens.gd отложены (`docs/tasks/SCRUM-968_ui_screens_tail.md`).
...
- Источники/лицензии: `docs/design/audio.md` + `docs/design/references/audio_sources/SOURCES.md`; CC BY-атрибуции (Kevin MacLeod ×6) — `docs/CREDITS.md` (player-facing экран — в ui_screens-хвосте).
...
| Pause menu / stats | В активном бою Escape сразу открывает SCRUM-983 hero dossier внутри общей полой `meta40/frame_border.png` shell. Header показывает hero/build summary, body разделён на independently vertical-scrollable hero и 2×2 derived-stat zones, а четыре действия закреплены отдельным footer; только «Завершить забег» danger-red. В 720p/1080p/2K весь контент, hitboxes, focus outlines и scrollbar lanes остаются внутри exact inner rect. Все реальные stat rows/chips фокусируемы и показывают полное описание/formula/source/influences; B/Escape и Continue сохраняют прежний pause-stack/run state. |
...
- SCRUM-983 reuses the SCRUM-981 production shell `assets/sprites/ui/meta40/frame_border.png`: 160px source rails, `draw_center=false`, visual-only final layer. Exact frame safe rects are `133,113,1014,494`, `200,169,1520,742`, `267,225,2026,990`; inner content reserves are 24px / 24px / 32px. Four opaque mouse-ignore masks cover `viewport − inner rect` below content/frame so the combat HUD cannot show through ornament or reserve bands.
- `DossierHeader` owns only title and selected hero/weapon summary. `PauseControlButtons` is a fixed footer: Continue, Settings and Main Menu use neutral state tints; End Run alone is danger-red in normal/hover/focus/pressed/disabled.
- `DossierBody` uses exact responsive hero/derived columns. At 720p the hero viewport is `157,209,320,286`, derived viewport `489,209,634,286` and both scroll vertically with a reserved 14px scrollbar lane; horizontal scrolling is disabled. The 1080p body is `224,281,1472,470`, 2K body `299,385,1962,650`.
- `BaseStatsGrid` contains exactly eight real `BaseStatRow_<id>` controls: one column at 720p and two columns at 1080p/2K. The 1080p two-column name lane remains at least wide enough for the rendered short label `Сила`; longer localized names use meaningful visible prefixes plus the complete tooltip. `DerivedStatsGroups` is a real 2×2 grid for physical, magic, support/control and periodic-damage groups. Derived chips use deterministic compact Russian aliases (`Скор. атаки`, `Маг. урон`, `Скор. снар.`, `Период. ур.` etc.) while the tooltip retains the canonical name. Compact values use localized units (`/с`, `%`, `×`) and reserve enough width to render the complete value.
- Every base, survival and derived stat control is `FOCUS_ALL`. Mouse hover and keyboard/gamepad focus use the same internal `DossierFocusTooltip` clipped vertical-scroll viewport (actual panel max 430×288); unbounded engine/global hover is disabled for stat rows. Wheel reveals long text only while the pointer remains on a stat row; Page Up/Down and gamepad shoulders work for focus-only tooltips. Wheel outside stat rows remains owned by the underlying Hero/Derived scroll. Both modes read name/current value/description/formula/source/influences directly from `scripts/stat_formulas.gd`. Geometric D-pad neighbors preserve rows/columns; scroll-value changes and footer focus rebuild Up/Down neighbors from rows that are actually visible after ScrollContainer clipping.
- Runtime and visual evidence: `tests/scrum983_escape_dossier_test.gd`, `tools/capture_scrum983_escape_dossier.gd`, `build/qa/scrum983/`; accepted PixelLab/component geometry remains under `docs/design/{mockups,references,previews}/scrum983_escape_dossier/`.
