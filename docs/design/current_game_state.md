...
- **Tab switcher art**: исторические SCRUM-396/v2 three-slot strips больше не являются live geometry. SCRUM-1025 использует четыре независимые global-kit plates `SettingsTabButton_0..3`, поэтому орнамент не растягивается и каждая подпись/PixelLab-иконка остаётся внутри собственной пустой зоны. Runtime Game icon: `assets/sprites/ui/frames/settings_v6/ui_settings_v6_icon_game.png` (принятый SCRUM-975 source).
