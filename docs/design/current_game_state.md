...
SCRUM-356 подготовил Design-ready unified Hero Select frame для следующего runtime pass: `assets/sprites/ui/frames/hero_select/ui_frame_hero_select_unified_panel.png` (`1536x1024` RGBA) объединяет portrait+description в один тонкий D&D/Dark Fantasy frame, а `ui_frame_hero_select_asc_button_small.png` (`256x256` RGBA) дает компактный общий frame для `-`/`+`. Пока это не подключено runtime-кодом: Back-end handoff `docs/tasks/backend_hero_select_unified_portrait_description_frame_integration_task.md` должен заменить две отдельные панели на цельный proportional `TextureRect`, оставить radar top-right и carousel bottom отдельными и соблюдать source-space safe zones из `docs/design/references/hero_select_unified_panel/scrum356_unified_panel_metadata.json`.
...
- Победа над боссом также дает `skill_points` для общего древа умений; дерево имеет 4 ветки (`Богатство`, `Знания`, `Мощь`, `Стойкость`) с последовательной покупкой узлов, стартовым золотом/скидками/боевыми run modifiers и capstone-эффектами. На том же экране есть компактный раздел «Классы» для выбранного героя: победы над боссами, открытые личные бонусы и следующий порог.
