...
- Кнопка «Повышение уровня (N)» появляется при непотраченных level-up выборах в бою и на небоевых экранах; она открывает тот же зафиксированный набор из 3 карт и показывает pending-бейдж. В бою кнопка закреплена в правом нижнем углу, полностью непрозрачна и не меняет стиль при hover/focus, чтобы не закрывать центр арены и не мигать во время движения мышью. При `pending_level_ups > 0` это единственная точка входа в level-up: `UpgradeFabButton` скрывается, чтобы не дублировать действие. Когда pending-уровней нет, FAB остается доступен для докачки характеристик за золото. На событии докачка отключена (повторный вход перегенерировал бы выборы события).
