...
SCRUM-654 tightened the combat level-up callout: the world-space `LevelUpEffect` badge now renders at `160x80`, replaces older live nodes in the `level_up_effects` group on rapid level-up bursts, and remains the only visible `Level Up` text badge while `LevelUpToast` stays textless sparkle/ring feedback.
