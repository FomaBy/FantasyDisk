...
Contextual UI frame kits подготовлены на review в `assets/sprites/ui/frames/contextual/`: Wild Start, Grave Defeat, Laurel Reward и Parchment/Codex/Map. После пользовательского фидбэка первый плоский вариант заменён на более реалистичный D&D/tabletop raster pass с текстурными материалами из текущих ассетов проекта. Они пока не являются активным runtime theme selection; подключение экранов должно идти через `docs/tasks/backend_contextual_ui_frame_theme_integration_task.md` после Design review.
