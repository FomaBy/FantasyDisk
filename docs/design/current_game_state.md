...
- Экран выбора героя — fullscreen `HeroSelectScreen` v4: слева full-art портрет выбранного героя в dedicated `ui_frame_hero_select_portrait`, в центре отдельная frame-досье (`ui_frame_hero_select_dossier`) с единственным именем героя, описанием, сильными/слабыми сторонами, списком 3 оружий, селектором Возвышения и compact кнопкой «Выбрать», а программная радар-диаграмма по 8 базовым характеристикам вынесена в плавающий `HeroSelectRadarPanel` с `ui_frame_hero_select_radar` в правом верхнем углу. Layout резервирует место под радар, поэтому досье остается слева от него и не наползает на виджет. Селектор Возвышения использует dedicated 54x62 +/- frames и показывает только дельту выбранного уровня (`Уровень N: ...`). Радар нормируется по общему максимуму каждой характеристики среди всех игровых героев. Runtime smoke проверяет, что правый край описания/досье остается левее радара с зазором, что радар parented to `HeroSelectScreen`/anchored top-right и что нет overlap на 1280x720, 1600x900 и 2560x1440; dump: `build/qa/hero_select_radar_rects.md`. Внизу находится адаптивная лента кликабельных миниатюр-картинок в `ui_frame_hero_select_thumbnail_strip`: Carusel frame рисуется цельной картинкой без 9-slice/one-axis stretch и масштабируется пропорционально (`1024x170`, `1536x255`, `2048x340` на 720p/1080p/1440p), а миниатюры центрируются внутри отдельной content-зоны с base margins `112/46/112/46` и адаптируются `42-124px`, не перекрывая орнамент. QA screenshots: `build/qa/scrum281/hero_select_1280x720.png`, `hero_select_1920x1080.png`, `hero_select_2560x1440.png`; SCRUM-320 copies: `build/qa/scrum320/hero_select_carousel_*.png`. Все кнопки игры используют pointer-курсор.
