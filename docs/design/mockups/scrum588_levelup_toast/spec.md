...
or other content on frame ornament. The existing world-space `Level Up` badge
remains the only text callout; this HUD toast is a framed sparkle/ring layer.
...
| level_up_toast_frame | PanelContainer | generated toast frame | centered on player screen position, 480x300 | center at player screen position; fallback viewport center | 480x300 | first child | visible/fading | viewport |
...
The sprite center and initial sparkle positions must stay inside this rect. No
runtime text or badge is drawn by `LevelUpToast`.
...
- 2560x1440: base @2K target; `LevelUpToastFrame` is 480x300 with content rect `340x76`.
...
- Visible: spawned by `_show_level_up_toast()` when the player levels up.
- Hidden: self-cleans after fade-out and emits `finished`.
- Input: `mouse_filter = IGNORE`; no hover/pressed/focus states.
- Text/badge: none in this node. The accepted `Level Up` badge remains in `LevelUpEffect`.
...
this toast is a framed sparkle accent, not the main `Level Up` text badge.
