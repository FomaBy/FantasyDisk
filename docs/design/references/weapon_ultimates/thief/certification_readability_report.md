...
- The Thief class manifest declares `presence` devices (darken backdrop, camera shake, 100 ms hitstop, 0.4 time-scale dip) and a crouched-toss cast pose, but the shipped timeline scenes author only the sprite formation: no backdrop veil, no camera-shake device, no hero pose node, and no shared runtime consumer of those presence fields exists outside the schema. The reduced-motion and photosensitivity-safe variants are therefore identical to the normal frame by construction. Evaluated against this card's acceptance on 2026-09-09: the presence declarations stay valid under the shared v2 schema (AC5), the quality declarations are proven by the gate against the driven scenes (zero full-screen surfaces, formation-signature parity between the normal and reduced-motion variants, per-element alpha rise rates from the pack envelope), and authoring the missing devices would add new full-screen surfaces, camera and time-scale behaviour beyond a smallest quality/accessibility correction; recorded for a PM decision, not repaired.
