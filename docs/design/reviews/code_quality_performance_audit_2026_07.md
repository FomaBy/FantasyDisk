...
gameplay semantics, and creates dedicated follow-ups for work that needs native
profiling, external administration, branch protection or wider architecture.
...
- Python regression tests cover discovery (234 suites after FAN-1040), lock
...
4. `get_method_list()` reflection occurs on hits and active DoT physics paths.
...
There is no `.github/workflows`. `.githooks/post-commit` tests the task SHA, then
may create and push a different merge SHA in a detached process, and offers a
skip flag. This is incompatible with Multica completion safety and cannot
protect `dev`. FAN-1043 owns task branches, required exact-merge CI, branch
protection, merge queue, Windows-native correctness and scheduled performance.
...
Final changed-profile:

- 8 static/security/export-config checks passed;
- 9 diff-selected Godot suites passed: target cache, enemy separation, Engineer
  kit, feedback retry/config, hot-path cache, persistent hazard contract,
  combat smoke and UI smoke;
- umbrella `runtime_smoke_test.gd`: passed in an isolated process;
- skill structure validation: passed;
- discovery audit: 234/234 current GDScript suites selected by `full` profile.

The entire 234-process `full` profile was not executed during this macOS pass;
