...
- SCRUM-368 (2026-06-14) перевел route elites `iron_bastion`, `night_stalker` и `plague_prophet` на production full-frame SpriteFrames через `FullFrameAnimationRegistry` kind `elite`. `iron_bastion` и `night_stalker` сохраняют legacy 6f rows; FAN-2622 переводит `plague_prophet` на explicit eight-direction rows: `idle` 1f, `move` 8f loop, `skill_poison_volley` 10f one-shot, `hit` 5f one-shot и `death` 7f one-shot на каждый октант. Backend phase variants (`<elite_behavior>:<attack_id>:<phase>`) резолвятся в `skill_poison_volley_<dir>` без изменения damage/VFX timing; `attack`/`attack_primary` aliases для directional pack не добавляются.
