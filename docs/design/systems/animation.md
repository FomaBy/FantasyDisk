...
- FAN-3606 обновляет восемь mini-elite (`mini_scavenger_reaper`, `mini_plague_bellringer`, `mini_bone_warden`, `mini_spark_wight`, `mini_shadow_devourer`, `mini_siege_rammer`, `mini_swarm_sniper`, `mini_void_phantom`) dedicated PixelLab character packs. Каждая строка `idle`/`move`/`walk`/`attack`/`attack_primary`/`hit`/`death` и обе `skill_*` имеет по восемь реальных направлений и одинаковое число кадров; runtime rows нормализованы на прозрачный canvas 512×512 с footline `y=480`, а registry `explicit_eight_directions: true` отключает зеркальный `flip_h`. Source ID, state mapping и SHA-256 alpha/frame audit сохранены в `assets/sprites/elites/pixellab/mini_<id>/manifest.json`. `mini_plague_berserker` оставлен на явном fallback к `plague_prophet` и не входит в этот visual replacement.
