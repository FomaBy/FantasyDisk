...
| id | Название | T | Механика | Реализация | Balance note | Icon (style notes) |
| --- | --- | --- | --- | --- | --- | --- |
| `armor_protocol` | Бронепротокол | 2 | Броня-идентичность усилена: часть каждого удара просто игнорируется. | mods `absorb_flat: 5.0` EXISTS, `defense_flat: 0.08` EXISTS | Defense следует строгой асимптоте <`SURVIVABILITY_DEFENSE_CAP 0.62`; внутренний absorb пропускает ≥42% удара — «финальный игнор» безопасен. | riveted armor protocol plate, hazard chevrons |
...
| id | Название | T | Механика | Реализация | Balance note | Icon (style notes) |
| --- | --- | --- | --- | --- | --- | --- |
| `prayer_beads` | Четки молитвы | 2 | После выбранной боевой молитвы первые секунды боя дополнительно усилены. | mods `prayer_opening_power: 0.30`; trigger `on_battle_start` — SCRUM-926 вызывает хук только ПОСЛЕ обязательного `select_battle_prayer`: первые 6с боя +30% магического урона и +30% исходящего лечения | Не подменяет и не усиливает численно один из трёх вариантов; это отдельное короткое opening-окно после выбора. | worn wooden prayer beads, glowing cross pendant |
