...
| id | Название | T | Механика | Реализация | Balance note | Icon (style notes) |
| --- | --- | --- | --- | --- | --- | --- |
| `chain_wand` | Цепная палочка | 2 | Луч палочки рикошетит до трёх врагов малыми взрывами. | mods `wand_chain_blasts: 3.0` NEW — хук `class_weapon.gd` (beam/dark_wand: первые 3 пробитые цели получают взрыв 35% урона, r70) | Взрывы не порождают новые рикошеты — без каскада. | crooked dark wand, chained spark orbs |
...
| `mirror_page` | Зеркальная страница | 3 | Взрыв Книги тьмы зеркалится в точке, симметричной относительно мага. | mods `book_mirror_blast: 1.0` NEW — хук `class_weapon.gd` (aoe_projectile/dark_book: каждый взрыв дублируется на 55% в зеркальной точке через позицию мага) | Покрытие тыла — фирменный «двойной AoE» книги в квадрате. | torn grimoire page, mirrored ink explosion |
...
| id | Название | T | Механика | Реализация | Balance note | Icon (style notes) |
| --- | --- | --- | --- | --- | --- | --- |
| `fourth_ring` | Четвертое кольцо | 3 | К трём стихиям приходит четвёртая — земля: квадратная AoE с физикой, DoT и отбросом. | mods `elemental_orb_extra_count: 1.0` EXISTS (`player.gd:1014`), `earth_orb_mode: 1.0` NEW — хук `class_weapon.gd` (elemental_orbit: 4-я орбита «земля» — физический удар + мини-DoT + нокбэк, квадратная разметка зоны) | Кольцо Трёх Стихий буквально становится Кольцом Четырёх. | four elemental orbs in square formation, earth rune |
| `prismatic_cross` | Призматический крест | 3 | Крест призмы пробивает карту насквозь X-образными линиями. | mods `prism_cross_pierce: 1.0` NEW — хук `class_weapon.gd` (prism_rift: лучи креста удлиняются до края экрана, дальняя часть 60% урона); `prism_rift_radius_mult: 0.20` EXISTS (`player.gd:1051`) | Дальняя часть ослаблена — против снайп-доминации. | crossed prism beams, x-shaped light lattice |
| `meteor_heart` | Сердце метеора | 2 | Метеор — самый долгий и самый жирный пэйофф; воронка догорает DoT. | mods `meteor_heart_mode: 1.0` NEW — хук `class_weapon.gd` (meteor_shards: `fire_interval` +45%, центральный удар +70%, воронка горит DoT 2.5с) | DPS ≈ паритет, пик-момент и телеграф — больше. | molten meteor heart core, dripping magma |
