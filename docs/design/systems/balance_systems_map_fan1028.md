...
- **Runaway-гейты** (FAN-1031/FAN-1039: знаменатель замера = Σ process delta, не жёсткие 8с — иначе под нагрузкой 480 кадров ≫ 8с игрового времени и DPS раздувался/шумел; «гонять ОДИН без параллельных Godot» БОЛЬШЕ НЕ нужно — замер оконно-инвариантен): berserk_dps_runaway_gate.gd (медиана N прогонов через tools/berserk_runaway_gate.py — у молота остаётся дискретная «полка» крит/геометрии/слэмов, p99 одиночного = 15.5k ≫ медианы 9.05k; single-run p99-потолок замаскировал бы runaway → нужна медиана); pool_dot_runaway_gate.gd (acid_flask честно ≈9.7k, было раздуто ~7×; полок нет → одиночный прогон + потолок 12k по p99, откат pool-диминишинга ловится).
...
- **berserk_dps_runaway_gate (FAN-1039)** — гейтится МЕДИАНОЙ N изолированных прогонов через `tools/berserk_runaway_gate.py` (одиночный живой замер имел межпроцессный разброс полками до 2× — потолок FAN-1034 4400 пробивался шумом в ~40%). Замер (`tests/berserk_dps_runaway_gate.gd`) починен: нормировка на реальное сим-время (было ÷ жёсткие 8.0 при 480 idle-кадрах < 8с), сид боевого крит-RNG, пин игрока (дрейф от контактной дамми), гашение авто-ульты (sustain), `--fixed-fps 60`. Медианные потолки: 20t ≤ 10000 (медиана dev ≈ 9050), 1t ≤ 1250 (медиана ≈ 1132). Числа выросли с ~3600 не из-за баланса, а из-за починки заниженного замера (истинный sustained DPS). Runaway +30% ловится (медиана 20t → ~11800)
