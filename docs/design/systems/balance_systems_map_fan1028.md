...
progression_data_weapons.gd (1235 строк), 17×3 оружия, тиров/анлоков нет. `ProgressionData.weapon()` (progression_data.gd:2045-2057) инжектит `budget_damage_multiplier` из авто-тюнера:
...
XP: старт 5, `next = ceil(cur×1.09+0.8)` (player.gd:145,3491; progression_data_balance.gd:418) → ~14-15 lvl к боссу А1, ~23-24 к А2, ~32 за 20-боёвый ран. Level-up = 1 из 3 карт из **24** LEVEL_UP_REWARDS (✅ пересчитано: 24; progression_data_content.gd:478), редкая «+1 главный стат» 5%/слот (MAIN_STAT_SLOT_CHANCE, ui_screens.gd:12194). Артефакты-семьи +2/+4/+7 за тир (цены 30/55/95, ролл ≈64/29/8%). Мета: созвездие класса = +18..25% силы (гейт, meta_progression.gd:1350-1364), Атлас ≤1.35 аккаунт / класс-дельта ≤0.18, CLASS_PROGRESSION к 9 победам +12% урона/+6% HP/+4% AS, трек возвышений ~×1.1235 урона к A5. Возвышенские награды кладутся в run_modifiers = **ПОД softcap** (main.gd:1201-1221).
...
4. **Боссовые зоны/сламы/хазарды — БЕЗ капа**: slam = contact×(1.5+0.22/фаза), rift zone = proj×(1.25+0.18/фаза), phase hazard = proj×(1.35+0.25/фаза) (boss.gd:773,731,1048); на st16 базовые 3.4-4.1 → **55-66 урона за зону** = ваншот для трио с 37.5 HP (elementalist/dark_mage/chemist). На L5 телеграфы ещё ×0.72.
...
| **druid** | 43/62.5 | маг | Аура дикой силы: 0.10×buff_power, кап 0.30, радиус ×0.85 | **summon_amulet 1.85 — ЕДИНСТВЕННОЕ оружие без пассива** (заглушка :615-640; exp 1.22; 5+floor(LDR/4)=7 юнитов) / briar 0.70 (физ-хиты ×0.34, НЕ dot-ось) / raven 0.66 (вороны ×0.85) | 1.00/1.00/1.00 | Саммонер: AS-баффы не работают (player.gd:3598); ступени floor(LDR/4) — малые правки статов = 0 или скачок |
...
1. ✅ **LEVEL_UP_REWARDS: 23 (derived-stats) vs 24 (economy-levelup)** → в progression_data_content.gd:478-500 ровно **24** записи. Economy прав.
...
6. **Канал ульты**: char-data — «захардкожен per-class в player.gd:2472-2700, не читается из CLASS_DAMAGE_PARAMETER», но player.gd:2922 и :3182 местами ЧИТАЮТ `damage_parameter_for(character_id)`. Реальная картина смешанная — проверить каждую из 17 ульт на соответствие канала CLASS_DAMAGE_PARAMETER до любых правок каналов.
