...
| `ranger` | «сторожевой лук» (SCRUM-909..913): trait — каждый лучный хит отбрасывает ОТ героя; сплит-болт 1→4, дальний пирс-конус, перманентные капканы с параличом и зелёным кровотечением; частые выстрелы сохраняют заряд стойки до единственного полного release |
...
| `elementalist` | `elementalist_orb_ring` | Кольцо Четырёх Стихий | `ElementalistOrbRing.tscn` | `elemental_orbit` | SCRUM-948: квадратная AoE в точке каста — тики бьют тремя каналами (магия+физика+ожог) и отбрасывают ближайшее скопление врагов от центра; FAN-1028: жёсткий width-cap `orbit_max_targets` (ближайшие цели ядра, дальний хвост толпы пропускается целиком — не хит нулём) |
...
| `sniper` | `sniper_deadeye_rifle` | Винтовка Мертвого Глаза | `SniperDeadeyeRifle.tscn` | `sniper_lockshot` | Дальний lockshot: короткий прицел, затем точный beam по locked target и falloff по линии; финал после основного попадания открывает на этой цели одну owner-scoped weakpoint на 4с, которую следующий основной lockshot расходует на +30% без charge-полей или изменения базового баланса |
...
| `biologist` | `biologist_spore_lens` | Споровая Линза | `BiologistSporeLens.tscn` | `bio_spore_bloom` | ЛОКАЛЬНЫЙ AoE у персонажа (range 235): три расширяющихся кольца с falloff; задетые замедлены 5→20% по прогрессии, а ближайшие ещё и заражены bio_infection (SCRUM-896); FAN-1028: заражение width-capped `status_max_targets` (дальний хвост толпы ранен+замедлен, но не заражён) |
...
| `robot` | `robot_reactor_core` | Реакторное Ядро | `RobotReactorCore.tscn` | `robot_reactor_vent` | Exactly 4 reactor vents at 90° from a world phase (no homing) that rotate +6° clockwise per cast — the fan sweeps the full circle over 15 casts; per-vent damage = roll × 0.42; `extra_projectile` is a dormant/internal injected-only compatibility seam with no live production source and does not expand vents |
...
| `engineer` | `engineer_repair_drone` | Орбитальный Дрон | `EngineerRepairDrone.tscn` | `engineer_orbit_drone` | SCRUM-906/FAN-1075/FAN-1101: базовая пара укрупнённых дронов (scale 0.36, контакт 66 px — FAN-1101 +50% к 0.24/44) движется строго напротив друг друга на общем кольце 121 px (+55%); с третьего дрона включается спираль (+14% радиуса за слот); число 2+floor(max(sa−12,0)/4), рельс 6; физический контакт, per-enemy CD 0.85с, attack_speed крутит RPM; ремонт удалён |
...
| `ranger` | `moon_crossbow` | Лунный арбалет | `MoonCrossbow.tscn` | `moon_split_shot` | SCRUM-910: физический болт в цель + расщепление в 4 РАЗНЫХ соседей (радиус `aoe_radius` 260) с ТЕМ ЖЕ уроном, без рекурсии; каждый хит отбрасывает от героя. Заряд стойки переживает partial auto-fire и сбрасывается ровно полным release; constellation-final метит только primary на 4с, а следующий primary-hit получает +28% один раз |
| `ranger` | `storm_longbow` | Грозовой длинный лук | `StormLongbow.tscn` | `storm_pierce_cone` | SCRUM-911/1037: конус 5 пробивающих стрел (34°, дальность 980), пирс 4/стрела без спада, дедуп на залп; каждый хит отбрасывает от героя; один Animator release VFX играет на залп без замены live beam-коридоров. Использует тот же контракт заряда: partial release сохраняет, полный release сбрасывает цикл |
| `ranger` | `hunter_trap` | Охотничий капкан | `HunterTrap.tscn` | `trap` | SCRUM-913: ПЕРМАНЕНТНЫЙ капкан (кап 6 живых, не таймер), игроку безопасен; триггер = физ. AoE + паралич 2.2с (movement_locked, боссы ×0.25) + зелёное кровотечение 5с по dot-оси. Снапшот заряда следует общему контракту: partial release сохраняет, полный release сбрасывает цикл |
...
| `chemist` | `blast_powder` | Взрывная пыль | `BlastPowder.tscn` | `aoe_projectile` | SCRUM-943: быстрый ПРЯМОЙ физический close-mid AoE (fire 0.62, r150, range 430), без луж/DoT; trait периодики его не усиливает. FAN-2238: каждый бросок заряжен ОДНИМ реагентом, соседние броски чередуются — база от этого не меняется, чередование читает только финал «Несовместимые реагенты» |
| `chemist` | `acid_flask` | Кислотная колба | `AcidFlask.tscn` | `aoe_projectile` | SCRUM-944: долгая (7с) полупрозрачная лужа; тики по врагам в сердцевине лужи + один ВЕЧНЫЙ кислотный заряд с каждой отдельной лужи (кап 5, артефакт +3), заряды тикают по dot-оси до смерти носителя; FAN-1028: тик лужи width-capped `pool_max_targets` (плотный хвост толпы кислота не режет — area-denial по ядру) |
| `chemist` | `homunculus_vial` | Склянка гомункула | `HomunculusVial.tscn` | `summon` | SCRUM-946/FAN-1076: постоянная крупная пара — танк (4x max HP Химика, глобальный приоритет аггро, 50% эффективного регена и вампиризма владельца, смертен, респавн 4с) + неуязвимый кастер (вне боевого лимита, волны r240 каждые 1.7с вешают вечный DoT-заряд, кап 4; fallback-позиция — плечо Химика) |
...
Source-specific summon/deploy visuals (SCRUM-157): `scripts/summoner_weapon.gd` reads `ally_visual_id` / `ally_visual_ids` and passes the selected ID into `AllyMinion.set_visual_id()`. `summon_amulet` randomly uses `ally_druid_beast` or `ally_druid_pack_spirit`; `homunculus_vial` uses the SCRUM-945 PixelLab pair art (`homunculus_tank_*`/`homunculus_caster_*`, 4-directional static frames chosen by movement axis in `AllyMinion`/`SummonerWeapon`). FAN-1076 задаёт паре отдельные крупные масштабы (`1.20` для танка, `2.40` для кастера) и разносит их якоря, чтобы силуэты не перекрывались. Арт `leadership_echo` остаётся в реестре анимаций, но runtime-потребителя не имеет: универсальный Leadership-echo удалён (FAN-1893), summoner_weapon больше не мапит этот id. `scripts/class_weapon.gd` reads optional `deploy_texture_path`: `sound_amp` deploys `deploy_sound_amp_field.png`, while `raven_totem` deploys `deploy_raven_totem_field.png`.
...
## FAN-1893: явный capability-контракт оружия

Каждый из 51 конфигов `progression_data_weapons.gd` объявляет две явные capability-оси (сертифицирующий гейт — `tests/fan1893_capability_contract_test.gd`, 51-row матрица с исполняемыми positive/negative probes):

- `real_projectile_count` — число реальных снарядов за атаку, которым управляет generic-ось `run_modifiers.extra_projectile`. Capability-набор: `soldier_rifle` 1, `sniper_shatter_rounds` 6, `storm_longbow` 5, `plague_syringe` 1, `engineer_sentry_wrench` 2, `restore_potion` 1, `blast_powder` 2, `acid_flask` 1, `briar_staff` 1; остальные 42 оружия объявляют 0. Ловушки, тики, звенья цепи/рикошета, ширина луча/зоны и зеркальная пара книги снарядами не считаются и ось не потребляют (перегруженные интерпретации удалены из `class_weapon.gd`).
- `summon_semantics` — семантика призыва/деплоя: `pack` (амулет Друида), `pair` (гомункулы Химика), `deploy` (амп Гитариста, тотем Друида), `device` (турели/дроны Инженера), `mine_field` (мины Инженера), `none` (остальные 44). Capability-derived класс-набор призыва/Лидерства = **Guitarist, Chemist, Druid, Engineer** и совпадает с config-derived `ProgressionData.class_summon_capable`; только эти четыре класса получают summon/Leadership-предложения.
- `summon_bonus` входит в единый capped-парк ровно один раз — `player._apply_weapon_scaling`: `min(база + floor(Лидерство/4) + summon_bonus, max_summons_cap)` — и только у `pack`/`deploy`-китов (`AttributeContract.weapon_consumes_summon_bonus`). `pair` ведёт популяцию сам, `device` считает парк от derived `summon_amount` (только статы), derived `summon_amount` `summon_bonus` не читает (анти-двойное-применение, гейт `tests/stat_formulas_derived_sync_test.gd`). Карточки и advisor показывают фактический capped-парк до/после (`AttributeContract.summon_runtime_count`, `LevelUpAdvisor.forecast_reward`).
- Универсальный `_leadership_echo` удалён из `player.gd`: высокий `summon_amount` у 13 классов без реальных призывов больше не конвертируется в synthetic-урон; их `CLASS_INTERPRETATIONS`-тексты честно объявляют отсутствие потребителя.
- Fail-closed: конфиг без capability-ключей ведёт себя как 0/`none` (без наград и без крэша); неизвестное значение семантики трактуется как `none`; legacy-сейв-показ с summon-картой у класса без потребителя сбрасывается санитайзером.
...
- `tower_shield` «Конус-баш» (SCRUM-922): конус 95° целится в направление
  ...
  где `knockback_power` = derived (база 60 + Strength×4) ×
