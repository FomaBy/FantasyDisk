...
| `engineer` | `engineer_repair_drone` | Орбитальный Дрон | `EngineerRepairDrone.tscn` | `engineer_orbit_drone` | SCRUM-906/FAN-1075: базовая пара увеличенных дронов (scale 0.24) движется строго напротив друг друга на общем кольце 121 px (+55%); с третьего дрона включается спираль (+14% радиуса за слот); число 2+floor(max(sa−12,0)/4), рельс 6; физический контакт, per-enemy CD 0.85с, attack_speed крутит RPM; ремонт удалён |
