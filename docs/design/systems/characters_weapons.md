...
| `soldier` | `soldier_grenade` | Граната с фитилем | `SoldierGrenade.tscn` | `grenade_cook` | Бросок и посадка без урона; после fuse телеграфированная зона взрывается с falloff |
...
| `elementalist` | `elementalist_meteor_core` | Ядро Метеора | `ElementalistMeteorCore.tscn` | `meteor_shards` | Long-cast метеор: крупный central impact с falloff и вторичные shard-зоны |
...
Summon role runtime (SCRUM-254/SCRUM-854): summon/deploy configs may define `summon_role` and role coefficients. `SummonerWeapon` builds an `AllyMinion.set_combat_profile()` payload from owner `derived_parameters` and Leadership: damage, move speed, attack interval, lifetime, max HP, control knockback, support healing and small splash. Current roles are `pack_damage` (Druid beasts), `tank_control` (Chemist homunculus), `support_totem` (Druid raven totem), `engineer_sentry` and `support_drone`. Mobile summon weapons tag minions by owner+weapon, prefill about half of the current cap at battle start, and then replenish normally. `ProgressionData.weapon_archetype()` treats `summon_role` weapons as summon archetype, and the balance harness models pure summon DPS through minion output rather than an invisible direct hit.
