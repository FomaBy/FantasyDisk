...
- SCRUM-854: Berserk `sweep` exact overlay is a readable outward wedge whose apex remains on the character and whose arc extends along attack direction; the old low-alpha zone was too easy to read as an inverted sickle. Chemist/Druid-style ground pools keep up to 6 active pools per weapon owner and expire by their own `pool_duration`; Engineer pressure mines are persistent hazards that tick each `pool_tick_interval` while enemies remain inside and clean up only at lifetime end.
