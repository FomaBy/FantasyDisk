...
- SCRUM-356 adds Design-ready assets for the requested single portrait+description Hero Select frame. `ui_frame_hero_select_unified_panel.png` is `1536x1024` RGBA and must be drawn as one proportional `TextureRect`, not 9-sliced or stretched on one axis. Runtime content may only use these source-space safe zones: portrait `Rect2(130,145,420,560)`, description `Rect2(610,145,786,500)`, bottom controls `Rect2(570,705,660,178)`. `ui_frame_hero_select_asc_button_small.png` is a compact `256x256` RGBA stepper frame for both `-` and `+`, with content margins `[76,74,76,76]`. Back-end integration must replace the separate portrait/dossier layout with this unified frame while keeping radar top-right and carousel separate.
