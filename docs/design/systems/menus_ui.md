...
- Hero select uses the reopened SCRUM-436 v2 fullscreen runtime contract. `_show_character_select()` builds a centered proportional `1536x1024` `HeroSelectCanvas` from `docs/design/references/hero_select_mockup/hero_select_layout_mockup.png`; title, portrait, dossier/title/body/traits/weapons, ascension controls, Select/Back buttons, the image-only carousel and tooltip-safe footer are placed from corrected mockup source rects. The live SCRUM-322/SCRUM-347 `HeroSelectRadarPanel` / `HeroStatRadar` compass-rose contract is preserved and only positioned in the mockup `stat_radar` slot. SCRUM-389 makes character selection default the ascension selector to `ascension_selectable_max(character_id)` for that class; the player can still lower or restore the chosen run level with the existing minus/plus controls before selecting a weapon. SCRUM-416 binds every static character portrait surface to the accepted cleaned full-frame idle frame via `ProgressionData.character_config(...).sprite_path`, and SCRUM-417 keeps `HeroSelectLargePortrait` on covered aspect scaling inside the SCRUM-436 portrait safe zone. Runtime smoke asserts proportional canvas aspect, safe-zone containment, preserved radar placement, carousel containment, portrait texture paths/scaling and no-overlap at 1280x720, 1600x900 and 2560x1440; QA dumps live in `build/qa/scrum436/`.
- Hero Select must preserve 720p safe areas: `HeroSelectBackButton` stays visible inside the bottom mockup button slot, portrait/dossier/radar remain separated, and the bottom thumbnail strip stays fully visible with adaptive image-only previews. The active runtime frame kit is `assets/sprites/ui/frames/hero_select_v2/ui_hero_select_*.png`; corrected slices are recut from the accepted mockup where the Vision bboxes were wrong (`portrait_panel`, `dossier_panel`, `ascension_stepper`, `select_button`, `carousel_strip`, `back_button`). These frames are drawn as whole `TextureRect` layers with uniform canvas scaling, not as one-axis-stretched generic StyleBox frames. `HeroSelectLargePortrait`, labels, ascension controls, hover highlights, thumbnails and button hit targets stay inside their source-space content zones and off red gems, metal borders, carousel ornaments and dragon flourishes. After SCRUM-347 the old `HeroStatRadarTitle` is removed; `HeroStatRadar` is the only content in the compass field, centered in the frame, with polygon radius factor `0.36` and tighter label offsets so graph labels remain inside the frame. QA rect dumps live at `build/qa/scrum436/hero_select_v2_runtime_rects.md` and `build/qa/scrum436/hero_select_v2_no_overlap_matrix.md`; recut before/after evidence is under `build/qa/scrum436/reopened_frame_recuts/`.
...
- SCRUM-436 runtime integration is complete for Sprint 0.1.6 after the user reopen: the live Control rects in `scripts/ui_screens.gd` now mirror the accepted `hero_select_layout_mockup.png` composition rather than the older 1920x1080 Design package. Do not place future Hero Select labels, portraits, icons, buttons, hover highlights or tooltips outside the corrected source-space safe zones or on frame ornament.
