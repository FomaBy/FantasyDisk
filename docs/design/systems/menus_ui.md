...
- SCRUM-338 prepares a dedicated reward-card frame kit for battle rewards and elite artifact rewards: `assets/sprites/ui/frames/rewards/ui_frame_reward_card.png`, `_hover.png`, `ui_frame_reward_elite_artifact_card.png` and `_hover.png`. These are Design-ready assets only until Back-end wires `backend_reward_screens_per_reward_frames_integration_task.md`. Runtime must use the metadata in `docs/design/references/rewards/reward_frames_scrum338_metadata.json` and keep title, icon, description, artifact tier labels and `Получить`/choice hit areas inside the safe content fields, not on the red gems, top crests, side metal or bottom ornaments.
