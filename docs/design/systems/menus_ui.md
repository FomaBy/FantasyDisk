...
- SCRUM-373 adds the design-ready unified master frame kit in `assets/sprites/ui/frames/unified/` for the next Back-end centralization pass. Generic panels/cards/tooltips/HUD may use `ui_frame_unified_master.png` as a true 9-slice StyleBoxTexture with tile stretch on both axes, texture margins `128/128/128/128` and content margins `132/132/132/132`. Screen-specific whole-image frames with authored source safe zones, including Hero Select SCRUM-356, the radar, carousel and settings tab switcher, must stay proportional and must not be forced into the generic 9-slice builder. Optional top/bottom unified ornaments are for large windows only; no runtime content may overlap them.
