...
- 2026-06-30 active Hero Select is a user-requested minimal black screen, not the older ornate/Pixellab frame layout. `_show_character_select()` / `_build_character_select_v4()` now builds `HeroSelectScreen` over `HS4BlackBackground`; no title frame, no PixelLab backdrop, no compass rose, and no `HeroStatRadar` are active. The selected hero preview sits at `250x250` on the left and keeps SCRUM-416/SCRUM-687 directional SpriteFrame rotation when available. The center `HS4DossierFrame` contains character description, strengths, weaknesses, weapon names, class identity and the eight main stat buttons with hover tooltips explaining affected derived attributes. The right `HS4AscensionFrame` contains ascension description, `-`/`+` selection and `HS4ChooseButton`. The bottom `HS4Carousel` uses `150x150` `HS4CarouselSlot_*` buttons, cyclic arrow paging and default focus on the selected visible slot.
