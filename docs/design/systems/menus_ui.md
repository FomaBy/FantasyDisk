...
- SCRUM-355 supersedes the earlier dossier/carousel content-zone guidance for Design-safe ornament avoidance: the live `ui_frame_hero_select_dossier.png` and `ui_frame_hero_select_thumbnail_strip.png` were recomposed thinner/lighter by `tools/build_hero_select_thin_frames.py`; strict source margins are dossier `Vector4(126, 160, 126, 172)` and thumbnail strip `Vector4(132, 62, 132, 62)`. Runtime must keep those frames whole-image proportional and must not place labels, description text, ascension controls, start button, thumbnails, hover states or selection states on the crest, metal, side gems or bottom ornaments. Back-end SCRUM-354 owns switching `scripts/ui_screens.gd` from the older runtime margins to these strict values and preserving a visible vertical gap between dossier and carousel.
