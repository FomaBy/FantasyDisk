...
Заполнено в T1 (SCRUM-828) по правилам §3: у каждого класса РОВНО 3
взаимоисключающих keystone (cost 4, exclusive-группа `<class>_keystones`,
активен ≤1, переключение купленных бесплатно). Числовой downside обязателен и
≥25% ценности апсайда в весах силы (`POWER_WEIGHTS` дата-модуля); «чистый вклад»
(последний столбец) — вклад keystone в damage-mult-эквивалент бюджета §6,
удержан в узкой полосе 0.036–0.054 ради кросс-классового спреда ≤1.25.
Эталоны §3 адаптированы под ключи, реально разведённые в `player.gd`
(источник данных: `CONSTELLATION_SPECS` в `scripts/meta_progression_tree_data.gd`;
код и таблица генерируются из одной спеки — расхождение = баг).

| Класс | Keystone | Апсайд | Downside | Чистый вклад |
| --- | --- | --- | --- | --- |
| **Берсерк** | «Кровавый танец» (`berserk_k0`) | +4% к шансу вампиризма; +0.5 к лечению вампиризмом | −30% к получаемому лечению | 0.051 |
|  | «Несущий бурю» (`berserk_k1`) | +9% к урону | −4% к макс. здоровью | 0.050 |
|  | «Последний рубеж» (`berserk_k2`) | +29% к урону при низком HP; +0.4 к регенерации при низком HP | −2.2% к защите | 0.049 |
| **Солдат** | «Подавляющий огонь» (`soldier_k0`) | +6% к скорости атаки; +3% к дальности атаки | −6% к скорости движения | 0.036 |
|  | «Одиночными» (`soldier_k1`) | +5% к урону; +1.8% к шансу крита | −4% к скорости атаки | 0.046 |
|  | «Гранатный подсумок» (`soldier_k2`) | +7% к радиусу области; +9% шанс взрыва при убийстве | −6% к дальности атаки | 0.039 |
| **Вор** | «Большой куш» (`thief_k0`) | +2.7% к шансу крита; +4% к скорости движения | −4% к макс. здоровью | 0.046 |
|  | «Ночная работа» (`thief_k1`) | +1.8% к уклонению; +10% к урону крита | −4% к урону | 0.051 |
|  | «Азарт канатоходца» (`thief_k2`) | +17% к урону крита; +9% рывок после уклонения | −2.2% к защите | 0.047 |
| **Элементалист** | «Сверхновая» (`elementalist_k0`) | +9% к радиусу области; +4% к урону | −6% к дальности атаки | 0.046 |
|  | «Канал ульты» (`elementalist_k1`) | +7% к силе ультимейта; +7% к заряду ультимейта | −4% к урону | 0.048 |
|  | «Огонь по площади» (`elementalist_k2`) | +11% шанс взрыва при убийстве; +1 периодического урона | −6% к скорости движения | 0.044 |
| **Снайпер** | «Один выстрел» (`sniper_k0`) | +16% к урону крита; +3% к урону | −4% к скорости атаки | 0.050 |
|  | «Свинцовый ветер» (`sniper_k1`) | +5% к скорости атаки; +33 к скорости снарядов | −12% к урону крита | 0.041 |
|  | «Гнездо ястреба» (`sniper_k2`) | +6% к дальности атаки; +2% к шансу крита | −6% к скорости движения | 0.040 |
| **Священник** | «Хор искупления» (`priest_k0`) | +3% к шансу вампиризма; +0.6 к лечению вампиризмом | −4% к урону | 0.050 |
|  | «Щит веры» (`priest_k1`) | +2.7% к защите; +4% к макс. здоровью | −6% к скорости движения | 0.046 |
|  | «Глас гнева» (`priest_k2`) | +5% к урону; +2.9% к силе поддержки | −2.2% к защите | 0.042 |
| **Биолог** | «Пандемия» (`biologist_k0`) | +1.4 периодического урона; +0.06 к скорости тиков | −4% к урону | 0.048 |
|  | «Симбионт» (`biologist_k1`) | +1 к силе призыва; +0.4 к лечению вампиризмом | −0.07 к скорости тиков | 0.050 |
|  | «Регенеративный цикл» (`biologist_k2`) | +0.15 к регенерации; +4% к макс. здоровью | −6% к скорости движения | 0.044 |
| **Робот** | «Овердрайв» (`robot_k0`) | +11% к заряду ультимейта; +5% к силе ультимейта | −2.2% к защите | 0.049 |
|  | «Осадный режим» (`robot_k1`) | +3 к поглощению; +4% к макс. здоровью | −6% к скорости движения | 0.037 |
|  | «Протокол мести» (`robot_k2`) | +11% шанс контр-волны; +18% отражение урона шипами | −4% к скорости атаки | 0.051 |
| **Инженер** | «Армия машин» (`engineer_k0`) | +1.1 к силе призыва; +2.2% к силе поддержки | −4% к урону | 0.053 |
|  | «Крепость-мастерская» (`engineer_k1`) | +2.7% к защите; +9 к радиусу ауры | −6% к скорости движения | 0.042 |
|  | «Перегретые стволы» (`engineer_k2`) | +33 к скорости снарядов; +5% к скорости атаки | −2.2% к защите | 0.042 |
| **Тёмный маг** | «Запретное знание» (`dark_mage_k0`) | +7% к урону; +6% к урону крита | −4% к макс. здоровью | 0.052 |
|  | «Гниль» (`dark_mage_k1`) | +1.4 периодического урона; +6% к радиусу области | −4% к урону | 0.048 |
|  | «Договор пустоты» (`dark_mage_k2`) | +0.9 к силе призыва; +14% к урону при низком HP | −30% к получаемому лечению | 0.044 |
| **Гитарист** | «Крещендо» (`guitarist_k0`) | +4% к силе поддержки; +11 к радиусу ауры | −4% к урону | 0.054 |
|  | «Пауэр-аккорд» (`guitarist_k1`) | +9% к отталкиванию; +5% к урону | −4% к силе поддержки | 0.036 |
|  | «Фронтмен» (`guitarist_k2`) | +7% к силе ультимейта; +7% к заряду ультимейта | −4% к макс. здоровью | 0.048 |
| **Ассасин** | «Из тени» (`assassin_k0`) | +17% к урону крита; +3% к скорости движения | −4% к макс. здоровью | 0.048 |
|  | «Тысяча порезов» (`assassin_k1`) | +5% к скорости атаки; +2.6% к шансу вампиризма | −12% к урону крита | 0.041 |
|  | «Призрачный шаг» (`assassin_k2`) | +1.8% к уклонению; +12% рывок после уклонения | −4% к урону | 0.050 |
| **Рейнджер** | «Град стрел» (`ranger_k0`) | +5% к скорости атаки; +6% к радиусу области | −6% к дальности атаки | 0.038 |
|  | «Выстрел навылет» (`ranger_k1`) | +5% к урону; +33 к скорости снарядов | −4% к скорости атаки | 0.046 |
|  | «Хозяин тропы» (`ranger_k2`) | +6% к скорости движения; +2.2% к шансу крита | −4% к макс. здоровью | 0.052 |
| **Доктор** | «Триаж» (`doctor_k0`) | +0.15 к регенерации; +4% к макс. здоровью | −4% к урону | 0.052 |
|  | «Полевая хирургия» (`doctor_k1`) | +0.6 к лечению вампиризмом; +3% к шансу вампиризма | −6% к скорости движения | 0.042 |
|  | «Доза адреналина» (`doctor_k2`) | +5% к скорости атаки; +2.9% к силе поддержки | −30% к получаемому лечению | 0.041 |
| **Химик** | «Каталитический распад» (`chemist_k0`) | +1.3 периодического урона; +0.07 к скорости тиков | −4% к урону | 0.051 |
|  | «Гремучая колба» (`chemist_k1`) | +7% к радиусу области; +9% шанс взрыва при убийстве | −1.2 периодического урона | 0.042 |
|  | «Пары эфира» (`chemist_k2`) | +6% к скорости движения; +1.5% к уклонению | −4% к макс. здоровью | 0.053 |
| **Рыцарь** | «Бастион» (`knight_k0`) | +2.5% к защите; +4% к макс. здоровью | −6% к скорости движения | 0.042 |
|  | «Марш легиона» (`knight_k1`) | +5% к урону; +4% к скорости движения | −2.2% к защите | 0.038 |
|  | «Шипастый панцирь» (`knight_k2`) | +27% отражение урона шипами; +7% шанс контр-волны | −4% к скорости атаки | 0.049 |
| **Друид** | «Вожак стаи» (`druid_k0`) | +1.1 к силе призыва; +7 к радиусу ауры | −4% к урону | 0.054 |
|  | «Дикий рост» (`druid_k1`) | +0.13 к регенерации; +4% к макс. здоровью | −6% к скорости движения | 0.037 |
|  | «Гнев леса» (`druid_k2`) | +1.4 периодического урона; +18% отражение урона шипами | −0.13 к регенерации | 0.043 |

Примечание к эталонам §3: «Кровавый танец» реализует вампирский стиль через
разведённые ключи vampiric_* с downside на получаемое лечение (аналог «лечение
лавки −50%» на уровне всего входящего лечения); «Один выстрел» → крит-пакет с
downside на скорость атаки; «Пандемия»/«Симбиоз» → DoT/призыв-пакеты биолога;
«Бастион»/«Марш легиона» → защитный и маршевый стили рыцаря. Формульные
триггеры («при HP<50%», «за каждые 2с движения») — кандидаты на carve-out 4.1
после появления соответствующих ключей в player.gd.

## Приложение B — реестр новых ключей эффектов

Заполнено в T1 (SCRUM-828). Новые ключи, разведённые этим тикетом (все — через
`apply_meta_skill_modifiers` в `player.gd`; потребители — существующие
артефакт-триггеры SCRUM-500):
...
| `kill_explosion_chance` | `soldier_k2`, `elementalist_t3/k2/h0`, `biologist_h0`, `engineer_t3`, `dark_mage_h0`, `chemist_t3/k1/h0` | `META_SKILL_FLAT_MAP` → триггер «взрыв при убийстве» | wired-гейт smoke |
