...
- Стиль — существующий кит leather/gold/parchment + новый слой «звёздная
  ...
  строго в safe-area рам (правило проекта). Пиксель-арт ассеты: фон-рама,
  3 типа сокетов, звезда аллокации, 17 гербов классов (эмблемы ядер).
...
2. **T2 art (design-lane)**: 17 гербов классов + недостающие UI-ассеты по
   списку §7 (PixelLab, фикс-margin, safe-area, alpha-cleanup по протоколу).
