...
│ └────┘                                                        │[Вло- ││
│                                                               │жить] ││
│ [Сброс умений — бесплатно]  [Легенда: ◉ куплено ◎ доступно ? скрыто] │
└──────────────────────────────────────────────────────────────────────┘
```
...
- **Панель узла справа**: русский титул, owning weapon + явный scope, затронутая
  ось, точный числовой эффект, `до → после`, прогресс пути, цена и кнопка
  «Вложить эмблему». Длинная механика живёт в scroll viewport, а цена/Buy
  закреплены ниже. Для финала отдельный callout показывает trigger, уникальную
  механику, caps/поведение против босса и floor силы относительно узла 5;
...
- **Сброс в footer** (SCRUM-1070): обе вкладки используют одну широкую
  `text/standard_420x104` пластину ровно 420 px по visible/hit width. Высота
  равна 72 px при viewport `<760`, 88 px при `<1000` и 104 px выше; compact
  action-текст закреплён на читаемых 21 px (role minimum 16 px). Подписи
  `Сброс умений` / `Сброс умений Атласа` остаются в одну строку без wrap,
  clip, ellipsis или fit-downscale. Кнопка и легенда делят только нижнюю пустую
  content-зону `AtlasSafeArea`; легенда уступает свободное место, но кнопка не
  сжимается и не заходит на орнамент. Tooltip, gamepad focus, подтверждение и
  полный возврат валюты выбранного scope остаются прежними.
...
дизайна; отступления — только через комментарий PM в Multica issue.
...
## SCRUM-1069 — усиленный Атлас гильдии

Guild Atlas сохраняет 25 ID, текущий граф, schema 5, стоимость `59` и cap
звёздной пыли `50`; старые покупки не респекаются и автоматически получают
новые значения. Четыре покупаемые ветви теперь дают:

- «Казна» — `+20%` золота, `+100` стартового золота и гарантированный
  эпический товар;
- «Лавка» — `−20%` цены магазина и докачки атрибутов плюс первая награда
  основной характеристики;
- «Знание» — `+15%` опыта, дополнительный вариант докачки и `100%` стартового
  заряда ультимейта;
- «Дорога» — `+60` радиуса подбора, `+20%` лечения и once-per-run death-save,
  восстанавливающий `30%` max HP.

Все minor-узлы соблюдают floors `5% / 25 gold / 30 pickup / 8% healing`.
Codex-hidden даёт `+10%` золота, Secret-Boss-hidden — `+50` стартового золота;
до выполнения условия оба эффекта отсутствуют. Death-save clamp `25..30%` и
persisted `death_save_used` исключают повтор/исцеляющий цикл.

Power formula source-aware: class constellation uses `POWER_WEIGHTS`, а Guild
часть — `GUILD_ATLAS_POWER_WEIGHTS`. Полный недоступный 59-cost Atlas даёт
account multiplier `1.174 < 1.35` и одинаковую class-power delta `0.174 ≤ 0.18`
для всех 17 классов; это строгий upper bound любой легальной 50-point сборки.
Точные before/after consumer/value tables:
