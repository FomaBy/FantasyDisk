...
- **FAN-1729 Прямая проверка soft-cap'а забеговых множителей в гейте Берсерка
  ...
  - Эталон `20t` на неизменённом `dev 82343091`, 20 изолированных прогонов:
    `8136..8615`, p50 `8267`, p95 `8591`, max `8615`, среднее `8317`; `1t` `931..967`.
  - Ложных красных нет: 20 изолированных прогонов на кандидате — 20/20 зелёных,
    `20t 8138..8615` (p50 `8263`, p95 `8612`), `1t 931..967`, false-red 0. Строка
    `[soft-cap]` во всех 20 прогонах побитово одинаковая (проверка детерминированная).
  - Чувствительность подтверждена в одноразовом чекауте: мутация
    ...
    `[soft-cap]`, при этом `[ceiling]` оставался зелёным (`20t 9444/9445/9852`,
    `1t 1105/1105/1065`), то есть пробел FAN-1713 воспроизведён и закрыт. Мутация
    `circle_target_diminish 0.62 → 0.0` — 3/3 КРАСНЫХ ровно по `[ceiling]`
    (`20t 19676/18810/18813`) при зелёном `[soft-cap]`. Обе мутации откачены.
