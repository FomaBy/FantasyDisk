...
- FAN-1078: re-entry магазина привязан к ветке, записанной в
  `route_selected_indices`. Соседние узлы рядом с выбранным магазином остаются
  locked; доступными становятся только узлы следующего ряда, связанные именно с
  этой веткой. Сам магазин остаётся единственным исключением для движения назад,
  чтобы случайный выход/Escape можно было отменить повторным входом.
