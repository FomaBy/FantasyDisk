...
The shared registry and Player contracts admit any complete ready data/executor
pair while incomplete or malformed pairs still fail closed to legacy fallback.
The runtime smoke oracle waits within each ready profile's declared timing
window, so telegraphed mechanics are required to produce bounded observable
impact and cleanup rather than immediate activation-frame damage.
