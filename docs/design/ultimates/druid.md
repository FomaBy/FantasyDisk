...
| --- | --- | --- |
| `summon_amulet` | Дикая Охота | Eight transient spectral beasts burst along the compass directions, then complete six fixed hunt waves against every live enemy. The nearby splash is only a local attribution bonus; all primary strikes reach the map. All spirits are activation-owned and vanish with the sigil. |
...
The three Druid slices were already delivered with map-wide runtime selection.
This aggregate card records that contract in the shared coverage ratchet and
removes only the obsolete declarations that the executors no longer read:
`target_cap`, `hunt_splash_target_cap`, `crowd_cap`, and
`dive_target_cap`. Pack size, seed count, wave/pulse counts, splash radius and
damage coefficients remain identity or per-target shaping; no Druid cast beat,
damage coefficient, control policy, boss cap or scene was changed here.

The rebuilt 51-row live artifact is `build/ultimate_effectiveness_baseline.json`.
It was generated once from the current `dev` revision and records the following
...
The class aggregate proof is `tests/ultimates/mechanics/druid_balance_test.gd`;
the package proof is `druid_package_test.gd`. The shared ratchet now requires
Druid to be both absent from `COVERAGE_MIGRATION_ALLOWLIST` and present in
`COVERAGE_V2_CLASSES`, with all three executor sources free of count-shaped
reach parameters. The per-weapon live tests remain the evidence for the three
already-integrated slices.
