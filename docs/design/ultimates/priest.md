...
| `priest_censer` | Нерушимый Обет | The giant censer's chain orbit is a 7.6s strong temporary absorb window. A local listener records the existing Player signal's actual `absorbed_amount`, stores only 65% up to its declared cap, and returns that stored value as one holy finish counter. No incoming hit means no counter damage. |
...
- Censer disconnects the existing actual-prevention listener when its node is
  freed. It adds no Player or ClassWeapon branch and never estimates prevented
  damage from a requested incoming hit.
...
## Shared-suite limitation

`tests/ultimates/controller_player_integration_test.gd` still hard-codes the
Biologist-only ready set (`SHIPPED_READY_CLASS`, `SHIPPED_READY_WEAPONS` and 48
legacy pairs). It is a shared foundation test outside this card's locked paths;
the package, live and registry tests above validate the Priest trio directly.
