...
Codex Documentation dispatcher может создавать новые задачи только как backlog
`0.1.5` (`Статус: new`, `Версия: 0.1.5`, Jira fixVersion `0.1.5`, вне active
sprint) или как явно текущие bug/QA defect/regression/release blocker задачи.
Остальные source tasks готовит PM/другая LLM. Статусы всех задач отслеживаются
на доске `docs/process/task_board.md`: при взятии задачи исполнитель ставит в
файле задачи `Статус: in_progress`, по завершении — `done` (или `review`) с
коротким резюме результата. Закрытие задачи — ответственность
исполнителя/ревьюера: dispatcher не ставит `done` за агента, а только
синхронизирует уже записанный результат или QA-вердикт.
