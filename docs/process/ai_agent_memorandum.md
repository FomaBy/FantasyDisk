...
- PM owns scope/readiness judgment. Qwen Operations Dispatcher alone performs
  ...
  the exact pushed candidate independently.
...
3. Implement the smallest complete task-owned change, matching surrounding
   patterns.
4. Run focused checks, then the smallest certifying regression/security checks
   required by risk.
...
then maps each acceptance criterion to executed evidence. QA never fixes
production code in the review scope and never calls a blocked or unexecuted
check PASS.
