...
New source/reference binaries belong only in `docs/design/reference-assets-lfs/<issue-or-pack>/<file>` as valid Git LFS pointers. New or changed binary destinations elsewhere under `docs/design/**` or `build/qa/**` are rejected. Runtime `assets/**` remain normal Git content. Release/tag operations require full history and an exact detached source; shallow/sparse candidate checkouts are never release sources.
