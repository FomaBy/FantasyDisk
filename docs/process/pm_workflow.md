...
issues в активном `Спринт 0.1.5` только для bug/QA defect/regression/release
blocker или явно разрешённых PM board-строк. Новые не-баговые feature requests
во время фриза получают `Версия: 0.1.6` и backlog Jira без active sprint.
...
## Feature Freeze (АКТИВЕН для 0.1.5 с 2026-06-14)

Фриз 0.1.5 включён директивой пользователя 2026-06-14. Активен
`Спринт 0.1.5` (id 67) — патч «Бой и баланс», но новые фичи в него больше не
добавляются. Доделываются только уже заведённые строки `docs/process/task_board.md`,
текущие bug/QA defect/regression/release blocker задачи и уже записанные
результаты исполнителей до Jira/QA sync. Новые не-баговые запросы оформляются
как `Версия: 0.1.6` и остаются вне active sprint до открытия следующей версии.
...
обычные задачи; Codex Documentation dispatcher во время фриза 0.1.5 создаёт
текущие `0.1.5` задачи только для bug/QA defect/regression/release blocker или
явно разрешённых PM board-строк. Новые не-баговые feature requests идут в
backlog следующей версии (`Версия: 0.1.6`). При dispatch, блокировке, review,
...
Feature block 0.1.5 включён 2026-06-14. Активен `Спринт 0.1.5`, но его scope
закрыт: можно dispatch'ить и переводить в `in_progress` только существующие
board rows 0.1.5, баги, QA defects, regressions, release blockers и owner nudges
по уже активной работе. Новые не-баговые запросы уходят в backlog следующей
версии с `Версия: 0.1.6`, без active sprint assignment и без dispatch до PM
override/открытия 0.1.6.
