...
Фриз включён по директиве пользователя. В спринт 0.1.4 идут ТОЛЬКО: уже начатые
(in_progress) задачи до завершения, баги, QA-дефекты, регрессии, release blockers.
Любой НОВЫЙ запрос (фича/улучшение/арт/контент) PM оформляет строкой
`Версия: 0.1.5` в task-файле → sync-скрипт уводит его в бэклог (Jira fixVersion
0.1.5, вне активного спринта), статус new, но НЕ dispatch и НЕ in_progress без
явного решения PM. Спорное (баг/фича) по умолчанию = backlog 0.1.5.
...
1. В текущий sprint/release выдаются только баги, QA-дефекты, регрессии и
   release blockers, а также доводятся уже начатые `in_progress` задачи 0.1.4.
