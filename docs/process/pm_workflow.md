...
3. Оценивает полный объём до Definition of Done по модели CUE и шкале
   Фибоначчи `1, 2, 3, 5, 8, 13`, записывает `Story points` с обоснованием;
...
7. Синхронизирует локальные mirrors с Multica: issue key в `.md`, строка на
   локальном dashboard, активный спринт и комментарии/статусы.
8. Следит, чтобы документация в `docs/design/` обновлялась вместе с изменениями.
9. Работает автономно: не задает пользователю вопросы, если требование можно разумно доформулировать самому. Спрашивает только при противоречии с текущим дизайном игры или при выборе, меняющем направление продукта.
10. Проверяет себя: перед выдачей задачи сверяется с `current_game_state.md`, `content_registry.md` и `mechanics_extract.md`, чтобы требования не противоречили уже реализованному.
...
Для всех actionable tasks действует единое правило
`docs/process/story_points.md`: относительная модель CUE (Complexity,
Uncertainty, Effort) и шкала Фибоначчи `1, 2, 3, 5, 8, 13`. Оценка включает не
только реализацию, но и ассеты, документацию, тесты, ручную проверку, интеграцию
и cleanup до Definition of Done.

CUE обсуждается целостно и **не складывается по формуле**: PM выбирает ближайший
относительный размер из канонической рубрики. PM не создаёт отдельную числовую
шкалу факторов, механическую сумму C, U и E или таблицу порогов; инструкции
могут только переносимо пересказывать `docs/process/story_points.md`.
