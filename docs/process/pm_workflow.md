...
## Feature Freeze (АКТИВЕН с 2026-06-13 — стабилизация 0.1.4)

Фриз включён по директиве пользователя. Поздний PM override пользователя
2026-06-13: **всю текущую board надо доделать в версии `0.1.4`**. Поэтому уже
существующие board-задачи остаются/возвращаются в текущий спринт и дожимаются до
QA/release. Любой НОВЫЙ запрос после этой директивы (фича/улучшение/арт/контент)
PM оформляет строкой `Версия: 0.1.5` в task-файле → sync-скрипт уводит его в
бэклог (Jira fixVersion 0.1.5, вне активного спринта), статус new, но НЕ
dispatch и НЕ in_progress без явного решения PM. Спорное (баг/фича), чего ещё
нет на board, по умолчанию = backlog 0.1.5.
...
## Feature Block (активный механизм для стабилизации 0.1.4)

Feature block снова активен с 2026-06-13 по директиве пользователя: текущий
релиз стабилизируется как `0.1.4`, а всё следующее уходит в `0.1.5`.

1. В текущий sprint/release доводится вся уже существующая board, включая
   deferred-by-freeze задачи, плюс баги, QA-дефекты, регрессии и release blockers.
2. Любой новый запрос пользователя после override, который не является багом, PM оформляет как
   backlog-задачу `Версия: 0.1.5`.
...
5. Фриз снимается сразу после релиза `v0.1.4`; sprint `0.1.5` забирает backlog.
