...
Фриз включён по директиве пользователя. Критичная PM-коррекция 2026-06-13:
задачи с `Версия: 0.1.5` НЕ затягивать в `0.1.4`, не dispatch'ить и не менять на
`in_progress` до релиза `v0.1.4` или явного PM override. В текущий sprint идут
только уже активные задачи `0.1.4`/без версии, баги, QA-дефекты, регрессии,
release blockers и уже записанные executor results, которые нужно
синхронизировать. Любой новый запрос (фича/улучшение/арт/контент) PM оформляет
...
1. В текущий sprint/release доводятся уже активные задачи `0.1.4`/без версии,
