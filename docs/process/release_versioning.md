...
Текущий feature block активен для стабилизации `0.1.5` с 2026-06-14: в текущий
релиз остаются только уже заведенные строки доски, bugfix/QA defect/regression/
release-blocker задачи и явно разрешенные PM исключения. Новые не-баговые фичи
оформляются как `Версия: 0.1.6`, остаются вне текущего sprint и не пополняют
changelog 0.1.5 после включения блока. При планировании новой версии backlog
переносится в её спринт.
