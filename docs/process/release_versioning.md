...
10. **Jira-спринт**: завершить активный спринт (complete) на доске SCRUM —
    Готово-тикеты уходят в отчет спринта, незакрытые переносятся; создать и
    запустить следующий «Спринт X.Y.Z+1» (цель = следующий релиз). Новые тикеты
    попадают в активный спринт автоматически (tools/jira_board_sync.py). Готово.

## Feature Block

С 2026-06-12 включен feature block для релизной стабилизации `0.1.3`.

- В текущий релиз допускаются только bugfix/regression/release-blocker задачи.
- Новые фичи и любые не-баговые изменения заводятся в backlog с target version
  `0.1.4` и не включаются в текущий sprint.
- Release notes/changelog для `0.1.3` не должны пополняться новыми фичами после
  включения feature block; исключение — исправления багов и QA-дефектов.
- При планировании `0.1.4` backlog-задачи отдельно переносятся в новый sprint.
