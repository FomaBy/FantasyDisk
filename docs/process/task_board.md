...
Ведёт: PM. Доска показывает ТОЛЬКО активную работу. Завершённые задачи (≈199) не
...
| --- | --- | --- | --- |
| [bug_hero_select_radar_out_of_frame_description_left_task.md](../tasks/bug_hero_select_radar_out_of_frame_description_left_task.md) | Back-end (UI) | new | БАГ 0.1.4: роза ветров внутри рамки (регрессия SCRUM-224) — вынести в правый верхний угол вне рамки, описание слева, no-overlap |
| [backend_ascension_per_level_changes_near_start_task.md](../tasks/backend_ascension_per_level_changes_near_start_task.md) | Back-end (UI) | new (БЭКЛОГ 0.1.5) | ФРИЗ: у кнопки старта показывать ТОЛЬКО изменение выбранного уровня возвышения (дельта, не кумулятив); не dispatch до релиза 0.1.4 |
...
| [design_codex_new_bosses_mini_elites_sprites_task.md](../tasks/design_codex_new_bosses_mini_elites_sprites_task.md) | Design (Codex генерация) → Claude-Designer | review | Jira: SCRUM-156; implemented 2026-06-13. 3 boss + 6 mini-elite painterly D&D source sprites generated at stable `assets/sprites/bosses/` and `assets/sprites/elites/mini_*.png` paths, previews `new_bosses_mini_elites_contact.png` and scale sheet ready; PNG validation + Godot import + runtime/animation smoke pass; Back-end SCRUM-155 and Animator SCRUM-204 handoff notes updated |
...
| [backend_test_runtime_smoke_suite_split_task.md](../tasks/backend_test_runtime_smoke_suite_split_task.md) | Back-end | done | Jira: SCRUM-202; umbrella runtime smoke preserved, focused suites added for UI, combat, progression/economy, weapon mechanics and boss/elite; all focused suites + umbrella passed headless |
| [backend_refactor_class_weapon_mode_registry_task.md](../tasks/backend_refactor_class_weapon_mode_registry_task.md) | Back-end | in_progress | Jira: SCRUM-196; unblocked/dispatched Back-end `019eabd9-780b-78a2-9f4b-e7203d659ef2` 2026-06-13 as serialized queue item 2 after SCRUM-202. `scripts/class_weapon.gd` clean; preserve all weapon behaviors; keep High/no low |
...
≈199 завершённых задач не показываются на доске. Полный список и история —
