...
| --- | --- | --- | --- |
| [backend_quit_confirmation_dialog_and_escape_task.md](../tasks/backend_quit_confirmation_dialog_and_escape_task.md) | Back-end (UI) | new | UX 0.1.5: диалог подтверждения выхода (кнопка «Выйти» больше не quit сразу) + Escape в главном меню открывает тот же диалог; модальный, фокус на «Отмена» |
