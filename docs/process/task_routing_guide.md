...
6. Для каждой реализации/арта/анимации диспетчер создает парную задачу
   `qa_review_<source_task_stem>_task.md` для Claude-QA. Это правило действует
...
- **QA-контур Claude**: через task board. Для каждой работы, которая пошла в
  Back-end/Design/Animator, должна существовать QA review task со ссылкой на
  исходный task-файл, acceptance criteria, ожидаемые проверки и область риска.
  Если позже появится явный QA-чат/thread id, диспетчер может дополнительно
  отправлять туда handoff, но не должен создавать новое окно.
