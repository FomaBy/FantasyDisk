...
В текущем спринте добиваются только уже начатые `in_progress` задачи, баги,
QA-дефекты, регрессии и release blockers. Новые фичи/улучшения/арт/контент
оформляются с `Версия: 0.1.5`, остаются в backlog/fixVersion `0.1.5`, не
добавляются в active sprint и не отправляются исполнителям до релиза `v0.1.4`.
Спорное по умолчанию считать фичей `0.1.5`. Фриз снимается сразу после релиза
`v0.1.4`; новый sprint `0.1.5` забирает накопленный backlog.
