...
Feature freeze включён пользователем 2026-06-13 для стабилизации `0.1.4`, но
позднее в тот же день пользователь уточнил: **всю текущую board надо доделать в
этой версии**. Значит уже существующие board-задачи раздаются/дожимаются до
QA/release в `0.1.4`; новые фичи/улучшения/арт/контент после этой директивы
оформляются с `Версия: 0.1.5`, остаются в backlog/fixVersion `0.1.5`, не
добавляются в active sprint и не отправляются исполнителям без явного PM
override. Спорное, чего ещё нет на board, по умолчанию считать фичей `0.1.5`.
Фриз снимается сразу после релиза `v0.1.4`; новый sprint `0.1.5` забирает
накопленный backlog.
