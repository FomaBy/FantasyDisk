...
- текущая разработка: `Спринт 0.1.7` на `dev`; `Спринт 0.1.6` закрыт 2026-06-27,
  `v0.1.6` влита в `main`.

## Feature Block

Feature block 0.1.5 снят релизом `v0.1.5` (2026-06-15). Сейчас активен
`Спринт 0.1.7`: задачи 0.1.7 ведутся через Jira (task board — локальное зеркало),
...
| `dev` | Активная рабочая ветка `0.1.x` | Работать по Jira (task board — зеркало); параллельные Codex/Claude задачи разводить через owner и locked paths |
...
2. `dev` используется для активной разработки текущей версии `0.1.7`.
3. Активные задачи `0.1.7`, баги, QA defects и release blockers закрываются в
...
- `dev` используется для активного `Спринт 0.1.7`; `Спринт 0.1.6` закрыт в Jira (66/66 готово).
