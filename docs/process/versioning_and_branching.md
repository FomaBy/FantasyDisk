...
- текущая стабилизация: `0.1.4`; задачи с `Версия: 0.1.5` остаются backlog до
  релиза `v0.1.4` или явного PM override.
...
- Критичная PM-коррекция 2026-06-13: задачи с `Версия: 0.1.5` не затягивать в
  `0.1.4`, не dispatch'ить и не менять на `in_progress` до релиза `v0.1.4` или
  явного PM override.
- Любые новые фичи, улучшения, балансные изменения, новый арт/анимации/UI,
  которые не исправляют баг, заводятся в backlog как scope версии `0.1.5`.
...
| `dev` | Активная рабочая ветка `0.1.x` | Стабилизация `0.1.4`; `Версия: 0.1.5` остается backlog до релиза/PM override |
...
3. Активные задачи `0.1.4`, баги и release blockers закрываются в `0.1.4`;
   задачи `Версия: 0.1.5` во время feature block копятся в backlog.
