...
- Exact commit: `8553691c7390be492e8edf8df59a5cce3a26e5bc` (tree
  `a63263bdc450a1eb3ebc213444f4707c5510de06`), `origin/dev`, checked out clean
  (`git status` clean throughout the run).
- Machine: MacBook, Apple M4 Pro (`Mac16,8`), 48 GiB RAM, macOS 26.5.1
  ...
  thermal or performance warning at any point during the run.
...
was warmed once for all scenarios by the setup import pass above.
...
| dense_wave | Плотная волна | `fight battle`; `spawn basic 40`; `spawn shooter 8` |
...
repeats (same `$HOME`, reused shader cache) = 9 runs total. Each run: 12 s
discarded warm-up window (post-`fight`, scenario setup settling) + 45 s
measured steady-state capture window, real frame-by-frame sampling (not a
fixed-timestep tick — `frame_delta_ms` is wall-clock time between successive
real frames). Raw CSVs (9 measurement files + 3 subsystem-correlation detail
files, ~15k rows total) are attached to the Multica card as disposable
evidence, not committed to the repo.
...
| dense_wave / cold | 5211 | 9.74 | 14.14 | 15.45 | 544.7 | 5 |
| dense_wave / warm1 | 5046 | 9.54 | 14.20 | 16.26 | 546.6 | 11 |
| dense_wave / warm2 | 5183 | 10.57 | 14.22 | 15.10 | 573.6 | 5 |
...
p50/p95/p99 are flat and healthy (~10-15 ms, consistent with the ~120 Hz
...
tail spikes: every single one of the 9 windows, cold **and** warm, produced
...
- `dense_wave/warm1`: t=3.2s (507.1ms), t=21.7s (546.6ms)
...
~1000 ms); `vfx_heavy` produces more frequent but smaller spikes (80-190 ms).
...
cold and warm `$HOME` states (e.g. `vfx_heavy/warm1` warm-up: 5 spikes, same
count order as its own cold run). Because warm `$HOME` already has a
...
primitives, and 2D physics activity per frame.
...
| vfx_heavy | 8 | 14.3-19.1 ms | 0.9-15.3 ms | same range |
...
- Reproduction: the 3-contour scenario matrix and dev-console command
  ...
  `gl_compatibility`.
- Current baseline (this report, 3 repeats/contour): 5-11 frames >50ms per
  45 s window in every contour and every cold/warm state; worst observed
  frame 992.4 ms (`ordinary/warm2`).
...
Only this report file was added. No files under `scripts/`, `scenes/`,
