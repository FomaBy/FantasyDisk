# Animation: Кислотная колба (acid_flask) attack VFX redraw
...
Jira: SCRUM-727
Locked paths: assets/sprites/effects/vfx_weapon_acid_flask.png, docs/design/references/weapon_attack_animations/acid_flask/, docs/design/previews/weapon_attack_animations/acid_flask_contact.png, scenes/AcidFlask.tscn, assets/sprites/weapons/acid_flask.png
...
Это задача только для `acid_flask` / **Кислотная колба** класса **Химик**.
Текущая механическая роль: Большая poison/acid pool; combo explosion с другим элементом.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/acid_flask/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_acid_flask.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `acid_flask` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Кислотная колба**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Большая poison/acid pool; combo explosion с другим элементом. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `acid_flask` использовать reference `assets/sprites/weapons/acid_flask.png` и текущую сцену `scenes/AcidFlask.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_acid_flask.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Кислотная колба**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
