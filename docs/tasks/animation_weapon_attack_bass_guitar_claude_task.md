# Animation: Бас-гитара (bass_guitar) attack VFX redraw
...
Jira: SCRUM-729
Locked paths: assets/sprites/effects/vfx_weapon_bass_guitar.png, docs/design/references/weapon_attack_animations/bass_guitar/, docs/design/previews/weapon_attack_animations/bass_guitar_contact.png, scenes/BassGuitar.tscn, assets/sprites/weapons/bass_guitar.png
...
Это задача только для `bass_guitar` / **Бас-гитара** класса **Гитарист**.
Текущая механическая роль: Частый слабый контроль-пульс с сильным отталкиванием.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/bass_guitar/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_bass_guitar.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `bass_guitar` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Бас-гитара**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Частый слабый контроль-пульс с сильным отталкиванием. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `bass_guitar` использовать reference `assets/sprites/weapons/bass_guitar.png` и текущую сцену `scenes/BassGuitar.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_bass_guitar.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Бас-гитара**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
