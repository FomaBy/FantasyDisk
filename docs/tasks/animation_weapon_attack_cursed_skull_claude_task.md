# Animation: Проклятый череп (cursed_skull) attack VFX redraw
...
Jira: SCRUM-737
Locked paths: assets/sprites/effects/vfx_weapon_cursed_skull.png, docs/design/references/weapon_attack_animations/cursed_skull/, docs/design/previews/weapon_attack_animations/cursed_skull_contact.png, scenes/CursedSkull.tscn, assets/sprites/weapons/cursed_skull.png
...
Это задача только для `cursed_skull` / **Проклятый череп** класса **Темный маг**.
Текущая механическая роль: Самонаводящееся проклятие, DoT и небольшой splash по цели.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/cursed_skull/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_cursed_skull.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `cursed_skull` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Проклятый череп**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Самонаводящееся проклятие, DoT и небольшой splash по цели. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `cursed_skull` использовать reference `assets/sprites/weapons/cursed_skull.png` и текущую сцену `scenes/CursedSkull.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_cursed_skull.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Проклятый череп**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
