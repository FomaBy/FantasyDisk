# Animation: Книга тьмы (dark_book) attack VFX redraw
...
Jira: SCRUM-738
Locked paths: assets/sprites/effects/vfx_weapon_dark_book.png, docs/design/references/weapon_attack_animations/dark_book/, docs/design/previews/weapon_attack_animations/dark_book_contact.png, scenes/DarkBook.tscn, assets/sprites/weapons/dark_book.png
...
Это задача только для `dark_book` / **Книга тьмы** класса **Темный маг**.
Текущая механическая роль: Два AoE-снаряда в две ближайшие цели.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/dark_book/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_dark_book.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `dark_book` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Книга тьмы**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Два AoE-снаряда в две ближайшие цели. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `dark_book` использовать reference `assets/sprites/weapons/dark_book.png` и текущую сцену `scenes/DarkBook.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_dark_book.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Книга тьмы**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
