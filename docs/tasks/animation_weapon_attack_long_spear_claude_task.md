...
- `assets/sprites/effects/vfx_weapon_long_spear.png`: 256x256 RGBA, все 4 угла alpha=0, 0% полностью непрозрачных пикселей (весь слой полупрозрачный), видимое покрытие ~12.2%, median saturation 20 → без запечённого нейтрального/checkerboard фона; центр остаётся readable. `.import` сайдкар закоммичен, размеры сохранены (reimport не требуется).
- Визуал (SCRUM-751): вертикальный силуэт копья + узкий пронзающий strip вниз (роль «длинный точечный strip»); зона-коридор читается. Уникально читается как своё оружие, отличим от 10 соседних VFX (pairwise avg-hash — 0 near-dup пар из 55).
