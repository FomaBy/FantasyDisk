# Animation: Лунный арбалет (moon_crossbow) attack VFX redraw
...
Jira: SCRUM-752
Locked paths: assets/sprites/effects/vfx_weapon_moon_crossbow.png, docs/design/references/weapon_attack_animations/moon_crossbow/, docs/design/previews/weapon_attack_animations/moon_crossbow_contact.png, scenes/MoonCrossbow.tscn, assets/sprites/weapons/moon_crossbow.png
...
Это задача только для `moon_crossbow` / **Лунный арбалет** класса **Рейнджер**.
Текущая механическая роль: Stance-charged piercing shot.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/moon_crossbow/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_moon_crossbow.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `moon_crossbow` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Лунный арбалет**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Stance-charged piercing shot. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `moon_crossbow` использовать reference `assets/sprites/weapons/moon_crossbow.png` и текущую сцену `scenes/MoonCrossbow.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_moon_crossbow.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Лунный арбалет**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
...
QA проверяет именно `moon_crossbow` в игре: эффект виден при атаке, полупрозрачен, не заслоняет UI и отличим от соседних weapon VFX. Disk cleanup обязателен: удалить временные OpenAI/source scratch caches, оставить только committed source/evidence/runtime files.
...
- OpenAI `gpt-image-2`, quality=high, size=1024x1024; border-connected flood-fill alpha-cleanup (запечённый фон → прозрачный); downscale 256x256 LANCZOS;
  ...
  **ghost'а настоящего спрайта оружия** `assets/sprites/weapons/moon_crossbow.png`.
- Runtime: `assets/sprites/effects/vfx_weapon_moon_crossbow.png` (256×256 RGBA, размеры сохранены → .import валиден, reimport не требуется).
- Source + prompt manifest: `docs/design/references/weapon_attack_animations/moon_crossbow/vfx_source_1024.png`, `.../prompt_manifest.json`.
- Preview (тёмный/светлый фон): `docs/design/previews/weapon_attack_animations/moon_crossbow_contact.png`.
...
- `assets/sprites/effects/vfx_weapon_moon_crossbow.png`: 256x256 RGBA, все 4 угла alpha=0, 0% полностью непрозрачных пикселей (весь слой полупрозрачный), видимое покрытие ~22.7%, median saturation 26 → без запечённого нейтрального/checkerboard фона; центр остаётся readable. `.import` сайдкар закоммичен, размеры сохранены (reimport не требуется).
- Визуал (SCRUM-752): горизонтальный лунно-белый болт-луч, летящий вбок — направление и дальность выстрела читаются. Уникально читается как своё оружие, отличим от 10 соседних VFX (pairwise avg-hash — 0 near-dup пар из 55).
