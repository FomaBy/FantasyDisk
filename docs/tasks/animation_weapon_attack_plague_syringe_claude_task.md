# Animation: Чумной шприц (plague_syringe) attack VFX redraw
...
Jira: SCRUM-753
Locked paths: assets/sprites/effects/vfx_weapon_plague_syringe.png, docs/design/references/weapon_attack_animations/plague_syringe/, docs/design/previews/weapon_attack_animations/plague_syringe_contact.png, scenes/PlagueSyringe.tscn, assets/sprites/weapons/plague_syringe.png
...
Это задача только для `plague_syringe` / **Чумной шприц** класса **Доктор**.
Текущая механическая роль: Drain-связь с poison DoT и sustain.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/plague_syringe/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_plague_syringe.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `plague_syringe` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Чумной шприц**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Drain-связь с poison DoT и sustain. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `plague_syringe` использовать reference `assets/sprites/weapons/plague_syringe.png` и текущую сцену `scenes/PlagueSyringe.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_plague_syringe.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Чумной шприц**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
...
QA проверяет именно `plague_syringe` в игре: эффект виден при атаке, полупрозрачен, не заслоняет UI и отличим от соседних weapon VFX. Disk cleanup обязателен: удалить временные OpenAI/source scratch caches, оставить только committed source/evidence/runtime files.
...
- OpenAI `gpt-image-2`, quality=high, size=1024x1024; border-connected flood-fill alpha-cleanup (запечённый фон → прозрачный); downscale 256x256 LANCZOS;
  ...
  **ghost'а настоящего спрайта оружия** `assets/sprites/weapons/plague_syringe.png`.
- Runtime: `assets/sprites/effects/vfx_weapon_plague_syringe.png` (256×256 RGBA, размеры сохранены → .import валиден, reimport не требуется).
- Source + prompt manifest: `docs/design/references/weapon_attack_animations/plague_syringe/vfx_source_1024.png`, `.../prompt_manifest.json`.
- Preview (тёмный/светлый фон): `docs/design/previews/weapon_attack_animations/plague_syringe_contact.png`.
