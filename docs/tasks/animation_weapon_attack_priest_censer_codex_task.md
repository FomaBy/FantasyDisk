# Animation: Кадило Обета (priest_censer) attack VFX redraw
...
Jira: SCRUM-754
Locked paths: assets/sprites/effects/vfx_weapon_priest_censer.png, docs/design/references/weapon_attack_animations/priest_censer/, docs/design/previews/weapon_attack_animations/priest_censer_contact.png, scenes/PriestCenser.tscn, assets/sprites/weapons/priest_censer.png
...
Это задача только для `priest_censer` / **Кадило Обета** класса **Священник**.
Текущая механическая роль: Ward pulses: несколько защитных волн вокруг героя наносят урон и дают малое лечение.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/priest_censer/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_priest_censer.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `priest_censer` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Кадило Обета**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Ward pulses: несколько защитных волн вокруг героя наносят урон и дают малое лечение. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `priest_censer` использовать reference `assets/sprites/weapons/priest_censer.png` и текущую сцену `scenes/PriestCenser.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_priest_censer.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Кадило Обета**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
...
QA проверяет именно `priest_censer` в игре: эффект виден при атаке, полупрозрачен, не заслоняет UI и отличим от соседних weapon VFX. Disk cleanup обязателен: удалить временные OpenAI/source scratch caches, оставить только committed source/evidence/runtime files.
