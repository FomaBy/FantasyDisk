...
- OpenAI `gpt-image-2`, quality=high, size=1024x1024; солид-чёрный фон + глобальный dark-dropout (замкнутый центр кольца → прозрачный); downscale 256x256 LANCZOS;
  ...
  **ghost'а настоящего спрайта оружия** `assets/sprites/weapons/raven_totem.png`.
- Runtime: `assets/sprites/effects/vfx_weapon_raven_totem.png` (256×256 RGBA, размеры сохранены → .import валиден, reimport не требуется).
- Source + prompt manifest: `docs/design/references/weapon_attack_animations/raven_totem/vfx_source_1024.png`, `.../prompt_manifest.json`.
- Preview (тёмный/светлый фон): `docs/design/previews/weapon_attack_animations/raven_totem_contact.png`.
...
- `assets/sprites/effects/vfx_weapon_raven_totem.png`: 256x256 RGBA, все 4 угла alpha=0, 0% полностью непрозрачных пикселей (весь слой полупрозрачный), видимое покрытие ~31.0%, median saturation 165 → без запечённого нейтрального/checkerboard фона; центр остаётся readable. `.import` сайдкар закоммичен, размеры сохранены (reimport не требуется).
- Визуал (SCRUM-757): тёмно-фиолетовое кольцо вороньих перьев/тьмы вокруг цели (тотем-аура). Уникально читается как своё оружие, отличим от 10 соседних VFX (pairwise avg-hash — 0 near-dup пар из 55).
