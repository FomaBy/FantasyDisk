# Animation: Зелье восстановления (restore_potion) attack VFX redraw
...
Jira: SCRUM-758
Locked paths: assets/sprites/effects/vfx_weapon_restore_potion.png, docs/design/references/weapon_attack_animations/restore_potion/, docs/design/previews/weapon_attack_animations/restore_potion_contact.png, scenes/RestorePotion.tscn, assets/sprites/weapons/restore_potion.png
...
Это задача только для `restore_potion` / **Зелье восстановления** класса **Доктор**.
Текущая механическая роль: Drain/lifesteal-связь к цели.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/restore_potion/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_restore_potion.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `restore_potion` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Зелье восстановления**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Drain/lifesteal-связь к цели. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `restore_potion` использовать reference `assets/sprites/weapons/restore_potion.png` и текущую сцену `scenes/RestorePotion.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_restore_potion.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Зелье восстановления**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
...
QA проверяет именно `restore_potion` в игре: эффект виден при атаке, полупрозрачен, не заслоняет UI и отличим от соседних weapon VFX. Disk cleanup обязателен: удалить временные OpenAI/source scratch caches, оставить только committed source/evidence/runtime files.
...
- OpenAI `gpt-image-2`, quality=high, size=1024x1024; солид-чёрный фон + глобальный dark-dropout (замкнутый центр кольца → прозрачный); downscale 256x256 LANCZOS;
  ...
  **ghost'а настоящего спрайта оружия** `assets/sprites/weapons/restore_potion.png`.
- Runtime: `assets/sprites/effects/vfx_weapon_restore_potion.png` (256×256 RGBA, размеры сохранены → .import валиден, reimport не требуется).
- Source + prompt manifest: `docs/design/references/weapon_attack_animations/restore_potion/vfx_source_1024.png`, `.../prompt_manifest.json`.
- Preview (тёмный/светлый фон): `docs/design/previews/weapon_attack_animations/restore_potion_contact.png`.
...
- `assets/sprites/effects/vfx_weapon_restore_potion.png`: 256x256 RGBA, все 4 угла alpha=0, 0% полностью непрозрачных пикселей (весь слой полупрозрачный), видимое покрытие ~35.3%, median saturation 186 → без запечённого нейтрального/checkerboard фона; центр остаётся readable. `.import` сайдкар закоммичен, размеры сохранены (reimport не требуется).
- Визуал (SCRUM-758): золотисто-жёлтое сияющее кольцо восстановления (зона хила/пул). Уникально читается как своё оружие, отличим от 10 соседних VFX (pairwise avg-hash — 0 near-dup пар из 55).
