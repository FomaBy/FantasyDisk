# Animation: Амулет призыва (summon_amulet) attack VFX redraw
...
Jira: SCRUM-771
Locked paths: assets/sprites/effects/vfx_weapon_summon_amulet.png, docs/design/references/weapon_attack_animations/summon_amulet/, docs/design/previews/weapon_attack_animations/summon_amulet_contact.png, scenes/SummonAmulet.tscn, assets/sprites/weapons/summon_amulet.png
...
Это задача только для `summon_amulet` / **Амулет призыва** класса **Друид**.
Текущая механическая роль: Командуемая beast pack, scaling from Leadership.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/summon_amulet/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_summon_amulet.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `summon_amulet` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Амулет призыва**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Командуемая beast pack, scaling from Leadership. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `summon_amulet` использовать reference `assets/sprites/weapons/summon_amulet.png` и текущую сцену `scenes/SummonAmulet.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_summon_amulet.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Амулет призыва**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
...
QA проверяет именно `summon_amulet` в игре: эффект виден при атаке, полупрозрачен, не заслоняет UI и отличим от соседних weapon VFX. Disk cleanup обязателен: удалить временные OpenAI/source scratch caches, оставить только committed source/evidence/runtime files.
...
- OpenAI `gpt-image-2`, quality=high, size=1024x1024; солид-чёрный фон + глобальный dark-dropout (замкнутый центр кольца → прозрачный); downscale 256x256 LANCZOS;
  ...
  **ghost'а настоящего спрайта оружия** `assets/sprites/weapons/summon_amulet.png`.
- Runtime: `assets/sprites/effects/vfx_weapon_summon_amulet.png` (256×256 RGBA, размеры сохранены → .import валиден, reimport не требуется).
- Source + prompt manifest: `docs/design/references/weapon_attack_animations/summon_amulet/vfx_source_1024.png`, `.../prompt_manifest.json`.
- Preview (тёмный/светлый фон): `docs/design/previews/weapon_attack_animations/summon_amulet_contact.png`.
...
- `assets/sprites/effects/vfx_weapon_summon_amulet.png`: 256x256 RGBA, все 4 угла alpha=0, 0% полностью непрозрачных пикселей (весь слой полупрозрачный), видимое покрытие ~39.2%, median saturation 216 → без запечённого нейтрального/checkerboard фона; центр остаётся readable. `.import` сайдкар закоммичен, размеры сохранены (reimport не требуется).
- Визуал (SCRUM-771): золотое кольцо призыва с узловыми точками по окружности (deploy-зона). Уникально читается как своё оружие, отличим от 10 соседних VFX (pairwise avg-hash — 0 near-dup пар из 55).
