# Animation: Башенный щит (tower_shield) attack VFX redraw
...
Jira: SCRUM-776
Locked paths: assets/sprites/effects/vfx_weapon_tower_shield.png, docs/design/references/weapon_attack_animations/tower_shield/, docs/design/previews/weapon_attack_animations/tower_shield_contact.png, scenes/TowerShield.tscn, assets/sprites/weapons/tower_shield.png
...
Это задача только для `tower_shield` / **Башенный щит** класса **Рыцарь**.
Текущая механическая роль: Shield bash / frontal control + сильный block/counter.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/tower_shield/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_tower_shield.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `tower_shield` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Башенный щит**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Shield bash / frontal control + сильный block/counter. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `tower_shield` использовать reference `assets/sprites/weapons/tower_shield.png` и текущую сцену `scenes/TowerShield.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_tower_shield.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Башенный щит**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
...
QA проверяет именно `tower_shield` в игре: эффект виден при атаке, полупрозрачен, не заслоняет UI и отличим от соседних weapon VFX. Disk cleanup обязателен: удалить временные OpenAI/source scratch caches, оставить только committed source/evidence/runtime files.
...
- OpenAI `gpt-image-2`, quality=high, size=1024x1024; border-connected flood-fill alpha-cleanup (запечённый фон → прозрачный); downscale 256x256 LANCZOS;
  ...
  **ghost'а настоящего спрайта оружия** `assets/sprites/weapons/tower_shield.png`.
- Runtime: `assets/sprites/effects/vfx_weapon_tower_shield.png` (256×256 RGBA, размеры сохранены → .import валиден, reimport не требуется).
- Source + prompt manifest: `docs/design/references/weapon_attack_animations/tower_shield/vfx_source_1024.png`, `.../prompt_manifest.json`.
- Preview (тёмный/светлый фон): `docs/design/previews/weapon_attack_animations/tower_shield_contact.png`.
...
- `assets/sprites/effects/vfx_weapon_tower_shield.png`: 256x256 RGBA, все 4 угла alpha=0, 0% полностью непрозрачных пикселей (весь слой полупрозрачный), видимое покрытие ~38.8%, median saturation 40 → без запечённого нейтрального/checkerboard фона; центр остаётся readable. `.import` сайдкар закоммичен, размеры сохранены (reimport не требуется).
- Визуал (SCRUM-776): золотой щит-купол/кольцо (защитная block-зона по периметру). Уникально читается как своё оружие, отличим от 10 соседних VFX (pairwise avg-hash — 0 near-dup пар из 55).
