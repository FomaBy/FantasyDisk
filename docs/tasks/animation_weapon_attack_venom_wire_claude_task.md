# Animation: Ядовитая струна (venom_wire) attack VFX redraw
...
Jira: SCRUM-777
Locked paths: assets/sprites/effects/vfx_weapon_venom_wire.png, docs/design/references/weapon_attack_animations/venom_wire/, docs/design/previews/weapon_attack_animations/venom_wire_contact.png, scenes/VenomWire.tscn, assets/sprites/weapons/venom_wire.png
...
Это задача только для `venom_wire` / **Ядовитая струна** класса **Ассасин**.
Текущая механическая роль: Тонкая poison-линия с DoT + crit shadow burst у цели.
...
Source PNG и prompt notes сохранить в `docs/design/references/weapon_attack_animations/venom_wire/`. Accepted runtime PNG обновлять только по пути `assets/sprites/effects/vfx_weapon_venom_wire.png`, если не создан отдельный backend/runtime handoff.

## Требования

1. Сделать attack VFX/animation для `venom_wire` уникальным: форма, движение, цветовой акцент и weapon silhouette должны читаться как **Ядовитая струна**, а не как generic recolor.
2. Визуально показать область действия из роли оружия: Тонкая poison-линия с DoT + crit shadow burst у цели. Форма зоны должна помогать игроку понять направление/радиус/коридор/пул/цепь без изменения реальной механики.
...
4. Во время атаки должен появляться ghost/silhouette самого оружия на фоне эффекта, как уже сделано у части текущих персонажей. Для `venom_wire` использовать reference `assets/sprites/weapons/venom_wire.png` и текущую сцену `scenes/VenomWire.tscn`.
...
- [ ] `assets/sprites/effects/vfx_weapon_venom_wire.png` обновлен или подтверждён как accepted с новым OpenAI source/evidence.
- [ ] Source, prompt notes/manifest и preview/contact sheet сохранены в task-specific paths.
- [ ] Attack VFX уникально отражает **Ядовитая струна**, показывает зону действия и содержит полупрозрачный фоновой силуэт оружия.
...
QA проверяет именно `venom_wire` в игре: эффект виден при атаке, полупрозрачен, не заслоняет UI и отличим от соседних weapon VFX. Disk cleanup обязателен: удалить временные OpenAI/source scratch caches, оставить только committed source/evidence/runtime files.
...
- OpenAI `gpt-image-2`, quality=high, size=1024x1024; border-connected flood-fill alpha-cleanup (запечённый фон → прозрачный); downscale 256x256 LANCZOS;
  ...
  **ghost'а настоящего спрайта оружия** `assets/sprites/weapons/venom_wire.png`.
- Runtime: `assets/sprites/effects/vfx_weapon_venom_wire.png` (256×256 RGBA, размеры сохранены → .import валиден, reimport не требуется).
- Source + prompt manifest: `docs/design/references/weapon_attack_animations/venom_wire/vfx_source_1024.png`, `.../prompt_manifest.json`.
- Preview (тёмный/светлый фон): `docs/design/previews/weapon_attack_animations/venom_wire_contact.png`.
...
- `assets/sprites/effects/vfx_weapon_venom_wire.png`: 256x256 RGBA, все 4 угла alpha=0, 0% полностью непрозрачных пикселей (весь слой полупрозрачный), видимое покрытие ~14.9%, median saturation 0 → без запечённого нейтрального/checkerboard фона; центр остаётся readable. `.import` сайдкар закоммичен, размеры сохранены (reimport не требуется).
- Визуал (SCRUM-777): горизонтальная ядовитая струна с узлом-выбросом на конце (линия/цепь поражения). Уникально читается как своё оружие, отличим от 10 соседних VFX (pairwise avg-hash — 0 near-dup пар из 55).
