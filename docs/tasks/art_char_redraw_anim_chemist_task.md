# ART: Перерисовать «Химик» в едином стиле + анимации (5 move / 5 attack)
...
задача для класса **Химик** (`chemist`).

## Образ персонажа (арт-дирекция)
Химик: алхимик в кожаном фартуке и респираторе, пятна реагентов; БЕЗ колб в руках.
Строго следовать единому style-sheet из опорной задачи (ракурс, палитра, контур,
pivot, тень). **Без оружия в руках.**

## Требования
1. Нарисовать «Химик» в едином стиле (по style-sheet опорной задачи), БЕЗ оружия
   в руках.
...
3. Положить ассет по шаблонному пути assets/sprites/characters/chemist_sheet.png и
...
4. Если класс делил чужой спрайт — отвязать, дать собственный; старую текстуру в
   бэкап (не удалять).
5. Тест (smoke/animation): «Химик» строится с "walk"(5)/"attack"(5), проигрывается
   без ошибок, без оружия в руках; превью-гиф в build/qa/.
6. CHANGELOG; content_registry.

## Acceptance Criteria
- [ ] «Химик» перерисован в едином стиле, без оружия в руках.
...
## Документация
docs/design/content_registry.md (персонаж chemist), current_game_state.
