# ART: Перерисовать «Доктор» в едином стиле + анимации (5 move / 5 attack)
...
задача для класса **Доктор** (`doctor`).

## Образ персонажа (арт-дирекция)
Доктор: полевой медик в халате и чумной маске, перчатки; БЕЗ инструментов, руки свободны.
Строго следовать единому style-sheet из опорной задачи (ракурс, палитра, контур,
pivot, тень). **Без оружия в руках.**

## Требования
1. Нарисовать «Доктор» в едином стиле (по style-sheet опорной задачи), БЕЗ оружия
   в руках.
...
3. Положить ассет по шаблонному пути assets/sprites/characters/doctor_sheet.png и
...
4. Если класс делил чужой спрайт — отвязать, дать собственный; старую текстуру в
   бэкап (не удалять).
5. Тест (smoke/animation): «Доктор» строится с "walk"(5)/"attack"(5), проигрывается
   без ошибок, без оружия в руках; превью-гиф в build/qa/.
6. CHANGELOG; content_registry.

## Acceptance Criteria
- [ ] «Доктор» перерисован в едином стиле, без оружия в руках.
...
## Документация
docs/design/content_registry.md (персонаж doctor), current_game_state.
...
Лист скиллом fantasydisk-asset-generator (1920x1152, idle/walk/attack, без оружия) -> tools/build_character_sheet.py: flood-fill фон + центровка + нарезка в full_frame/doctor/ + авторинг doctor_spriteframes.tres. player.gd грузит .tres по конвенции. animation_smoke зелёный (exit 0).


## QA-Вердикт (2026-06-14)
Статус: PASSED — «Доктор» перерисован + анимирован, играет в игре

Проверено (фактически):
- **Frame counts** (`doctor_spriteframes.tres`): **walk 5 / attack 5** (+attack_primary 5,
...
- **Визуал** (`doctor_sheet_normalized.png` / контакт): перерисован в едином D&D
...
Acceptance:
- [x] «Доктор» перерисован в едином стиле, без оружия в руках.
