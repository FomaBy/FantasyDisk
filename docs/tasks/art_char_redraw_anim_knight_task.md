# ART: Перерисовать «Рыцарь» в едином стиле + анимации (5 move / 5 attack)
...
задача для класса **Рыцарь** (`knight`).

## Образ персонажа (арт-дирекция)
Рыцарь: тяжёлый латник в полном доспехе с плюмажем, но БЕЗ меча и щита в руках, руки свободны.
Строго следовать единому style-sheet из опорной задачи (ракурс, палитра, контур,
pivot, тень). **Без оружия в руках.**

## Требования
1. Нарисовать «Рыцарь» в едином стиле (по style-sheet опорной задачи), БЕЗ оружия
   в руках.
...
3. Положить ассет по шаблонному пути assets/sprites/characters/knight_sheet.png и
...
4. Если класс делил чужой спрайт — отвязать, дать собственный; старую текстуру в
   бэкап (не удалять).
5. Тест (smoke/animation): «Рыцарь» строится с "walk"(5)/"attack"(5), проигрывается
   без ошибок, без оружия в руках; превью-гиф в build/qa/.
6. CHANGELOG; content_registry.

## Acceptance Criteria
- [ ] «Рыцарь» перерисован в едином стиле, без оружия в руках.
...
## Документация
docs/design/content_registry.md (персонаж knight), current_game_state.
...
Лист скиллом fantasydisk-asset-generator (1920x1152, idle/walk/attack, без оружия) -> tools/build_character_sheet.py: flood-fill фон + центровка + нарезка в full_frame/knight/ + авторинг knight_spriteframes.tres. player.gd грузит .tres по конвенции. animation_smoke зелёный (exit 0).


## QA-Вердикт (2026-06-14)
Статус: PASSED — «Рыцарь» перерисован + анимирован, играет в игре

Проверено (фактически):
- **Frame counts** (`knight_spriteframes.tres`): **walk 5 / attack 5** (+attack_primary 5,
...
- **Визуал** (`knight_sheet_normalized.png` / контакт): перерисован в едином D&D
...
Acceptance:
- [x] «Рыцарь» перерисован в едином стиле, без оружия в руках.
