...
задача для класса **Священник** (`priest`).

## Образ персонажа (арт-дирекция)
Священник: жрец света в бело-золотой рясе, нимб/священный знак, благословляющая поза; БЕЗ оружия, руки свободны.
- Сейчас делит чужой спрайт (doctor.png) — нужен СОБСТВЕННЫЙ уникальный спрайт.
Строго следовать единому style-sheet из опорной задачи (ракурс, палитра, контур,
pivot, тень). **Без оружия в руках.**

## Требования
1. Нарисовать «Священник» в едином стиле (по style-sheet опорной задачи), БЕЗ оружия
   в руках.
...
3. Положить ассет по шаблонному пути assets/sprites/characters/priest_sheet.png и
...
4. Если класс делил чужой спрайт — отвязать, дать собственный; старую текстуру в
   бэкап (не удалять).
5. Тест (smoke/animation): «Священник» строится с "walk"(5)/"attack"(5), проигрывается
   без ошибок, без оружия в руках; превью-гиф в build/qa/.
6. CHANGELOG; content_registry.

## Acceptance Criteria
- [ ] «Священник» перерисован в едином стиле, без оружия в руках.
...
## Документация
docs/design/content_registry.md (персонаж priest), current_game_state.
...
Статус: PASSED — «Жрец» перерисован + анимирован, играет в игре

Проверено (фактически):
- **Frame counts** (`priest_spriteframes.tres`): **walk 5 / attack 5** (+attack_primary 5,
...
- **Визуал** (`priest_sheet_normalized.png` / контакт): перерисован в едином D&D
...
Acceptance:
- [x] «Жрец» перерисован в едином стиле, без оружия в руках.
