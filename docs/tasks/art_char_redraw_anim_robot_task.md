# ART: Перерисовать «Робот» в едином стиле + анимации (5 move / 5 attack)
...
задача для класса **Робот** (`robot`).

## Образ персонажа (арт-дирекция)
Робот: механический страж, тяжёлый бронекорпус, светящиеся сенсоры; цельный, оружие НЕ в руках (встроено/убрано).
Строго следовать единому style-sheet из опорной задачи (ракурс, палитра, контур,
pivot, тень). **Без оружия в руках.**

## Требования
1. Нарисовать «Робот» в едином стиле (по style-sheet опорной задачи), БЕЗ оружия
   в руках.
...
3. Положить ассет по шаблонному пути assets/sprites/characters/robot_sheet.png и
...
4. Если класс делил чужой спрайт — отвязать, дать собственный; старую текстуру в
   бэкап (не удалять).
5. Тест (smoke/animation): «Робот» строится с "walk"(5)/"attack"(5), проигрывается
   без ошибок, без оружия в руках; превью-гиф в build/qa/.
6. CHANGELOG; content_registry.

## Acceptance Criteria
- [ ] «Робот» перерисован в едином стиле, без оружия в руках.
...
## Документация
docs/design/content_registry.md (персонаж robot), current_game_state.
...
Лист скиллом + tools/build_character_sheet.py -> full_frame/robot/ кадры + robot_spriteframes.tres. player.gd грузит .tres. animation_smoke зелёный (exit 0).


## QA-Вердикт (2026-06-14)
Статус: PASSED — «Робот» перерисован + анимирован, играет в игре

Проверено (фактически):
- **Frame counts** (`robot_spriteframes.tres`): **walk 5 / attack 5** (+attack_primary 5,
...
- **Визуал** (`robot_sheet_normalized.png` / контакт): перерисован в едином D&D
...
Acceptance:
- [x] «Робот» перерисован в едином стиле, без оружия в руках.
