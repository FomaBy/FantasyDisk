# ART: Перерисовать «Снайпер» в едином стиле + анимации (5 move / 5 attack)
...
задача для класса **Снайпер** (`sniper`).

## Образ персонажа (арт-дирекция)
Снайпер: дальнобойный стрелок в длинном плаще с капюшоном, оптический монокль; БЕЗ винтовки/лука, руки свободны.
- Сейчас делит чужой спрайт (ranger.png) — нужен СОБСТВЕННЫЙ уникальный спрайт.
Строго следовать единому style-sheet из опорной задачи (ракурс, палитра, контур,
pivot, тень). **Без оружия в руках.**

## Требования
1. Нарисовать «Снайпер» в едином стиле (по style-sheet опорной задачи), БЕЗ оружия
   в руках.
...
3. Положить ассет по шаблонному пути assets/sprites/characters/sniper_sheet.png и
...
4. Если класс делил чужой спрайт — отвязать, дать собственный; старую текстуру в
   бэкап (не удалять).
5. Тест (smoke/animation): «Снайпер» строится с "walk"(5)/"attack"(5), проигрывается
   без ошибок, без оружия в руках; превью-гиф в build/qa/.
6. CHANGELOG; content_registry.

## Acceptance Criteria
- [ ] «Снайпер» перерисован в едином стиле, без оружия в руках.
...
Лист скиллом fantasydisk-asset-generator (1920x1152, idle/walk/attack, без оружия) -> tools/build_character_sheet.py: flood-fill фон + центровка + нарезка в full_frame/sniper/ + авторинг sniper_spriteframes.tres. player.gd грузит .tres по конвенции. animation_smoke зелёный (exit 0).


## QA-Вердикт (2026-06-14)
Статус: PASSED — «Снайпер» перерисован + анимирован, играет в игре

Проверено (фактически):
- **Frame counts** (`sniper_spriteframes.tres`): **walk 5 / attack 5** (+attack_primary 5,
...
- **Визуал** (`sniper_sheet_normalized.png` / контакт): перерисован в едином D&D
...
Acceptance:
- [x] «Снайпер» перерисован в едином стиле, без оружия в руках.
