...
Пользователь снял `user-hold` с задач в `К выполнению`: SCRUM-425 снова доступна
...
## Контекст
Пер-персонажная задача инициативы «Перерисовка персонажей v2» (0.1.6) — класс **Доктор** (`doctor`).
По style-sheet опорной задачи: ЯРКО и ЭПИЧНО, прозрачный фон, move+idle (без attack),
2× размер монстра.

## Образ (арт-дирекция)
Доктор: полевой медик, чистые бело-голубые тона, мягкое целебное свечение; светло. Без оружия в руках (оружие — отдельно). Прозрачный фон обязателен.
...
## Требования
1. Нарисовать «Доктор» v2 в ярком эпичном стиле класса (по опорной), на ПРОЗРАЧНОМ
...
6. Тест: animation+runtime smoke зелёные; на экране «Доктор» яркий, прозрачный,
   вдвое крупнее монстра, move/idle плавные. Превью-гиф в build/qa/.
7. CHANGELOG; content_registry.

## Acceptance Criteria
- [ ] «Доктор» перерисован v2: ярко/эпично по классу, прозрачный фон (нет белого/каймы/карманов).
...
## Документация
docs/design/content_registry.md (doctor), current_game_state.
