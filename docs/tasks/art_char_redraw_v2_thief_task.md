...
## Образ (арт-дирекция)
Вор: дерзкий плут в тёплой коже, золотые акценты, лукавая поза; ярко, задорно. Без оружия в руках (оружие — отдельно). Прозрачный фон обязателен.
...
## Требования
1. Нарисовать «Вор» v2 в ярком эпичном стиле класса (по опорной), на ПРОЗРАЧНОМ
...
6. Тест: animation+runtime smoke зелёные; на экране «Вор» яркий, прозрачный,
