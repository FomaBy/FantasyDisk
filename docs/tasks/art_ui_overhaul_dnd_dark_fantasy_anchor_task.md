...
Статус: done — superseded (PM 2026-06-14: широкий UI-overhaul закрыт как дубль, покрыто SCRUM-373/384/273)
Приоритет: high
...
Широкий UI-Overhaul кластер закрыт как покрытый более конкретными задачами: SCRUM-373/384 (единый мастер-фрейм) + SCRUM-273 (базовые кнопки). Дубль не нужен.

## QA-Вердикт (2026-07-03, административное закрытие)
Статус: PASSED
Проверено: тикет закрыт PM как superseded (дубль SCRUM-373/384/273); кода на приёмку нет, административное закрытие. Первое слово «Статус:» приведено к done, чтобы board_sync не ревертил «Готово» → «К выполнению» (дрейф пойман dry-run'ом 2026-07-03).
Баги: нет
