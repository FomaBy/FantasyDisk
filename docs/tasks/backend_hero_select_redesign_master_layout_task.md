...
## Требования — РАЗМЕР ИКОНОК КАРУСЕЛИ (запрос пользователя 2026-06-14)
3a. **Увеличить иконки персонажей в карусели** — сейчас персонажей плохо видно,
    иконки мелкие, а отступы между ними и сверху/снизу избыточные. Сделать иконки
    заметно крупнее (поднять _make_hero_thumbnail_button с 124×88 до большего;
    при необходимости увеличить высоту полосы HeroThumbnailStripFrame и портрет
    внутри), уменьшить разделители (HBoxContainer separation) и вертикальные
    отступы, чтобы герои читались чётко и занимали слот по максимуму без наезда на
    рамку. Сохранить ровный единый размер всех иконок и влезаемость всего ряда на
    1280×720 (адаптивно сжимать только если иначе не помещается).
...
- [ ] Иконки персонажей в карусели крупнее, отступы уменьшены, герои чётко видны; ряд влезает на 1280×720.
