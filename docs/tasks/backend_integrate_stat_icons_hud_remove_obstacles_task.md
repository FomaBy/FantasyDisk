...
Статус: done 2026-06-11. Результат: UIIconRegistry подключает базовые/производные/HUD icons; combat HUD минимальный HP/XP/money с иконками; Escape stats и level-up используют icon mapping; ямы и колонны отключены из активной игры, collision/pathing не генерируются, документация помечает препятствия как отключенные. Runtime smoke test покрывает icon registry, HUD и отсутствие активных obstacle blockers, проходит.
