...
Статус: PASSED (fix SCRUM-171 верифицирован)
Починено коммитом 84d6e2e (рабочее дерево сведено, экономика SCRUM-153 закоммичена).
Проверка независимая: `git worktree add /tmp/qa_wt HEAD` + `--import` + все 6 smoke —
ЗЕЛЁНЫЕ на ЧИСТОМ чекауте HEAD (5d8de89). `drop_class_rewards` присутствует в HEAD
progression_data. Разрыв «потребитель в HEAD, определение в рабочем дереве» закрыт.
