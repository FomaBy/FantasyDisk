...
Jira: SCRUM-147
Blocked: (1) feature freeze 0.1.3; (2) референсы пользователя ещё не выложены в
...
Не останавливаться для подтверждений. НО: backlog `0.1.4` — не брать в работу
до релиза 0.1.3 / отдельного решения PM.
