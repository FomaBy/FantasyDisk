...
## ТЕКУЩИЕ ДЕФЕКТЫ (скрин пользователя 2026-06-15) — ОБЯЗАТЕЛЬНО устранить в v3
Текущая (старая) реализация плохая, v3 ДОЛЖЕН это починить:
1. **Карусель — персонажи НЕЧИТАЕМЫ**: иконки мелкие/тёмные. Сделать крупнее,
   светлее/контрастнее, чёткие; герой узнаваем.
2. **Роза ветров КВАДРАТНАЯ**: рамка радара — квадрат. Сделать рамку розы ветров
   КРУГЛОЙ/компас-формы (под круглый радар), не квадратный бокс.
3. **Описание героя НЕ ПОМЕЩАЕТСЯ**: текст (характеристики/возвышение) вылезает за
   рамку. Контент строго в content-зоне; длинный текст умещается/скроллится;
   текст возвышения (длинный) не обрезается и не наезжает.
4. **Превью героя НЕ ПОМЕЩАЕТСЯ и ВЫЛАЗИТ**: портрет выходит за рамку-слот.
   Портрет масштабируется внутри content-зоны рамки, не торчит за орнамент.
Любой из 4 дефектов в готовом экране = QA FAILED.
...
- [ ] Карусель читаема (крупные чёткие иконки); роза ветров КРУГЛАЯ; описание и превью ПОМЕЩАЮТСЯ в рамках (не вылазят).
