...
## Source Asset
- PNG: `docs/design/references/backgrounds/field_meadow_reference.png`
- Model: `gpt-image-2`
- Size: `2560x1440`
- Quality: `high`
- Prompt:
...
```text
FantasyDisk combat arena background, top-down Dungeons and Dragons dark fantasy tabletop battlemap, realistic painterly ground texture, 2560x1440 wide arena floor, low-to-medium contrast central gameplay area, no tall objects, no walls blocking play, no characters, no monsters, no UI, no text, no watermark, flush-to-ground details only, subtle directional light from upper left, readable for animated heroes and monsters on top, edges naturally detailed without hard seams, not noisy, not over-saturated, dark fantasy dragon-era game art style. Biome: natural meadow grass, worn dirt patches, small low flowers, flattened grass paths, a few embedded flat stones, fresh but restrained fantasy color.
```
