...
## Source Asset
- PNG: `docs/design/references/scrum380_death_rows/homunculus_death_row_reference.png`
- Model: `gpt-image-2`
- Size: `1536x1024`
- Quality: `high`
- Prompt:
...
```text
FantasyDisk Dungeons and Dragons dark fantasy transparent sprite sheet reference: six full-frame death animation poses for small allied alchemical homunculus construct, brass joints, glass vial body, tiny wings or limbs, collapsing with green alchemy sparks and broken bottle glints. Same exact character across all frames, left-facing top-down/isometric game sprite, 6 equal poses in one horizontal row, readable silhouette, stable bottom-center pivot, no crop, no text, no labels, no UI frame, no baked background except solid bright green chroma for alpha cleanup. The animation must progress clearly from alive/defeated start to final dissipated remains, non-loop death row, painterly but game-readable.
```
