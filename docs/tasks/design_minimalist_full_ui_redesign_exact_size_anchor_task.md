...
## Прогресс 2026-06-20 (фаза 4 — self-QA)
АВТОМАТИЧЕСКИЙ SELF-QA ЗЕЛЁНЫЙ и исчерпывающий: `ui_no_overlap_matrix_test` гоняет 18 экранов ×
5 разрешений (1152/1280/1600/1920/2560), логирует rects ВСЕХ контролов и подтверждает: НЕТ
наложений, контент в content-зонах фреймов, текст в границах. Отчёт `build/qa/ui_no_overlap_matrix.md`.
runtime_smoke_ui + dark_fantasy_ui_theme также зелёные. Размеры ассетов 1:1 к слотам (без stretch).

ИТОГ SCRUM-478: редизайн всего UI на базе ярких кнопок ВЫПОЛНЕН — единый яркий янтарный минимал
(кнопки 75 + все фрейм-темы ~123 текстуры), точный размер без растяжения, бэкапы по темам.
