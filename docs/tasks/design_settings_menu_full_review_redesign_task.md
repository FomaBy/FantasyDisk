...
PM/Fable коммент 2026-07-02T09:41 (после моего клейма 09:16): забрал тикет на
себя по прямому карт-бланшу пользователя — полный редизайн настроек С НУЛЯ
(settings_v5), НЕ наследуя v4; поставил метку `hold`; QA-проверку звать после
ЕГО финальной v5-сдачи. Мой live-прогон доехал до готового v4 и запушил его
(коммит `da568ed8` в origin/dev) как инкремент-стопгэп — оба прежних QA-блокера
закрыты, гейты зелёные. НО в QA НЕ перевожу (уважаю hold + план v5): статус .md
оставлен `in_progress` (board_sync НЕ должен двигать в «Контроль качества» и
затирать метку `hold`). Ticket остаётся «В работе»/hold под v5 PM. Если PM решит
взять v4 как базу — код и evidence готовы ниже.
