...
## АРТ-ДИРЕКЦИЯ ФРЕЙМА (уточнение пользователя 2026-06-14) — ОБЯЗАТЕЛЬНО
- **ТОНКИЕ металлические рамки** — тонкая металлическая окантовка, НЕ массивная/
  не толстый орнамент. Углы — **такие же тонкие** (не громоздкие).
- **Красные самоцветы на углах** (небольшие, аккуратные) — акцент в углах.
- **Плашка дракона в СЕРЕДИНЕ краёв** металлической рамки (центральный
  драконий медальон/накладка) — опционально, «где уместно» (крупные панели/окна),
  как включаемый центральный оверлей.
- Рамка должна быть **тонкой**, чтобы ВСЕ элементы, которым положено быть внутри
  (кнопки/иконки/текст/портреты/области выбора), помещались во **внутренней
  content-зоне** рамки и не заходили на металл/окантовку (content margins тонкие,
  но ≥ толщины тонкой окантовки — контент строго внутри).
- Тонкость не должна ломать читаемость на 2560×1440 (не «теряться»).
...
## Acceptance Criteria
- [ ] Единый мастер-фрейм (9-slice: ТОНКАЯ металлическая окантовка + тонкие углы с красными самоцветами + опц. драконья плашка в середине краёв + опц. фон + опц. hover) создан скиллом; контент строго во внутренней content-зоне.
