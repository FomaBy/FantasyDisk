...
- Проверено: статическая QA-инспекция docs/code/tests на `origin/dev`
  ...
  `skill_tree_per_hero_test.gd` успели пройти перед остановкой длинного
  импорта, остальные gates не засчитывались после найденного acceptance blocker.
- Блокер: ранний hook Атласа гильдии не выполнен. PM-комментарий 2026-07-02
  16:32 требовал вернуть в T1 минимум один узел Атласа за 1 пыль; документация
  описывает первую победу как 1 stardust и открытие первого QoL-узла. Код
  действительно выдаёт 1 stardust за первую победу класса, но оба первых
  соседа `atlas_hub` стоят 2, поэтому после первой победы нельзя купить ни один
  узел Атласа.
- Тестовый риск: `tests/meta_skill_tree_smoke_test.gd` закрепляет неверное
