...
func _run() -> void:
	...
	var f := FileAccess.open("res://evidence/p3-object-budget-rework/frame-residency-causal/census-%s.json" % String(Time.get_datetime_string_from_system(false)).replace(":", ""), FileAccess.WRITE)
