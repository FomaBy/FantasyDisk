class_name BiologistUltimatePresentationScene
extends Node2D

## Class-local visual runner for a frozen Biologist ultimate timeline
## (Ultimate Direction v2, FAN-2944 §1/§3.1). This runner deliberately owns
## only its presentation nodes and delegates timing, pause, headless behavior,
## and handle cleanup to the frozen contract class.
...
func _backdrop_color(alpha: float) -> Color:
	if str(_presence.get("backdrop", "")) == "flash":
		return Color(0.34, 0.98, 0.44, alpha)
	return Color(0.02, 0.05, 0.03, alpha)
...
func _palette_color(alpha: float) -> Color:
	...
	if weapon_id == "biologist_sample_injector":
		return Color(0.55, 1.0, 0.86, alpha)
	if weapon_id == "biologist_symbiote_seed":
		return Color(1.0, 0.48, 0.92, alpha)
	return Color(0.68, 1.0, 0.52, alpha)
