...
## package spawns nothing, so this scene is the only live effect channel its
## cast owns — the per-victim read has to ride the executor beats.
...
@export var impact_frames: SpriteFrames = null
...
func present(_event_id: String, payload: Dictionary) -> void:
	...
	# multi-pulse cast reads as one cascade rather than restarting per beat.
