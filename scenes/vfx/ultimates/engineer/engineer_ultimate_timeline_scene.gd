...
func _build_v2_overlay() -> void:
	...
	backdrop_texture.gradient = _solid_gradient(Color.WHITE)
	...
	for index in count / 2:
		var chord_texture := GradientTexture2D.new()
		chord_texture.gradient = _solid_gradient(Color.WHITE)
		chord_texture.width = 64
		chord_texture.height = 4
		var chord := Sprite2D.new()
		chord.name = "CrossfireChord%d" % index
		var seat := _v2_seat(index, count, radius)
		chord.texture = chord_texture
		chord.scale = Vector2(seat.length() * V2_CHORD_REACH * 2.0 / chord_texture.get_width(), V2_CHORD_WIDTH / chord_texture.get_height())
		chord.rotation = seat.angle()
...
func _solid_gradient(color: Color) -> Gradient:
