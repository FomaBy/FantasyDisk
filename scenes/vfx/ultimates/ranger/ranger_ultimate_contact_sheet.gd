...
func _draw_formation(sheet: Image, cell: Vector2i, element: Image, pivot: Dictionary, weapon_id: String, phase_name: String, progress: float) -> void:
	...
	for point in Pack.formation_points(weapon_id, phase_name, progress):
		...
		if alpha <= 0.01 or scale <= 0.01:
			continue
		_blit_element(sheet, element, center + (point.get("position", Vector2.ZERO) as Vector2) * VIEW_SCALE, scale, alpha, float(point.get("rotation", 0.0)), pivot, bounds)


func _blit_element(sheet: Image, element: Image, center: Vector2, scale: float, alpha: float, rotation: float, pivot: Dictionary, bounds: Rect2i) -> void:
