...
const CHARACTER_PLAYSTYLE := {
	...
	"thief": "Уловки и позиция: монеты цепляются по врагам и крадут золото, плащ заводит за спину, дым дает окно уклонения.",
	"elementalist": "Смена стихий: орбиты защищают личное пространство, призма режет крестом, метеор взрывается осколками.",
	"sniper": "Дальний контроль: фиксирует прицел, ставит kill-zone и раскалывает патроны по соседним целям.",
	"priest": "Священный sustain: освящает цели, держит защитные ward-круги и связывает врагов молитвенной цепью.",
