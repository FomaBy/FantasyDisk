...
func spawn_tick(start_global: Vector2, scale: Vector2, modulate: Color, group: String) -> void:
	...
	# The base alpha is captured at the FIRST animation step, not at spawn —
	# the same semantics the previous node-bound tween had. External adapters
	# (e.g. the engineer photosensitivity driver bounding hit-tick alpha before
	# the first frame) must keep their value as the fade start.
...
func _step_ticks(delta: float) -> void:
	...
	while index < _active_ticks.size():
		...
		if elapsed >= TICK_LIFETIME:
			...
			_active_ticks.remove_at(index)
			continue
		if not bool(record["captured"]):
			record["captured"] = true
			record["base_alpha"] = tick.modulate.a
