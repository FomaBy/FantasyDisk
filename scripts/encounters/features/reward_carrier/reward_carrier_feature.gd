...
func on_trigger(context, beat_def: Dictionary) -> bool:
	if _settings.is_empty() and not _load_settings(beat_def):
		return false
	if not context.is_normal_battle() or context.game == null:
		return false
...
func _on_carrier_died(_enemy: Node2D) -> void:
	if _confirmed_defeat:
		return
...
func _transfer_confirmed_reward() -> void:
	...
	if _player.has_method("gain_money"):
