...
const RANGER_WEAPONS := {
	...
	"storm_longbow": {
		...
		"description": "Три грозовых луча веером: дальний контроль линии и пробивание нескольких целей.",
	...
	"hunter_trap": {
		...
		"description": "Ставит ловушку перед Рейнджером: первый враг в зоне запускает взрыв и отбрасывание.",
...
const DOCTOR_WEAPONS := {
	...
	"plague_syringe": {
		...
		"description": "Быстрая инъекция-проклятие: одиночная цель получает яд, Доктор слегка лечится.",
		"scene_path": "res://scenes/PlagueSyringe.tscn",
		"attack_mode": "homing_curse", "damage_parameter": "magic_damage",
		"damage_multiplier": 0.64, "fire_interval": 0.78,
		"attack_range": 590.0, "aoe_radius": 80.0, "dot_ticks": 6,
		"projectile_speed": 700.0, "heal_percent_on_attack": 0.012,
	...
	"bone_saw": {
		...
		"projectile_count": 2, "dot_ticks": 3, "heal_percent_on_attack": 0.010,
...
const CHEMIST_WEAPONS := {
	...
	"acid_flask": {
		...
		"leaves_pool": true, "pool_duration": 4.2, "pool_tick_interval": 0.48,
...
const KNIGHT_WEAPONS := {
	...
	"tower_shield": {
		...
		"passive_mods": {"defense_flat": 0.08, "max_health_multiplier": 1.08},
	},
	"holy_flail": {
		...
		"passive_mods": {"knockback_multiplier": 1.20},
	},
}
