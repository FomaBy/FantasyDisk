...
const CLASS_BUDGET_PROFILES := {
	...
	"soldier": {"profile": "balanced", "survival": "steady", "damage_budget": 0.76, "solo_target": 1.00, "aoe_target": 1.00},  # FAN-1031 3d-final (v7): total 1.19 (solo 1.42 bayonet-driven, все оси ≥1). db 0.82→0.76 — малый шаг uniform per-hit cut всех трёх (unclamped). def 1.22 = steady (in-profile, не трогаем). Калибровать по v8.
	...
	"robot": {"profile": "balanced", "survival": "tank", "damage_budget": 0.80, "solo_target": 1.07, "aoe_target": 1.05},  # FAN-1031 3d-final (v7): total 1.33, но это ТАНК-driven (def 2.12; damage-оси solo 0.84/aoe 1.05/crowd 1.30). db 0.88→0.80 давит crowd/aoe per-hit (magnetic_anchor near-clamp; reactor clamped ceil — db его почти не двигает). ⚠️ ОСТАТОК total >1.15 — identity-price ТАНКА (ось defense=EHP): survival "tank" НЕ режем (координаторское решение v7). Калибровать по v8 по damage-осям, не по total.
	"engineer": {"profile": "balanced", "survival": "steady", "damage_budget": 0.96, "solo_target": 0.98, "aoe_target": 1.12},
	"dark_mage": {"profile": "aoe", "survival": "fragile", "damage_budget": 0.52, "solo_target": 0.66, "aoe_target": 1.30},  # FAN-1031 3d-final (v7): total 1.16 (aoe 1.69/crowd 1.33). db 0.58→0.52 — один малый шаг per-hit dark_book/dark_wand (unclamped, direct); cursed_skull curse_only DoT db НЕ трогает. aoe_target 1.30 профиля не трогаем. Калибровать по v8. Ниже — история 3c-c: overbudget v4 total 1.82 (solo 1.33 при цели 0.84 — сильнее всего над профилем) → per-hit down-tune; cursed_skull DoT режется отдельно (curse_tick_multiplier). 3d (v6): total 1.41 (aoe 2.19/crowd 1.72, dark_book mirror 3.76×/3.16×, dark_wand chain 2.71×). db 0.72→0.58 давит per-hit dark_book/dark_wand (оба unclamped, direct-канал); cursed_skull curse_only → его DoT db НЕ трогает (уже низок 0.34× crowd). Калибровать по v7.
	...
	"chemist": {"profile": "aoe", "survival": "fragile", "damage_budget": 0.85, "solo_target": 0.84, "aoe_target": 1.30},  # FAN-1031 3d-final (v7): total 1.36 (aoe 2.16 blast_powder 5.29×-driven, crowd 1.38). db 0.95→0.85 — малый шаг per-hit blast/acid (unclamped). Остаточный aoe-лид — blast per-hit (уже aoe-width капнут aoe_max 3); aoe_target 1.30 (профиль crowd-специалиста) НЕ выгрызаем до бессмысленности — принимаем как profile-identity с примечанием. Калибровать по v8.
