...
const CLASS_BUDGET_PROFILES := {
	...
	"soldier": {"profile": "balanced", "survival": "steady", "damage_budget": 0.82, "solo_target": 1.00, "aoe_target": 1.00},  # FAN-1031 3d (v6): total 1.39 (solo 1.63 bayonet-driven, все оси >1). db 1.00→0.82 uniform per-hit cut всех трёх (все unclamped); bayonet solo-спайк дополнительно сглажен ниже. def 1.22 = steady (in-profile).
	"thief": {"profile": "balanced", "survival": "fragile", "damage_budget": 1.08, "solo_target": 1.00, "aoe_target": 1.00},
	"elementalist": {"profile": "aoe", "survival": "fragile", "damage_budget": 0.70, "solo_target": 0.92, "aoe_target": 1.10},  # FAN-1031 3c-c: down-tune per-hit по всему киту (v4 total 3.41). 3c-final: crowd orb_ring теперь режется ЖЁСТКИМ orbit_max_targets (coverage); damage_budget 0.88→0.82 добивает solo/aoe-остаток. 3d (v6): total всё ещё 1.49 (solo 1.03/aoe 1.74/crowd 2.58, prism_focus full-map 3.91×). db 0.82→0.70 давит per-hit prism/meteor/orb (все unclamped); orb crowd дополнительно orbit_max 6→4. Калибровать по v7 (осторожно: двойной нерф crowd — width-кап + db).
	"sniper": {"profile": "solo", "survival": "steady", "damage_budget": 1.15, "solo_target": 1.15, "aoe_target": 0.80},  # FAN-1031 3d (v6): total 0.86 FAIL (damage-оси solo 0.66/aoe 0.80/crowd 0.52 низки для solo-класса; def 1.45 держит total). db 1.00→1.15 поднимает raw всех трёх (unclamped) + чинит random-A1 0.82. Калибровать по v7.
	"priest": {"profile": "balanced", "survival": "steady", "damage_budget": 0.86, "solo_target": 1.03, "aoe_target": 1.05},  # FAN-1031 3d (v6): total 1.35 (crowd 2.29-driven, reliquary crowd 3.24×). Основной рычаг — falloff crowd-width cap на reliquary (ниже, режет crowd БЕЗ solo); db 0.92→0.86 — мягкий per-hit нудж (reliquary unclamped; censer/chime clamped ceil, db их не двигает). solo 0.67 уже низок — db бережно. Калибровать по v7.
	...
	"dark_mage": {"profile": "aoe", "survival": "fragile", "damage_budget": 0.58, "solo_target": 0.66, "aoe_target": 1.30},  # FAN-1031 3c-c: overbudget v4 total 1.82 (solo 1.33 при цели 0.84 — сильнее всего над профилем) → per-hit down-tune; cursed_skull DoT режется отдельно (curse_tick_multiplier). 3d (v6): total 1.41 (aoe 2.19/crowd 1.72, dark_book mirror 3.76×/3.16×, dark_wand chain 2.71×). db 0.72→0.58 давит per-hit dark_book/dark_wand (оба unclamped, direct-канал); cursed_skull curse_only → его DoT db НЕ трогает (уже низок 0.34× crowd). Калибровать по v7.
	...
	"ranger": {"profile": "solo", "survival": "fragile", "damage_budget": 1.26, "solo_target": 1.30, "aoe_target": 0.70},  # FAN-1031 3d (v6): total 0.80 FAIL+ (все оси низки: solo 0.86/aoe 0.92/crowd 0.63, def 0.80). Тот же профиль, что assassin (0.93) — оружия просто слабее. db 1.15→1.26 поднимает raw всех трёх (unclamped). crowd 0.63 частично by-profile (solo-класс, aoe_target 0.70). Калибровать по v7.
	"doctor": {"profile": "balanced", "survival": "tank", "damage_budget": 0.85, "solo_target": 1.00, "aoe_target": 1.00},
	"chemist": {"profile": "aoe", "survival": "fragile", "damage_budget": 0.95, "solo_target": 0.84, "aoe_target": 1.30},  # FAN-1031 3d (v6): total 1.76 FAIL+ (aoe 3.28/crowd 2.11, blast_powder 8.78× 5t). db 1.15→0.95 давит per-hit blast/acid (unclamped); blast aoe-width 6→3 и acid pool-width 6→4 режут aoe/crowd БЕЗ solo (ниже). aoe_target 1.30 (профиль crowd-специалиста) НЕ трогаем. Калибровать по v7 (цель crowd_norm ≤1.56).
