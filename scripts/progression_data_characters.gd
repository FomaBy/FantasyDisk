...
const CHARACTER_CONFIGS := {
	...
	"berserk": {
		...
		"id": "berserk",
		"title": "Берсерк",
		"description": "Дерётся только вплотную и только напролом: врубается в толпу и косит всё вокруг, держась за счёт огромного запаса крови. Его ярость обращает боль в силу — чем ближе к смерти, тем сильнее удары (до +40%), поэтому опаснее всего он на последних каплях здоровья.",
	...
	"soldier": {
		"id": "soldier",
		"title": "Солдат",
		"description": "Надёжный стрелок средней дистанции, вся соль которого — в двойном спуске: почти каждое действие может сработать дважды. Ровно давит огнём и взрывчаткой и толпу, и одиночек, но в затяжной дуэли с боссом выдыхается.",
	...
	"thief": {
		...
		"id": "thief",
		"title": "Вор",
		"description": "Вёрткий добытчик, который превращает бой в наживу: добыча сама летит к нему, а удары приносят золото прямо на месте. Контролирует врагов ядом и растворяется в дыму, но здоровья мало — одна пропущенная серия в упор, и его нет.",
	...
	"elementalist": {
		...
		"id": "elementalist",
		"title": "Элементалист",
		"description": "Чистый маг зон: его магические бонусы на 30% сильнее, чем у других, а заклинания накрывают огромные площади. Пока враги стоят в его стихиях, он выжигает целые волны, но сам хрупок и почти беспомощен, если его продавят вплотную.",
	...
	"sniper": {
		...
		"description": "Дальнобойный ликвидатор, который бьёт тем больнее, чем дальше цель — на предельной дистанции урон вырастает до +60%. Снимает опасные цели с другого конца арены и живёт за счёт дистанции; подпустишь толпу вплотную — и его главная сила пропадает.",
	...
	"priest": {
		"id": "priest",
		"title": "Священник",
		"description": "Боевой пастырь средней дистанции, чей стиль задаёшь ты сам: перед каждым боем он выбирает одну молитву — кары, исцеления или защиты. Дальше он расчищает толпу святым огнём, но выбор всего один на схватку, и цена ему высока.",
	...
	"biologist": {
		"id": "biologist",
		"title": "Биолог",
		"description": "Сражается заразой: метит врагов спорами и ядом, а по заражённым бьёт на 20% больнее. Разгоняется медленно и сам непрочен, зато со временем превращает целую толпу в гниющее поле, которое доедает себя само.",
	...
	"robot": {
		...
		"description": "Ходячая крепость: бронекорпус гасит пятую часть любого урона, что бы в него ни прилетело. Медлителен, зато стягивает и перемалывает толпу прямо на месте и почти не умирает — берёт не темпом, а несокрушимостью.",
	...
	"engineer": {
		...
		"description": "Побеждает не своей рукой, а подготовленным полем: расставляет турели, дронов и вечные мины, и чем больше устройств работает, тем сильнее бьёт вся сеть. В начале боя, пока ничего не развёрнуто, он уязвим — его сила в накоплении.",
	...
	"dark_mage": {
		...
		"description": "Стеклянная пушка: выжигает целые области издалека цепными снарядами и проклятиями, а убитые им враги взрываются сами, добивая соседей. Урон огромный, но защиты почти нет — он должен убить раньше, чем до него доберутся.",
	...
	"guitarist": {
		...
		"title": "Гитарист",
		"description": "Маг-кастер, который раскачивается по ходу боя: пока он не пропускает удары, его магия бьёт всё сильнее. Кайтит толпу издалека и наказывает её по площади, но один пропущенный удар сбрасывает разогрев, а против боссов ему тяжело.",
	...
	"assassin": {
		...
		"description": "Единственный, кто способен разогнать шанс крита до 100% — и тогда каждый удар критический и растёт от крит-урона. Режет цели сериями рывков и выживает исключительно уворотом и позицией: здоровья мало, и любой промах дорого стоит.",
	...
	"ranger": {
		...
		"description": "Держит дистанцию силой: каждое попадание отталкивает врага прочь, не давая толпе сомкнуться. Расстреливает подходящих и ставит капканы на подходах, но теряет хватку вблизи и в движении — его стихия это контроль расстояния.",
	...
	"doctor": {
		...
		"description": "Врач без жалости, который лечится только собственным уроном: каждая нанесённая рана возвращает ему здоровье, а чужие лечилки на него не действуют. Бурста мало, зато в затяжном бою он почти не умирает, пока есть кого резать.",
	...
	"chemist": {
		...
		"description": "Алхимик выжженной земли: кислотные лужи, взрывы и пара гомункулов превращают арену в отраву, а его периодический урон усилен. Сам он хрупок и убивает не сразу — его дело поджечь поле и дать химии доделать остальное.",
	...
	"knight": {
		...
		"description": "Латная стена возмездия: любой, кто его ударит, отлетает прочь, а сам он держит строй бронёй и здоровьем. Медлителен и не продавливается, контролируя всё вокруг отбросом, — только боссов оттолкнуть нельзя, с ними придётся стоять насмерть.",
	...
	"druid": {
		...
		"description": "Командует дикой стаей: его аура усиливает и его самого, и призванных зверей, что рвут выбранную добычу, пока тернии и тотемы стерегут землю. Силён, когда стая рядом и в ауре, но в одиночку, растеряв призывов, уязвим.",
