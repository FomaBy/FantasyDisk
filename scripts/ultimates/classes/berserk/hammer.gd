...
func _lanes(step: int) -> void:
	...
	for axis in lane_axes(step):
		for raw_target in _activation.targets_in_corridor(
			center, axis, length, half_width, crowd_cap
		):
			var target := raw_target as Node
			if target == null or not is_instance_valid(target) \
					or seen.has(target.get_instance_id()):
				continue
			# One event id per beat: the four lanes overlap at the hero and a
			# single beat is one impact, not one impact per lane it touches.
