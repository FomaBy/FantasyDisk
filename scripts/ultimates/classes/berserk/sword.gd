...
func cross_slash() -> void:
	...
	for arm in cross_axes(_activation.aim_direction(length)):
		for raw_target in _activation.targets_in_corridor(
			center, arm, length, half_width, crowd_cap
		):
			var target := raw_target as Node
			if target == null or not is_instance_valid(target):
				continue
			# One shared event id per target: the arms overlap at the hero, and
			# the collapse is a single synchronized slash, not four hits.
			_deal(
				target,
