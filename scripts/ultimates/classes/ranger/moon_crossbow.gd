...
## The heir is chosen after the split bolts of the same wave already landed, so
## a neighbour that wave killed is no heir: the nearest-first rail is walked
## until a body still holding health takes the mark. A wave that leaves no
...
func _transfer_mark(prey: Node2D, neighbours: Array, wave: int) -> void:
	for raw_target in neighbours:
		...
		return
	_activation.consume_target_value(prey, MARK_KEY, "moon_hunt:release:%d" % wave)
