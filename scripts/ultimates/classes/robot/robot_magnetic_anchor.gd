...
static func execute(activation) -> float:
	var center: Vector2 = activation.aim_point(activation.param_float("aim_range", 520.0))
	...
	tween.tween_interval(activation.param_float("recovery", 3.70))
	return activation.param_float("release_delay", 0.45) \
		+ activation.param_float("implosion_delay", 0.60) \
		+ activation.param_float("recovery", 3.70)


static func release(activation, center: Vector2) -> void:
	if activation != null and not activation.is_finished():
...
static func implode(activation, center: Vector2) -> void:
	...
	for raw_target in activation.targets(
		...
		activation.apply_control(
			...
			{"duration": 0.45, "speed_multiplier": 0.65}
		)
