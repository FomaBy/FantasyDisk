...
static func execute(activation) -> float:
	...
	tween.tween_interval(activation.param_float("recovery", 4.50))
	return interval * float(activation.param_int("wave_count", 4)) + activation.param_float("recovery", 4.50)
...
static func vent_wave(activation, wave: int) -> void:
	...
	for index in count:
		var direction := Vector2.RIGHT.rotated(TAU * float(index) / float(count) + float(wave) * 0.22)
