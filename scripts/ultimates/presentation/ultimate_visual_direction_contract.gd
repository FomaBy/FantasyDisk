...
## to the per-victim impact, so a telegraph may stay only as a minority of the
## drawn nodes and may not blink.
const MAX_TELEGRAPH_DRAWN_RATIO := 0.5
const TELEGRAPH_BLINK_ALTERNATIONS := 3
const TELEGRAPH_GROUP := "ultimate_area_telegraph"
...
## Area-telegraph demotion (FAN-3008). The reach of an ultimate is read off the
## per-victim impact, so an area rectangle or overlay may only stay as flavour:
## a minority of the drawn nodes, and never blinking. Like the material gate,
## this walks a tree the caller owns and instantiates nothing.
static func scene_telegraph_violations(scene: Node, key: String) -> Array[String]:
	var errors: Array[String] = []
	if scene == null or not is_instance_valid(scene):
		errors.append("telegraph.scene_missing: %s" % key)
		return errors
	var telegraphs := area_telegraph_nodes(scene)
	if telegraphs.is_empty():
		return errors
	var drawn := _telegraph_drawn_nodes(scene)
	if telegraphs.size() >= drawn.size():
		errors.append(
			"telegraph.only_read: %s draws %d area telegraphs and no other effect"
			% [key, telegraphs.size()]
		)
	elif float(telegraphs.size()) > float(drawn.size()) * MAX_TELEGRAPH_DRAWN_RATIO:
		errors.append(
			"telegraph.dominant: %s reads %d of its %d drawn nodes as area telegraphs"
			% [key, telegraphs.size(), drawn.size()]
		)
	for blink in _telegraph_blinks(scene, telegraphs):
		errors.append("telegraph.blink: %s %s" % [key, blink])
	return errors


## Nodes that mark the affected area: an explicitly declared telegraph anywhere,
## plus the world-space rectangle shapes an area frame is authored from. A
## screen-space rectangle is the backdrop layer the material gate already owns,
## not an area marker, so structural detection stops at a CanvasLayer.
static func area_telegraph_nodes(root: Node) -> Array[Node]:
	var found: Array[Node] = []
	_collect_telegraphs(root, false, found)
	return found


static func _collect_telegraphs(node: Node, screen_space: bool, found: Array[Node]) -> void:
	var in_screen_space := screen_space or node is CanvasLayer
	if _is_declared_telegraph(node) or (not in_screen_space and _is_area_rectangle(node)):
		found.append(node)
	for child in node.get_children():
		_collect_telegraphs(child, in_screen_space, found)


static func _is_declared_telegraph(node: Node) -> bool:
	return node.is_in_group(TELEGRAPH_GROUP) or node.get_meta("area_telegraph", false) == true


static func _is_area_rectangle(node: Node) -> bool:
	if node is ReferenceRect or node is ColorRect:
		return true
	if node is Polygon2D:
		return _is_axis_aligned_rectangle((node as Polygon2D).polygon)
	if node is Line2D:
		return _is_axis_aligned_rectangle((node as Line2D).points)
	return false


## Four distinct corners over exactly two x and two y values: the axis-aligned
## frame or fill an area telegraph is drawn as. A closing fifth point repeats
## the first one.
static func _is_axis_aligned_rectangle(points: PackedVector2Array) -> bool:
	if points.size() < 4 or points.size() > 5:
		return false
	if points.size() == 5 and not points[0].is_equal_approx(points[4]):
		return false
	var xs := {}
	var ys := {}
	var corners := {}
	for index in 4:
		xs[snappedf(points[index].x, 0.01)] = true
		ys[snappedf(points[index].y, 0.01)] = true
		corners[Vector2(snappedf(points[index].x, 0.01), snappedf(points[index].y, 0.01))] = true
	return xs.size() == 2 and ys.size() == 2 and corners.size() == 4


## The drawn set this gate compares against: the runtime's performance-budget
## node types plus the two rectangle Controls an area frame is authored from,
## so a telegraph is always counted in both halves of the ratio.
static func _telegraph_drawn_nodes(root: Node) -> Array[Node]:
	var found: Array[Node] = []
	if root is Sprite2D or root is AnimatedSprite2D or root is Polygon2D or root is Line2D \
			or root is GPUParticles2D or root is CPUParticles2D \
			or root is ColorRect or root is ReferenceRect:
		found.append(root)
	for child in root.get_children():
		found.append_array(_telegraph_drawn_nodes(child))
	return found


## A telegraph that alternates visibility three or more times in one authored
## animation is a blinking frame. One fade in and out is two alternations, so
## short flavour still passes.
static func _telegraph_blinks(root: Node, telegraphs: Array[Node]) -> Array[String]:
	var names := {}
	for node in telegraphs:
	...
	for player in _animation_players(root):
		for animation_name in player.get_animation_list():
			...
			for track in animation.get_track_count():
				...
				if path.get_name_count() == 0 or not names.has(str(path.get_name(path.get_name_count() - 1))):
					continue
				if not _is_visibility_property(str(path.get_concatenated_subnames())):
					continue
				var alternations := _visibility_alternations(animation, track)
				if alternations >= TELEGRAPH_BLINK_ALTERNATIONS:
					findings.append(
						"%s blinks %d times in %s" % [str(path), alternations, str(animation_name)]
					)
	return findings


static func _animation_players(node: Node) -> Array[AnimationPlayer]:
...
static func _is_visibility_property(subnames: String) -> bool:
	for property in TELEGRAPH_BLINK_PROPERTIES:
		if subnames == property or subnames.begins_with("%s:" % property):
			return true
	return false


static func _visibility_alternations(animation: Animation, track: int) -> int:
	var alternations := 0
	var previous := -1
	for key_index in animation.track_get_key_count(track):
		var shown := 1 if _reads_as_shown(animation.track_get_key_value(track, key_index)) else 0
		if previous >= 0 and shown != previous:
			alternations += 1
		previous = shown
	return alternations


static func _reads_as_shown(value: Variant) -> bool:
	if value is bool:
		return value
	if value is Color:
		return (value as Color).a >= 0.5
	return _is_number(value) and float(value) >= 0.5
