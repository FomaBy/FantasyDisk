...
static func class_weapon_record(class_id: String, weapon_id: String) -> Dictionary:
	...
	var record := {
		...
		"crowd_cap": crowd_cap,
		# Raw values: null means "not declared" so the runtime can fall back to
		# scene metadata, and an invalid declaration stays visible to the
		# material validator instead of being coerced into a number.
