...
static func manifest_for_profile(profile: Dictionary) -> Dictionary:
	...
	var manifest := {
		...
		"runtime": {
			...
			"max_unique_materials": int(local.get("max_unique_materials", 0)),
			"max_fullscreen_materials": int(local.get("max_fullscreen_materials", 0)),
		},
	}
...
static func class_weapon_record(class_id: String, weapon_id: String) -> Dictionary:
	...
	var record := {
		...
		"crowd_cap": crowd_cap,
		"max_unique_materials": int((performance as Dictionary).get("max_unique_materials", 0)),
		"max_fullscreen_materials": int((performance as Dictionary).get("max_fullscreen_materials", 0)),
	}
	for block in ["presence", "identity", "quality"]:
