---
name: perenos-chata
description: "Перенос рабочего чата между контекстами с правдивым Markdown handoff и безопасным архивированием. Use when the user asks to 'перенеси этот чат', 'суммаризируй чат и создай новый', 'continue this chat in a new thread', or needs a Codex/Claude task transferred while preserving FantasyDisk or other repo rules, Jira ownership, Git/push state, verification, cleanup, and the exact next action."
---
...
2. Перечитай актуальные правила до действий.
   ...
   - Для FantasyDisk и других Jira-first проектов проверь live issue: sprint,
     status, comments, assignee, lane, owner/thread и locked paths. Jira имеет
     приоритет над локальными mirrors.
   - Выполняй разрешённые prompt/`AGENTS.md` in-scope действия автономно: не
     ...
     Jira sync, тестов, commit/push или явно запрошенного переноса thread.
...
4. Приведи активную задачу к правдивому состоянию.
   - Не оставляй issue в stale `В работе` за архивируемым worker/thread.
   - При продолжении тем же owner запиши heartbeat/handoff с новым thread id и
     ...
     `К выполнению`, QA/review или blocked state с причиной.
...
7. Заверши sync и cleanup, затем архивируй источник.
   - Выполни обязательные Jira/Git/docs/tests/disk операции или запиши
...
Продолжай с этого состояния. Сначала перепроверь live Jira, Git и применимые
