...
func _fixture_fragment() -> Dictionary:
	...
	for class_value in Pack.class_ids():
		...
		for scenario_value in Pack.PLAYABLE_SCENARIOS:
			...
			measurements["%s|%s" % [class_id, scenario_id]] = {
				"class_id": class_id,
				...
				"activation_count": formula.get("expected_initial_activations", 0),
				"activation_timing_seconds": [0.0] if int(formula.get("expected_initial_activations", 0)) == 1 else [],
				"ultimate_source": {"damage": 0.0, "hits": 0, "by_mechanic": {}},
				"sustain_source": {"damage": 1.0, "hits": 1},
				"formula": formula,
			}
	return {"fragment_schema": Pack.FRAGMENT_SCHEMA, "pack_id": Pack.PACK_ID, "pack_contract": Pack.PACK_CONTRACT, "issue": "FAN-1515", "contract": Pack.contract(), "scenario_manifest": Pack.scenario_manifest(), "per_weapon_sustain": Pack.per_weapon_sustain_rows(), "measurements": measurements}
