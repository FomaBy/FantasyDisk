...
# FAN-2239: a projection variant owns only the 408 weapon rows (~4 MB measured). A
# whole-dataset `duplicate(true)` also copies the 309-sample telemetry payload and
# costs ~670 MB, which is what exhausted the hosted runner's memory once seven
# variants were alive at once. The bound is deliberately far above the real cost so
# it flags a reintroduced deep duplicate, not ordinary allocator noise.
...
func _initialize() -> void:
	...
	if dataset is Dictionary:
		# FAN-2239: every variant below shares this payload by reference, so its digest
		# is taken before the first variant exists and re-checked after the last one.
...
func _verify_lineage_contract(historical: Dictionary, raw_text: String) -> void:
	...
	# FAN-2239: the one variant that edits telemetry. It isolates the sample list and the
	# single renamed sample instead of deep-copying all 309 payloads.
...
# FAN-2239: every candidate and negative mutation in this suite edits `weapon_rows`
# and reads the rest of the dataset, so a variant deep-copies only the projection
# rows and keeps sharing the immutable telemetry payload. The gate helpers this file
# calls never write to the dataset they verify, and the end-to-end digest check in
# `_initialize` fails closed if that ever stops being true.
...
# FAN-2239: fail closed if a later edit reintroduces the unbounded whole-dataset deep
# copy this suite was rebuilt to avoid.
