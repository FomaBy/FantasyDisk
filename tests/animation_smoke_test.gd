...
func _test_full_frame_animation_registry() -> void:
	...
	for elite_id in elite_full_frame_scenes.keys():
		...
		if elite_frames == null:
			...
			continue
		# FAN-2621/FAN-2901: branch on the registry's own explicit_eight_directions
		# flag, not a hardcoded ID list, so the next converted top-level elite
		# falls into the directional contract automatically instead of
		...
		if elite_is_directional:
			...
			for skill_state in elite_info["skill_states"]:
				var directional_skill_base := str(skill_state)
				for dir_suffix in FullFrameAnimationRegistry.DIRECTION_SUFFIXES:
					var directional_skill_row := "%s_%s" % [directional_skill_base, dir_suffix]
					if not elite_frames.has_animation(directional_skill_row):
						_fail("Expected %s elite SpriteFrames to expose %s." % [elite_id, directional_skill_row])
					elif elite_frames.get_animation_loop(directional_skill_row):
						_fail("Expected %s elite %s to be one-shot." % [elite_id, directional_skill_row])
		else:
