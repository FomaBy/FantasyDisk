...
# Потолки коридора лидеров (после SCRUM-503 soft-cap забеговых множителей).
# Замеренный berserk/hammer lvl20_ideal сидит ~26k/1.1k (20t/1t); до фикса было
# 184356/7636, после одного лишь частичного фикса экспонент — 60451/2520. Потолки
# с запасом над коридором (≤2.5x медианы: 20t≈28k, solo≈2.4k), но заметно ниже
# даже частично-фикснутого runaway — поэтому ловят и полный откат cap'а, и возврат
# раздутых upgrade_*_exponent молота, не флача на шуме замера (~±10%).
const MAX_IDEAL_20T := 40000.0   # коридор ~26k; 60451 (откат) и выше → FAIL
