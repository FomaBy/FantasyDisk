...
const CHARACTER_ID := "biologist"
const FRAMES_PATH := "res://assets/sprites/characters/biologist_spriteframes.tres"
const RUNTIME_DIR := "res://assets/sprites/characters/full_frame/biologist_pixellab"
const DIRECTIONS := [
	"south",
	"south_east",
	"east",
...
func _initialize() -> void:
	...
	if frames.has_animation("attack") or frames.has_animation("attack_primary"):
		_fail("Biologist PixelLab body pack must not expose attack animations by SCRUM-421 scope.")
		return
	if frames.get_frame_count("idle") != 1 or frames.get_frame_count("walk") != 6 or frames.get_frame_count("move") != 6:
		_fail("Expected Biologist fallback idle/walk/move frame counts to be 1/6/6.")
		return
	...
	for direction in DIRECTIONS:
		...
		for index in range(6):
			if not _assert_runtime_frame_size("%s/%s_move_%s_%02d.png" % [RUNTIME_DIR, CHARACTER_ID, file_direction, index]):
				return
	print("Biologist PixelLab pack smoke test passed.")
	quit(0)
...
func _assert_runtime_frame_size(path: String) -> bool:
	...
	if height != 245:
		_fail("Expected %s visible height to be normalized to 245 px, got %d." % [path, height])
		return false
	return true
