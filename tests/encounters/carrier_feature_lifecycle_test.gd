...
func _check_registry_default_off() -> void:
	...
	if not malformed.is_empty():
		...
		_expect(feature.plan(context, malformed).is_empty(), "out-of-cap reward config must fail closed")
...
func _check_failure_and_determinism() -> void:
	...
	var outcome := first_feature.resolve(first["context"], "combat_end")
	_expect(first["player"].xp == 0 and first["player"].money == 0 and str(outcome.get("reason", "")) == "carrier_lost",
