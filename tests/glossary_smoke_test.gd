...
# Smoke-тест glossary.gd (был непокрыт). Глоссарий — игровые термины с подсказками
# в кодексе и тултипах (ui_screens _make_glossary_term_button / _glossary_tooltip).
# Поломка (пустой/битый термин, потерянный базовый стат) всплыла бы только когда
# игрок откроет подсказку. Валидирует контент, аксессоры, защитную копию и
