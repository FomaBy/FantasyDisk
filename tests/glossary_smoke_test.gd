...
# Smoke-тест glossary.gd (был непокрыт). Глоссарий — игровые термины: карточки
# раздела кодекса и определения в правом досье (SCRUM-884, ui_screens
# _build_codex_glossary / _codex_glossary_sections). Поломка (пустой/битый термин,
# потерянный базовый стат) всплыла бы только когда игрок откроет статью.
