...
func _initialize() -> void:
	var frames := load("res://assets/sprites/characters/dark_mage_spriteframes.tres") as SpriteFrames
	if frames == null:
	...
	if portrait == null or portrait.texture == null:
		_fail("Expected Hero Select to show an animated Dark Mage portrait texture.")
		return
	var first_texture := portrait.texture
	await create_timer(0.24).timeout
	if portrait.texture == null or portrait.texture == first_texture:
		_fail("Expected Hero Select Dark Mage preview to advance through PixelLab direction frames.")
		return

	print("Hero Select Dark Mage PixelLab preview smoke test passed.")
	quit()
