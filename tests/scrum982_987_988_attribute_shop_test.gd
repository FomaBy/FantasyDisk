...
func _assert_attribute_shop_layout(main: Node, viewport_size: Vector2i, expected_count: int, context: String) -> void:
	...
	for offer_node in offers.get_children():
		...
		if interpretation == null or interpretation.text.strip_edges() == "" or not disclosure.contains(interpretation.text):
			_errors.append("%s: %s disclosure must preserve the unabridged class interpretation." % [context, offer.name])
		...
		if full_influence == "" or not disclosure.contains(full_influence):
			_errors.append("%s: %s disclosure must preserve the full influence list behind the compact lane." % [context, offer.name])
		for preview_line in full_preview.split("\n", false):
			if str(preview_line) != "" and not disclosure.contains(str(preview_line)):
				_errors.append("%s: %s disclosure lost full preview line '%s'." % [context, offer.name, preview_line])
