...
class Host extends Node2D:
	...
	var active_transitions: Array[bool] = []
	...
	func ultimate_host_set_active(value: bool) -> void:
		active = value
		active_transitions.append(value)
...
func _test_scarlet_whirlwind() -> void:
	...
	_check(is_equal_approx(boss.max_health - boss.health, boss.max_health * 0.1),
	...
	_check(is_equal_approx(activation.applied_total, _removed_health(host.fixture_targets)),
		"whirlwind attribution must equal HP actually removed, including overkill")
	controller.cancel()
	await process_frame
...
func _test_execution_loop() -> void:
	...
	_check(is_equal_approx(activation.applied_total, _removed_health(host.fixture_targets)),
		"execution-loop attribution must equal HP actually removed")
	controller.cancel()
	await process_frame
...
func _test_fourfold_rift() -> void:
	...
	_check(is_equal_approx(activation.applied_total, _removed_health(host.fixture_targets)),
		"fourfold-rift attribution must equal HP actually removed")
	controller.cancel()
	await process_frame
...
## composition instead of resolving early; every already queued callback stays
## inert until the activation tears itself down naturally.
func _test_ordered_composition_fails_closed() -> void:
	...
	var activation = controller.active_activation()
	var effect := _effect(activation)
	var queued_tweens: Array = activation.tweens_for_tests()
	effect.call("beat", 2)
	...
	await process_frame
	_check(activation.composition_aborted() and target.received.is_empty()
		and host.damage_calls == 0 and host.presentations.is_empty(),
		"queued beats after abort must emit no damage or presentation callback")
	_check(not controller.is_active() and activation.is_finished()
		and activation.tweens_for_tests().is_empty()
		and activation.spawned_for_tests().is_empty()
		and activation.target_ledger_size_for_tests() == 0,
		"an aborted composition must release tween, spawn and target ownership")
	for tween in queued_tweens:
		_check(tween == null or not tween.is_valid(),
			"the aborted composition must invalidate its queued lifecycle tween")
	_check(not is_instance_valid(effect) and host.active_transitions == [true, false],
		"abort must free the effect and close the active signal exactly once")
	await _drop(host)
...
func _removed_health(targets: Array[Node2D]) -> float:
	var removed := 0.0
