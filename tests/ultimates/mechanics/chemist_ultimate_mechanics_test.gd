...
func _test_package_admission() -> void:
	...
	_check(_registry.package_pair_keys() == [
		"chemist/acid_flask", "chemist/blast_powder", "chemist/homunculus_vial",
	], "exactly the three chemist pairs must be executable, got %s" % str(_registry.package_pair_keys()))
...
## Nothing outside the chemist package may have been switched on with it.
func _check_no_leakage() -> void:
	for raw_class_id in PD.WEAPONS_BY_CLASS.keys():
		...
		for raw_weapon_id in (PD.WEAPONS_BY_CLASS[class_id] as Dictionary).keys():
			...
			if class_id == CLASS_ID and WEAPON_IDS.has(weapon_id):
				continue
			_check(_registry.resolution_source(class_id, weapon_id)
					== Resolver.SOURCE_LEGACY_CLASS_FALLBACK,
				"%s/%s must still resolve to its legacy class ultimate" % [class_id, weapon_id])
			_check(not _registry.has_exact_executor_pair(class_id, weapon_id),
				"%s/%s must have no executable pair" % [class_id, weapon_id])
