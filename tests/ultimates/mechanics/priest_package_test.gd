...
func _initialize() -> void:
	...
	_check(registry.package_validation_errors().is_empty(),
		"Priest package discovery must be clean: %s" % [registry.package_validation_errors()])
	var pairs: Array[String] = []
	for key in registry.package_pair_keys():
		if key.begins_with(CLASS_ID + "/"):
			pairs.append(key)
	_check(pairs == [
		"priest/priest_censer", "priest/priest_chime", "priest/priest_reliquary",
	], "exactly the Priest trio must be admitted, got %s" % [pairs])
...
func _test_pair(registry: Registry, weapon_id: String) -> void:
	...
	_check(str(((profile.get("charge", {}) as Dictionary).get("strategy_id", ""))) == "ultimate_charge_ledger",
		"%s must declare the shared U3 charge ledger" % weapon_id)
	...
	_check(str(constants.get("PROFILE_ID", "")) == "weapon_ultimate.profile.priest.%s" % weapon_id,
		"%s PROFILE_ID must match the immutable catalog" % weapon_id)
	_check(str(constants.get("EXECUTOR_ID", "")) == "weapon_ultimate.executor.priest.%s" % weapon_id,
		"%s EXECUTOR_ID must match the immutable catalog" % weapon_id)
	var params := ((profile["executor"] as Dictionary)["params"] as Dictionary)
	for key in (EXPECTED_RAILS[weapon_id] as Dictionary):
		_check(params.get(key) == (EXPECTED_RAILS[weapon_id] as Dictionary)[key],
			"%s must freeze %s=%s" % [weapon_id, key, (EXPECTED_RAILS[weapon_id] as Dictionary)[key]])
...
func _test_fail_closed_mutation(registry: Registry) -> void:
	...
	var weapon_id := "priest_censer"
	var relative_path := "priest/%s.json" % weapon_id
	var document = JSON.parse_string(FileAccess.get_file_as_string(
		"res://data/ultimates/classes/%s" % relative_path
	))
	var executor = load("res://scripts/ultimates/classes/priest/%s.gd" % weapon_id)
	...
	((mutated["executor"] as Dictionary)["params"] as Dictionary)["unbounded_counter"] = 999.0
	var result := Discovery.new().validate_pair(
		mutated, relative_path, executor, base_profiles["priest/%s" % weapon_id]
	)
...
func _report() -> void:
	if _errors.is_empty():
		print("priest_package_test: PASS")
		quit(0)
		return
	for error in _errors:
		push_error("priest_package_test: %s" % error)
	print("priest_package_test: FAIL (%d)" % _errors.size())
	quit(1)
