...
func _initialize() -> void:
	...
	await _test_moon_all_neighbors_dead()
...
## When all split neighbours are dead (0 HP), the mark is cancelled safely:
## no stale target, no double signal/tween, and no crash.
func _test_moon_all_neighbors_dead() -> void:
	var host := _host()
	var prey := _target(host, Vector2(AIM_RANGE, 0.0), 10000.0)
	var neighbours: Array[Target] = []
	for index in 4:
		neighbours.append(_target(host, Vector2(AIM_RANGE + 40.0 + 20.0 * float(index), 60.0), 0.0))
	var controller := Controller.new(host, _registry)
	_check(controller.activate(CLASS_ID, "moon_crossbow"), "Moon Hunt must activate")
	var activation = controller.active_activation()
	var effect := _effect(activation)
	_advance(activation, 0.71)
	var mark_count_before := int(effect.get("mark_transfer_count_for_tests"))
	_advance(activation, 3.25)
	var prey_health := prey.health
	var living_count := 0
	for neighbour in neighbours:
		var n_health := 0.0
		if neighbour.has("health"):
			n_health = float(neighbour.get("health"))
		if n_health > 0.0:
			living_count += 1
	_check(prey_health <= 0.0, "prey must be dead after five bolt waves")
	_check(living_count == 0, "all neighbours must be dead (0 HP)")
	_check(int(effect.get("mark_transfer_count_for_tests")) == mark_count_before,
		"mark transfer count must not increase when no living neighbour exists")
	var final_target: Node = effect.get("marked_target_for_tests")
	_check(final_target == null or not is_instance_valid(final_target) or final_target == prey,
		"marked_target_for_tests must not point to a dead neighbour")
	if final_target == prey:
		_check(true, "mark safely cancelled: no living neighbour to inherit; marked_target_for_tests == prey means cleanup is pending")
	controller.cancel()
	await process_frame
	_check(not is_instance_valid(effect) and not host.active,
		"cancellation must remove the hunt scene and its active latch")
