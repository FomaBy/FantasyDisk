...
const DURATIONS := {"sniper_deadeye_rifle": 2.9, "sniper_spotter_scope": 3.4, "sniper_shatter_rounds": 3.1}
const STEP := 0.01
...
func _test_arena_coverage(weapon_id: String, crowd_size: int) -> void:
	...
	for target in targets:
		...
		if weapon_id == "sniper_spotter_scope":
			_check(not _status_with_prefix(target, "sniper_ultimate_spotter_").is_empty(),
				"Spotter must suppress every covered enemy")
		if weapon_id == "sniper_shatter_rounds":
			_check(not _status_with_prefix(target, "sniper_ultimate_shatter_").is_empty(),
				"Shatter must stagger every covered enemy")
	_check(not elite.received.is_empty() and elite.health < elite.max_health,
		"%s must damage the elite in the live arena" % weapon_id)
	if weapon_id == "sniper_spotter_scope":
		var suppression := _status_with_prefix(elite, "sniper_ultimate_spotter_")
		_check(not suppression.is_empty() and not suppression.has("movement_locked"),
			"Spotter must apply the resisted, non-pinning elite suppression")
	if weapon_id == "sniper_shatter_rounds":
		var stagger := _status_with_prefix(elite, "sniper_ultimate_shatter_")
		_check(not stagger.is_empty() and not stagger.has("movement_locked"),
			"Shatter must apply the resisted, non-pinning elite stagger")
