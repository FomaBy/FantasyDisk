...
## FAN-3942 focused gate for the Assassin live certification capture package.
##
...
##       --script res://tests/ultimates/presentation/assassin_certification_capture_test.gd

const Capture := preload("res://tests/ultimates/presentation/assassin_certification_live_capture.gd")
...
const CLASS_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/assassin/manifest.json"
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/assassin/certification_capture_manifest.json"
const POINTER_FIXTURE_PATH := "user://fan3942_assassin_capture_pointer.png"
...
const ACCEPTED_CEILINGS := {
	"chakrams": {"nodes": 14, "coverage": 0.30},
	"shadow_daggers": {"nodes": 9, "coverage": 0.14},
	"venom_wire": {"nodes": 11, "coverage": 0.24},
}
...
func _check_negative_probes(manifest: Dictionary, profile: Dictionary, class_manifest: Dictionary, errors: Array[String]) -> void:
	...
	_expect(png_violations("res://docs/design/reference-assets-lfs/ultimate-certification/assassin/absent.png", Vector2i(8, 8)).has("missing"), "a missing PNG must fail closed", errors)
...
func _finish(errors: Array[String]) -> void:
	if errors.is_empty():
		print("FAN-3942 Assassin certification capture package: PASS")
		quit(0)
		return
	for error in errors:
		push_error("FAN-3942 Assassin certification capture package: %s" % error)
	quit(1)
