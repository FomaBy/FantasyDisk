...
## FAN-3942 live certification capture for the three canonical Assassin weapon
## ultimates.
##
...
## `Player` configured for `assassin/<weapon>`, and shipped `Enemy` instances
...
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/assassin/certification_capture_manifest.json"
const OUTPUT_DIR := "res://docs/design/reference-assets-lfs/ultimate-certification/assassin"
const BACKDROP_NODE_NAME := "BackdropVeil"

const WEAPON_IDS: Array[String] = ["chakrams", "shadow_daggers", "venom_wire"]
...
const CAPTURE_SEED := 394220260911
...
func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-3942 Assassin certification capture skipped (headless); run windowed for PNG evidence.")
		quit(0)
		return
	seed(CAPTURE_SEED)
	_source = {
	...
	if manifest.is_empty():
		push_error("FAN-3942 Assassin certification capture: %s is missing or invalid" % MANIFEST_PATH)
		quit(1)
		return
	...
	if output_error != OK and output_error != ERR_ALREADY_EXISTS:
		push_error("FAN-3942 Assassin certification capture: cannot create %s (%s)" % [OUTPUT_DIR, error_string(output_error)])
		quit(1)
		return
	...
	if write_error != OK:
		push_error("FAN-3942 Assassin certification capture: manifest write failed (%s)" % error_string(write_error))
		quit(1)
		return
	print("FAN-3942 Assassin certification capture: %d samples, %d sheets written to %s" % [
...
func _render_sheet(host: Node2D, size: Vector2i, path: String) -> int:
	...
	if result == OK:
		print("FAN-3942 Assassin certification capture saved: %s" % path)
	return result
