...
## FAN-3935 focused gate for the Berserk live certification capture package.
##
...
const Capture := preload("res://tests/ultimates/presentation/berserk_certification_live_capture.gd")
...
const CLASS_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/berserk/manifest.json"
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/berserk/certification_capture_manifest.json"
const POINTER_FIXTURE_PATH := "user://fan3935_berserk_capture_pointer.png"
...
func declaration_violations(manifest: Dictionary, profile: Dictionary) -> Array[String]:
	...
	if str(manifest.get("issue", "")) != "FAN-3935":
...
## declared beat.
func _beat_time_violations(key: String, sample: Dictionary, weapon: Dictionary, declared: Variant) -> Array[String]:
	var violations: Array[String] = []
	...
	if not is_equal_approx(float(sample.get("declared_beat_seconds", -1.0)), declared_time):
		violations.append("%s: declared beat time %s does not match the shared contract %.3f" % [
	...
	if beat_id != "recovery":
		...
		return violations
	var live_release := float((weapon.get("timing_seconds", {}) as Dictionary).get("recovery", 0.0))
	var active_time := _declared_beat_time(declared, "active")
...
func _check_class_manifest_registration(class_manifest: Dictionary, manifest: Dictionary, errors: Array[String]) -> void:
	...
	if live.is_empty():
		...
		return
	if str(live.get("issue", "")) != "FAN-3935":
...
func _check_negative_probes(manifest: Dictionary, profile: Dictionary, class_manifest: Dictionary, errors: Array[String]) -> void:
	...
	((wrong_scene.get("samples", []) as Array)[0] as Dictionary)["presentation_scene"] = "res://scenes/vfx/ultimates/berserk/NotShipped.tscn"
	...
	_expect(png_violations("res://docs/design/reference-assets-lfs/ultimate-certification/berserk/absent.png", Vector2i(8, 8)).has("missing"), "a missing PNG must fail closed", errors)
...
func _finish(errors: Array[String]) -> void:
	if errors.is_empty():
		print("FAN-3935 Berserk certification capture package: PASS")
		quit(0)
		return
	for error in errors:
		push_error("FAN-3935 Berserk certification capture package: %s" % error)
	quit(1)
