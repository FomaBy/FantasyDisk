...
## FAN-3935 live certification capture for the three canonical Berserk weapon
## ultimates.
##
...
## `Player` configured for `berserk/<weapon>`, and shipped `Enemy` instances
...
##
## `BERSERK_CERT_SOURCE_REF`, `BERSERK_CERT_SOURCE_SHA` and
## `BERSERK_CERT_SOURCE_TREE` pin the checkout the captures were taken from. They
...
##
## `BERSERK_CERT_FRAME_DIR` optionally names a directory that receives every
...
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/berserk/certification_capture_manifest.json"
const OUTPUT_DIR := "res://docs/design/reference-assets-lfs/ultimate-certification/berserk"
const DRIVER_SCRIPT_PATH := "res://scenes/vfx/ultimates/berserk/berserk_ultimate_v2_driver.gd"
const BACKDROP_NODE_NAME := "BackdropVeil"

const WEAPON_IDS: Array[String] = ["sword", "axe", "hammer"]
...
## switch every Berserk driver reads in `_ready()`, and `combat_feedback` is the
...
const CAPTURE_SEED := 393520260909
...
func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-3935 Berserk certification capture skipped (headless); run windowed for PNG evidence.")
		quit(0)
		return
	seed(CAPTURE_SEED)
	_source = {
		"ref": OS.get_environment("BERSERK_CERT_SOURCE_REF").strip_edges(),
		"commit_sha": OS.get_environment("BERSERK_CERT_SOURCE_SHA").strip_edges().to_lower(),
		"tree_sha": OS.get_environment("BERSERK_CERT_SOURCE_TREE").strip_edges().to_lower(),
	}
	for field in ["ref", "commit_sha", "tree_sha"]:
		if str(_source[field]).is_empty():
			push_error("FAN-3935 Berserk certification capture: BERSERK_CERT_SOURCE_%s must name the checkout the captures come from" % field.to_upper())
			quit(1)
			return
	_frame_dir = OS.get_environment("BERSERK_CERT_FRAME_DIR").strip_edges()
	if not _frame_dir.is_empty():
		DirAccess.make_dir_recursive_absolute(_frame_dir)
	var manifest := _load_json(MANIFEST_PATH)
	if manifest.is_empty():
		push_error("FAN-3935 Berserk certification capture: %s is missing or invalid" % MANIFEST_PATH)
		quit(1)
		return
	...
	if output_error != OK and output_error != ERR_ALREADY_EXISTS:
		push_error("FAN-3935 Berserk certification capture: cannot create %s (%s)" % [OUTPUT_DIR, error_string(output_error)])
		quit(1)
		return
		...
		for weapon_id in WEAPON_IDS:
			for raw_mode in MODES:
				...
				if not failure.is_empty():
					push_error("FAN-3935 Berserk certification capture: %s" % failure)
					quit(1)
					return
		var matrix_error := await _write_matrix_sheet(viewport)
		if matrix_error != OK:
			push_error("FAN-3935 Berserk certification capture: matrix sheet %s failed (%s)" % [viewport["id"], error_string(matrix_error)])
			quit(1)
			return

	var write_error := _write_capture_manifest()
	if write_error != OK:
		push_error("FAN-3935 Berserk certification capture: manifest write failed (%s)" % error_string(write_error))
		quit(1)
		return
	print("FAN-3935 Berserk certification capture: %d samples, %d sheets written to %s" % [
...
func _capture_combination(viewport: Dictionary, weapon_id: String, mode: Dictionary) -> String:
	...
	## own settings, and before the ultimate spawns: the Berserk driver reads
	...
	for beat in beats:
		...
		if not _frame_dir.is_empty():
			frame.save_png("%s/berserk_%s_%s_%s_%s.png" % [
...
## A live cast releases its presentation node at the class manifest's `recovery`
## time, which is slightly earlier than the frame-local recovery beat, so the
## recovery sample is pulled back to the last frame the live scene still draws.
## Both times are published: nothing is silently retimed.
func _beats_for(weapon_id: String) -> Array:
	...
	for beat_id in BEAT_IDS:
		for raw_beat in declared as Array:
			...
			var sample_time := declared_time
			if beat_id == "recovery" and last_drawn_frame > 0.0:
				sample_time = minf(declared_time, last_drawn_frame)
			ordered.append({
				"phase": beat_id,
...
func _write_matrix_sheet(viewport: Dictionary) -> int:
	...
	_add_heading(host, size, "BERSERK ULTIMATES — LIVE %s CAPTURE • ACTIVE BEAT • REAL PLAYER, HAZARDS AND HUD" % str(viewport["id"]).to_upper())
	...
	var path := "%s/berserk_certification_%s.png" % [OUTPUT_DIR, str(viewport["id"])]
...
func _render_sheet(host: Node2D, size: Vector2i, path: String) -> int:
	...
	if result == OK:
		print("FAN-3935 Berserk certification capture saved: %s" % path)
	return result
...
func _write_capture_manifest() -> int:
	var payload := {
		...
		"capture": {
			...
			"capture_script": "tests/ultimates/presentation/berserk_certification_live_capture.gd",
			"focused_test": "tests/ultimates/presentation/berserk_certification_capture_test.gd",
			"seed": CAPTURE_SEED,
			"fixed_fps": 60,
			"godot_version": "%s.%s" % [
		...
		"commands": {
			"live_capture": "BERSERK_CERT_SOURCE_REF=<ref> BERSERK_CERT_SOURCE_SHA=<sha> BERSERK_CERT_SOURCE_TREE=<tree> python3 tools/godot_gate.py --path . --windowed --fixed-fps 60 --script res://tests/ultimates/presentation/berserk_certification_live_capture.gd",
			"focused_test": "python3 tools/godot_gate.py --headless --path . --script res://tests/ultimates/presentation/berserk_certification_capture_test.gd",
			"class_timelines": "python3 tools/godot_gate.py --headless --path . --script res://tests/ultimates/presentation/berserk_ultimate_timelines.gd",
