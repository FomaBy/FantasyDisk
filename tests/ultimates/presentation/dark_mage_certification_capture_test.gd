...
## Headless integrity gate for the FAN-3938 windowed Dark Mage capture package.
##
...
const Capture := preload("res://tests/ultimates/presentation/dark_mage_certification_live_capture.gd")
...
const LEGACY_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/dark_mage/manifest.json"
const POINTER_FIXTURE_PATH := "user://fan3938_dark_mage_capture_pointer.png"
...
func _check_manifest_shape(manifest: Dictionary, profile: Dictionary, errors: Array[String]) -> void:
	...
	_expect(str(manifest.get("class_id", "")) == "dark_mage", "capture manifest must stay Dark Mage-local", errors)
	_expect(str(manifest.get("capture_script", "")) == "tests/ultimates/presentation/dark_mage_certification_live_capture.gd", "capture manifest must pin its renderer", errors)
...
func _check_live_source_mapping(legacy_manifest: Dictionary, errors: Array[String]) -> void:
	...
	expected.sort()
	_expect(ids == expected, "legacy Dark Mage manifest must enumerate exactly the canonical trio", errors)
	var positive := DirectionContract.victim_impact_violations_from_sources("dark_mage", weapons)
	...
	if not missing.is_empty():
		(missing[0] as Dictionary)["weapon_id"] = "dark_mage_missing_capture_weapon"
	var negative := DirectionContract.victim_impact_violations_from_sources("dark_mage", missing)
	_expect(negative.size() == 1 and str(negative[0]).contains("dark_mage_missing_capture_weapon"), "a missing canonical weapon mapping must fail closed: %s" % str(negative), errors)
...
func _check_negative_probes(manifest: Dictionary, legacy_manifest: Dictionary, errors: Array[String]) -> void:
	...
	if source_weapons.is_empty():
		errors.append("legacy Dark Mage manifest must supply the source-mapping negative probe")


func _manifest_violations(manifest: Dictionary, profile: Dictionary) -> Array[String]:
	var violations: Array[String] = []
	if str(manifest.get("class_id", "")) != "dark_mage":
		violations.append("class_id")
	if str(manifest.get("capture_script", "")) != "tests/ultimates/presentation/dark_mage_certification_live_capture.gd":
...
func _finish(errors: Array[String]) -> void:
	if errors.is_empty():
		print("FAN-3938 Dark Mage four-viewport capture package: PASS")
		quit(0)
		return
	for error in errors:
		push_error("FAN-3938 Dark Mage four-viewport capture package: %s" % error)
	quit(1)
