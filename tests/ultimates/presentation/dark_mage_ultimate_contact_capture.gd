...
const CAPTURE_SPEC := preload("res://tests/ultimates/presentation/dark_mage_ultimate_timelines.gd")


func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-1478 Dark Mage ultimate contact capture skipped (headless); run windowed for PNGs.")
		quit(0)
		return
	for capture in CAPTURE_SPEC.CAPTURES:
		...
		await process_frame
		if error != OK:
			push_error("Dark Mage ultimate contact capture failed: %s" % error_string(error))
			quit(1)
			return
		print("Dark Mage ultimate contact capture saved: %s" % output)
	quit(0)
...
func _make_sheet(size: Vector2i) -> Node2D:
	...
	background.color = Color(0.018, 0.012, 0.032, 1.0)
	background.z_index = -20
	host.add_child(background)
	var title := Label.new()
	title.text = "DARK MAGE WEAPON ULTIMATES — DISTINCT PRESENTATION TIMELINES"
	title.position = Vector2(size.x * 0.5 - minf(size.x * 0.38, 560.0), size.y * 0.075)
	title.add_theme_font_size_override("font_size", maxi(18, int(size.y * 0.036)))
	title.add_theme_color_override("font_color", Color(0.89, 0.78, 1.0))
	...
	for raw_pack in CAPTURE_SPEC.PACKS:
		...
		panel.color = Color(0.065, 0.035, 0.095, 0.97)
		panel.z_index = -10
