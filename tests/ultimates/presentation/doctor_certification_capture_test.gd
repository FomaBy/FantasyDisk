...
const ACCEPTED_CEILINGS := {
	"restore_potion": {"nodes": 5, "coverage": 0.12},
	"plague_syringe": {"nodes": 8, "coverage": 0.30},
	"bone_saw": {"nodes": 8, "coverage": 0.10},
}
...
func _check_accessibility_modes(class_manifest: Dictionary, errors: Array[String]) -> void:
	...
	for mode in modes:
		...
		for raw_weapon in class_manifest.get("weapons", []) as Array:
			...
			var backdrop := scene.get_node_or_null("BackdropLayer/BackdropVeil") as TextureRect
			var screen_space: bool = backdrop != null and backdrop.get_parent() is CanvasLayer \
				...
				and backdrop.texture is GradientTexture2D \
				and (backdrop.texture as GradientTexture2D).fill == GradientTexture2D.FILL_RADIAL
...
func _check_negative_probes(manifest: Dictionary, profile: Dictionary, class_manifest: Dictionary, errors: Array[String]) -> void:
	...
	_expect(not _is_evidence_only_path("scenes/vfx/ultimates/doctor/changed_scene.tscn", source_sha),
		"a doctor scene edit must make the capture stale", errors)
