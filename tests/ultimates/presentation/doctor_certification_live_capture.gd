...
func _initialize() -> void:
	...
	for raw_viewport in VIEWPORTS:
		...
		for beat_id in BEAT_IDS:
			...
			if matrix_error != OK:
				push_error("FAN-3942 Doctor certification capture: matrix sheet %s/%s failed (%s)" % [viewport["id"], beat_id, error_string(matrix_error)])
				quit(1)
				return
...
func _write_matrix_sheet(viewport: Dictionary, beat_id: String) -> int:
	...
	_add_heading(host, size, "DOCTOR ULTIMATES — LIVE %s CAPTURE • %s BEAT • REAL PLAYER ACTIVATION" % [str(viewport["id"]).to_upper(), beat_id.to_upper()])
	...
	var path := "%s/doctor_certification_%s_%s.png" % [OUTPUT_DIR, str(viewport["id"]), beat_id]
...
func _write_capture_manifest() -> int:
	var payload := {
		...
		"commands": {
			"live_capture": "FSD_GODOT_EXCLUSIVE=1 python3 tools/godot_gate.py --path . --windowed --fixed-fps 60 --script res://tests/ultimates/presentation/doctor_certification_live_capture.gd",
...
func _clean_source() -> Dictionary:
	...
	if head.length() != 40 or tree.length() != 40 or branch.is_empty() or not dirty.is_empty():
		push_error("FAN-3942 Doctor certification capture: source checkout must be a clean named branch")
		return {}
