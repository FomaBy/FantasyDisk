...
## FAN-3942 live certification capture for the three canonical Doctor weapon
## ultimates.
##
...
## `Player` configured for `doctor/<weapon>`, and shipped `Enemy` instances
...
## `contact_sheet_beats_contract.gd`. All 144 samples are measured at their native
...
##       --fixed-fps 60 \
##       --script res://tests/ultimates/presentation/doctor_certification_live_capture.gd
##
## `DOCTOR_CERT_SOURCE_REF`, `DOCTOR_CERT_SOURCE_SHA` and
## `DOCTOR_CERT_SOURCE_TREE` pin the checkout the captures were taken from. They
...
##
## `DOCTOR_CERT_FRAME_DIR` optionally names a directory that receives every
...
const OUTPUT_DIR := "res://docs/design/reference-assets-lfs/ultimate-certification/doctor"
...
## The two persisted ultimate accessibility flags are applied through the shipped
## root-snapshot API before the authored scene spawns. `screen_shake` and
## `combat_feedback` retain their existing game-wide responsibilities.
const MODES := [
	{
		"id": "normal", "label": "NORMAL", "screen_shake": true,
		"combat_feedback": true, "reduced_motion": false,
		"photosensitivity_safe": false, "hazards": 6, "crowd_cap": false,
	},
	{
		"id": "crowded", "label": "CROWDED", "screen_shake": true,
		"combat_feedback": true, "reduced_motion": false,
		"photosensitivity_safe": false, "hazards": 6, "crowd_cap": true,
	},
	{
		"id": "reduced_motion", "label": "REDUCED MOTION", "screen_shake": false,
		"combat_feedback": true, "reduced_motion": true,
		"photosensitivity_safe": false, "hazards": 6, "crowd_cap": false,
	},
	{
		"id": "photosensitivity_safe", "label": "PHOTOSENSITIVITY SAFE", "screen_shake": true,
		"combat_feedback": false, "reduced_motion": false,
		"photosensitivity_safe": true, "hazards": 6, "crowd_cap": false,
	},
]
...
const CAPTURE_SEED := 394220260912
...
func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-3942 Doctor certification capture skipped (headless); run windowed for PNG evidence.")
		quit(0)
		return
	seed(CAPTURE_SEED)
	_source = {
		"ref": OS.get_environment("DOCTOR_CERT_SOURCE_REF").strip_edges(),
		"commit_sha": OS.get_environment("DOCTOR_CERT_SOURCE_SHA").strip_edges().to_lower(),
		"tree_sha": OS.get_environment("DOCTOR_CERT_SOURCE_TREE").strip_edges().to_lower(),
	}
	for field in ["ref", "commit_sha", "tree_sha"]:
		if str(_source[field]).is_empty():
			push_error("FAN-3942 Doctor certification capture: DOCTOR_CERT_SOURCE_%s must name the checkout the captures come from" % field.to_upper())
			quit(1)
			return
	_frame_dir = OS.get_environment("DOCTOR_CERT_FRAME_DIR").strip_edges()
	if not _frame_dir.is_empty():
		DirAccess.make_dir_recursive_absolute(_frame_dir)
	var manifest := _load_json(MANIFEST_PATH)
	if manifest.is_empty():
		push_error("FAN-3942 Doctor certification capture: %s is missing or invalid" % MANIFEST_PATH)
		quit(1)
		return
	...
	if output_error != OK and output_error != ERR_ALREADY_EXISTS:
		push_error("FAN-3942 Doctor certification capture: cannot create %s (%s)" % [OUTPUT_DIR, error_string(output_error)])
		quit(1)
		return
		...
		for weapon_id in WEAPON_IDS:
			for raw_mode in MODES:
				...
				if not failure.is_empty():
					push_error("FAN-3942 Doctor certification capture: %s" % failure)
					quit(1)
					return
		var matrix_error := await _write_matrix_sheet(viewport)
		if matrix_error != OK:
			push_error("FAN-3942 Doctor certification capture: matrix sheet %s failed (%s)" % [viewport["id"], error_string(matrix_error)])
			quit(1)
			return

	var write_error := _write_capture_manifest()
	if write_error != OK:
		push_error("FAN-3942 Doctor certification capture: manifest write failed (%s)" % error_string(write_error))
		quit(1)
		return
	print("FAN-3942 Doctor certification capture: %d samples, %d sheets written to %s" % [
...
func _capture_combination(viewport: Dictionary, weapon_id: String, mode: Dictionary) -> String:
	...
	## own settings and before the ultimate spawns, so the class driver reads one
	## coherent snapshot in `_ready()` and `begin()`.
...
func _screen_rect(item: CanvasItem) -> Rect2:
	...
	if item is Sprite2D:
		...
		local = Rect2(Vector2.ZERO, control.size)
	else:
		return Rect2()
...
func _presentation_root(main: Node, weapon_id: String) -> Node2D:
...
## and are sampled exactly. Every declared beat is inside the scene's live cancel
## envelope, so no synthetic seek or early replacement is necessary.
func _beats_for(weapon_id: String) -> Array:
	var frames := Beats.frames_for_class(CLASS_ID)
	var declared = frames.get(weapon_id, [])
	...
	for beat_id in BEAT_IDS:
		for raw_beat in declared as Array:
			...
			if str(beat.get("phase", "")) != beat_id:
				continue
			var declared_time := float(beat.get("time", 0.0))
			ordered.append({
				"phase": beat_id,
				"required_nodes": beat.get("required_nodes", []),
...
func _write_matrix_sheet(viewport: Dictionary) -> int:
	...
	_add_heading(host, size, "DOCTOR ULTIMATES — LIVE %s CAPTURE • ACTIVE BEAT • REAL PLAYER, HAZARDS AND HUD" % str(viewport["id"]).to_upper())
	...
	var path := "%s/doctor_certification_%s.png" % [OUTPUT_DIR, str(viewport["id"])]
...
func _render_sheet(host: Node2D, size: Vector2i, path: String) -> int:
	...
	if result == OK:
		print("FAN-3942 Doctor certification capture saved: %s" % path)
	return result
...
func _write_capture_manifest() -> int:
	var payload := {
		...
		"capture": {
			...
			"beat_source": "scripts/ultimates/presentation/contact_sheet_beats_contract.gd",
			"captured_at": Time.get_datetime_string_from_system(true, true),
		},
		"commands": {
			"live_capture": "FSD_GODOT_EXCLUSIVE=1 DOCTOR_CERT_SOURCE_REF=<ref> DOCTOR_CERT_SOURCE_SHA=<sha> DOCTOR_CERT_SOURCE_TREE=<tree> python3 tools/godot_gate.py --path . --windowed --fixed-fps 60 --script res://tests/ultimates/presentation/doctor_certification_live_capture.gd",
			"focused_test": "python3 tools/godot_gate.py --headless --path . --script res://tests/ultimates/presentation/doctor_certification_capture_test.gd",
			"class_timelines": "python3 tools/godot_gate.py --headless --path . --script res://tests/ultimates/presentation/doctor_ultimate_timelines.gd",
...
func _mode_declarations() -> Array:
	...
	for raw_mode in MODES:
		...
		declared.append({
			...
			"ultimate_reduced_motion": bool(mode["reduced_motion"]),
			"ultimate_photosensitivity_safe": bool(mode["photosensitivity_safe"]),
