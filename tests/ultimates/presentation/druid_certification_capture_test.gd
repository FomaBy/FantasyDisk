...
## FAN-3942 focused gate for the Druid live certification capture package.
##
...
##       --script res://tests/ultimates/presentation/druid_certification_capture_test.gd

const Capture := preload("res://tests/ultimates/presentation/druid_certification_live_capture.gd")
...
const CLASS_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/druid/manifest.json"
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/druid/certification_capture_manifest.json"
const POINTER_FIXTURE_PATH := "user://fan3942_druid_capture_pointer.png"
...
func _is_evidence_only_path(path: String, source_sha: String) -> bool:
	...
	if path == "CHANGELOG.md" or path == "assets/marketing/fantasydisk_031_announcement.png" \
		...
		or path == "assets/marketing/fantasydisk_032_announcement.png" \
		or path == "assets/marketing/fantasydisk_032_announcement.png.import" \
		or path.begins_with("docs/design/references/release_0_3_2/"):
		return true
...
func _check_negative_probes(manifest: Dictionary, profile: Dictionary, class_manifest: Dictionary, errors: Array[String]) -> void:
	...
	_expect(not _is_evidence_only_path("scenes/vfx/ultimates/druid/changed_scene.tscn", source_sha),
		"a druid scene edit must make the capture stale", errors)
	...
	_expect(_is_evidence_only_path("assets/marketing/fantasydisk_032_announcement.png.import", source_sha),
		"the 0.3.2 release poster import sidecar must leave the capture applicable", errors)
	_expect(not _is_evidence_only_path("assets/marketing/fantasydisk_033_announcement.png", source_sha),
		"a poster for another release must not be exempted", errors)
	_expect(not _is_evidence_only_path("docs/design/references/release_0_3_3/layout.json", source_sha),
		"another release reference directory must not be exempted", errors)
	...
	((wrong_scene.get("samples", []) as Array)[0] as Dictionary)["presentation_scene"] = "res://scenes/vfx/ultimates/druid/NotShipped.tscn"
	...
	_expect(png_violations("res://docs/design/reference-assets-lfs/ultimate-certification/druid/absent.png", Vector2i(8, 8)).has("missing"), "a missing PNG must fail closed", errors)
...
func _finish(errors: Array[String]) -> void:
	if errors.is_empty():
		print("FAN-3942 Druid certification capture package: PASS")
		quit(0)
		return
	for error in errors:
		push_error("FAN-3942 Druid certification capture package: %s" % error)
	quit(1)
