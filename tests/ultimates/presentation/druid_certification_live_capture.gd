...
## FAN-3942 live certification capture for the three canonical Druid weapon
## ultimates.
##
...
## `Player` configured for `druid/<weapon>`, and shipped `Enemy` instances
...
##       --fixed-fps 60 \
##       --script res://tests/ultimates/presentation/druid_certification_live_capture.gd
##
## `DRUID_CERT_SOURCE_REF`, `DRUID_CERT_SOURCE_SHA` and
## `DRUID_CERT_SOURCE_TREE` pin the checkout the captures were taken from. They
...
##
## `DRUID_CERT_FRAME_DIR` optionally names a directory that receives every
...
const MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/druid/manifest.json"
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/druid/certification_capture_manifest.json"
const OUTPUT_DIR := "res://docs/design/reference-assets-lfs/ultimate-certification/druid"
const BACKDROP_NODE_NAME := "BackdropVeil"

const WEAPON_IDS: Array[String] = ["summon_amulet", "briar_staff", "raven_totem"]
...
func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-3942 Druid certification capture skipped (headless); run windowed for PNG evidence.")
		quit(0)
		return
	seed(CAPTURE_SEED)
	_source = {
		"ref": OS.get_environment("DRUID_CERT_SOURCE_REF").strip_edges(),
		"commit_sha": OS.get_environment("DRUID_CERT_SOURCE_SHA").strip_edges().to_lower(),
		"tree_sha": OS.get_environment("DRUID_CERT_SOURCE_TREE").strip_edges().to_lower(),
	}
	for field in ["ref", "commit_sha", "tree_sha"]:
		if str(_source[field]).is_empty():
			push_error("FAN-3942 Druid certification capture: DRUID_CERT_SOURCE_%s must name the checkout the captures come from" % field.to_upper())
			quit(1)
			return
	_frame_dir = OS.get_environment("DRUID_CERT_FRAME_DIR").strip_edges()
	if not _frame_dir.is_empty():
		DirAccess.make_dir_recursive_absolute(_frame_dir)
	var manifest := _load_json(MANIFEST_PATH)
	if manifest.is_empty():
		push_error("FAN-3942 Druid certification capture: %s is missing or invalid" % MANIFEST_PATH)
		quit(1)
		return
	...
	if output_error != OK and output_error != ERR_ALREADY_EXISTS:
		push_error("FAN-3942 Druid certification capture: cannot create %s (%s)" % [OUTPUT_DIR, error_string(output_error)])
		quit(1)
		return
		...
		for weapon_id in WEAPON_IDS:
			for raw_mode in MODES:
				...
				if not failure.is_empty():
					push_error("FAN-3942 Druid certification capture: %s" % failure)
					quit(1)
					return
		var matrix_error := await _write_matrix_sheet(viewport)
		if matrix_error != OK:
			push_error("FAN-3942 Druid certification capture: matrix sheet %s failed (%s)" % [viewport["id"], error_string(matrix_error)])
			quit(1)
			return

	var write_error := _write_capture_manifest()
	if write_error != OK:
		push_error("FAN-3942 Druid certification capture: manifest write failed (%s)" % error_string(write_error))
		quit(1)
		return
	print("FAN-3942 Druid certification capture: %d samples, %d sheets written to %s" % [
...
func _write_matrix_sheet(viewport: Dictionary) -> int:
	...
	_add_heading(host, size, "DRUID ULTIMATES — LIVE %s CAPTURE • ACTIVE BEAT • REAL PLAYER, HAZARDS AND HUD" % str(viewport["id"]).to_upper())
	...
	var path := "%s/druid_certification_%s.png" % [OUTPUT_DIR, str(viewport["id"])]
...
func _render_sheet(host: Node2D, size: Vector2i, path: String) -> int:
	...
	if result == OK:
		print("FAN-3942 Druid certification capture saved: %s" % path)
	return result
...
func _write_capture_manifest() -> int:
	var payload := {
		...
		"commands": {
			"live_capture": "FSD_GODOT_EXCLUSIVE=1 DRUID_CERT_SOURCE_REF=<ref> DRUID_CERT_SOURCE_SHA=<sha> DRUID_CERT_SOURCE_TREE=<tree> python3 tools/godot_gate.py --path . --windowed --fixed-fps 60 --script res://tests/ultimates/presentation/druid_certification_live_capture.gd",
