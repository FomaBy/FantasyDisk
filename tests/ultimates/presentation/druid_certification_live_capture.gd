...
func _initialize() -> void:
	...
	for raw_viewport in VIEWPORTS:
		...
		for beat_id in BEAT_IDS:
			...
			if matrix_error != OK:
				push_error("FAN-3942 Druid certification capture: matrix sheet %s/%s failed (%s)" % [viewport["id"], beat_id, error_string(matrix_error)])
				quit(1)
				return
...
func _write_matrix_sheet(viewport: Dictionary, beat_id: String) -> int:
	...
	_add_heading(host, size, "DRUID ULTIMATES — LIVE %s CAPTURE • %s BEAT • REAL PLAYER ACTIVATION" % [str(viewport["id"]).to_upper(), beat_id.to_upper()])
	...
	var path := "%s/druid_certification_%s_%s.png" % [OUTPUT_DIR, str(viewport["id"]), beat_id]
