...
const CAPTURE_SPEC := preload("res://tests/ultimates/presentation/druid_ultimate_timelines.gd")
...
func _initialize() -> void:
	...
	for capture in CAPTURE_SPEC.CAPTURES:
		...
		await process_frame
		if error != OK:
			push_error("Druid ultimate contact capture failed: %s" % error_string(error))
			quit(1)
			return
...
func _make_sheet(size: Vector2i) -> Node2D:
	...
	background.color = Color(0.022, 0.036, 0.026, 1.0)
	...
	title.add_theme_color_override("font_color", Color(0.86, 0.97, 0.74))
	...
	for raw_pack in CAPTURE_SPEC.PACKS:
		...
		panel.color = Color(0.05, 0.08, 0.06, 0.96)
		panel.z_index = -10
