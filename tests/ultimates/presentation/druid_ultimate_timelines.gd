...
const PROFILE_PATH := "res://data/ultimates/schema/v1/classes/druid.json"
const MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/druid/manifest.json"
const TIMELINE := preload("res://scripts/ultimates/presentation/weapon_ultimate_presentation_timeline.gd")
const ImpactPlayer := preload("res://scripts/ultimates/presentation/victim_impact_player.gd")
const WEAPON_IDS := ["summon_amulet", "briar_staff", "raven_totem"]
const SCENES := {
	"summon_amulet": preload("res://scenes/vfx/ultimates/druid/DruidSummonAmuletWildHunt.tscn"),
	"briar_staff": preload("res://scenes/vfx/ultimates/druid/DruidBriarStaffForestInOneBreath.tscn"),
	"raven_totem": preload("res://scenes/vfx/ultimates/druid/DruidRavenTotemNightOfThousandWings.tscn"),
}
const EFFECT_SCENES := {
	"summon_amulet": preload("res://scripts/ultimates/classes/druid/summon_amulet.tscn"),
	"briar_staff": preload("res://scripts/ultimates/classes/druid/briar_staff.tscn"),
	"raven_totem": preload("res://scripts/ultimates/classes/druid/raven_totem.tscn"),
}
const FLIPBOOKS := {
	"summon_amulet": {
		"node": "WildHunt",
...
const PACKS := [
	...
	},
	{
		...
		"required_nodes": ["RavenVortex"],
	},
]
const CAPTURES := [
	{"name": "648p", "file": "druid_ultimate_timelines_648p.png", "size": Vector2i(1152, 648)},
	{"name": "720p", "file": "druid_ultimate_timelines_720p.png", "size": Vector2i(1280, 720)},
	{"name": "1080p", "file": "druid_ultimate_timelines_1080p.png", "size": Vector2i(1920, 1080)},
	{"name": "2k", "file": "druid_ultimate_timelines_2k.png", "size": Vector2i(2560, 1440)},
]
...
const SHEET_TITLE := "DRUID WEAPON ULTIMATES — DISTINCT PRESENTATION TIMELINES"
...
class ImpactActivationProbe extends RefCounted:
	...
	func param_float(key: String, fallback: float) -> float:
		return 0.0 if key == "hunt_splash_radius" else fallback
...
func _initialize() -> void:
	...
	if not errors.is_empty():
		_finish(errors)
		return
	_expect(str(manifest.get("class_id", "")) == "druid", "manifest must be class-local to Druid", errors)
	...
	_expect(packages.size() == 3, "manifest must contain exactly three Druid weapon packages", errors)
	...
	if not errors.is_empty():
		_finish(errors)
		return
	print("Druid ultimate timelines passed (three exact flipbooks, frozen phases, victim impacts, lifecycle, and crowd budgets).")
	quit(0)
...
func _check_provenance(manifest: Dictionary, errors: Array[String]) -> void:
	...
	_expect(assets == WEAPON_IDS, "provenance must list exactly the three admitted Druid packs", errors)
	_expect(str(provenance.get("integrated_art_candidate", "")) == "a14f9246527b866ea12ad9d425c2f122a41cc53e", "provenance must pin the accepted art candidate", errors)
...
func _check_scene(weapon_id: String, package: Dictionary, errors: Array[String]) -> void:
	...
	_expect(str(instance.get_meta("ultimate_id", "")) == "druid/%s" % weapon_id, "%s scene must retain its exact profile key" % weapon_id, errors)
...
func _check_weapon_local_impacts(packages: Dictionary, errors: Array[String]) -> void:
	for weapon_id in WEAPON_IDS:
		...
		match weapon_id:
			"summon_amulet":
				effect.call("stampede")
				effect.call("hunt", 0)
			"briar_staff":
				effect.call("grow_lattice")
				effect.call("impale", 1)
			"raven_totem":
				effect.call("mark")
				effect.call("dive", 0)
				effect.call("dive", 1)
...
func _finish(errors: Array[String]) -> void:
	for error in errors:
		push_error("Druid ultimate timeline: %s" % error)
	quit(1)
