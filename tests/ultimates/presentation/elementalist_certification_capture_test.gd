...
## FAN-3943 focused gate for the Elementalist live certification capture package.
##
...
const Capture := preload("res://tests/ultimates/presentation/elementalist_certification_live_capture.gd")

const CLASS_ID := "elementalist"
const PROFILE_PATH := "res://data/ultimates/schema/v1/classes/elementalist.json"
const CLASS_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/elementalist/manifest.json"
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/elementalist/certification_capture_manifest.json"
const POINTER_FIXTURE_PATH := "user://fan3943_elementalist_capture_pointer.png"
...
func declaration_violations(manifest: Dictionary, profile: Dictionary) -> Array[String]:
	...
	if str(manifest.get("issue", "")) != "FAN-3943":
...
func _beat_time_violations(key: String, sample: Dictionary, weapon: Dictionary) -> Array[String]:
	...
	var declared_time := float(timing.get(beat_id, -1.0))
	if declared_time <= 0.0:
		violations.append("%s: the class manifest declares no positive %s beat" % [key, beat_id])
		return violations
	if not is_equal_approx(float(sample.get("declared_beat_seconds", -1.0)), declared_time):
		violations.append("%s: declared beat time %s does not match the class manifest %.3f" % [
...
func _check_class_manifest_registration(class_manifest: Dictionary, manifest: Dictionary, errors: Array[String]) -> void:
	...
	if live.is_empty():
		...
		return
	if str(live.get("issue", "")) != "FAN-3943":
...
func _check_negative_probes(manifest: Dictionary, profile: Dictionary, class_manifest: Dictionary, errors: Array[String]) -> void:
	...
	((wrong_scene.get("samples", []) as Array)[0] as Dictionary)["presentation_scene"] = "res://scenes/vfx/ultimates/elementalist/NotShipped.tscn"
	...
	_expect(png_violations("res://docs/design/reference-assets-lfs/ultimate-certification/elementalist/absent.png", Vector2i(8, 8)).has("missing"), "a missing PNG must fail closed", errors)
...
func _finish(errors: Array[String]) -> void:
	if errors.is_empty():
		print("FAN-3943 Elementalist certification capture package: PASS")
		quit(0)
		return
	for error in errors:
		push_error("FAN-3943 Elementalist certification capture package: %s" % error)
	quit(1)
