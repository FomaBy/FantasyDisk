...
## FAN-3943 live certification capture for the three canonical Elementalist weapon
## ultimates.
##
...
## `Player` configured for `elementalist/<weapon>`, and shipped `Enemy` instances
...
##       --fixed-fps 60 \
##       --script res://tests/ultimates/presentation/elementalist_certification_live_capture.gd
##
## `ELEMENTALIST_CERT_SOURCE_REF`, `ELEMENTALIST_CERT_SOURCE_SHA` and
## `ELEMENTALIST_CERT_SOURCE_TREE` pin the checkout the captures were taken from. They
...
##
## `ELEMENTALIST_CERT_FRAME_DIR` optionally names a directory that receives every
...
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/elementalist/certification_capture_manifest.json"
const OUTPUT_DIR := "res://docs/design/reference-assets-lfs/ultimate-certification/elementalist"
...
const WEAPON_IDS: Array[String] = ["elementalist_orb_ring", "elementalist_prism_focus", "elementalist_meteor_core"]
const BEAT_IDS: Array[String] = ["release", "active", "recovery"]
const REQUIRED_NODES_BY_WEAPON := {
	"elementalist_orb_ring": ["Conclave"],
	"elementalist_prism_focus": ["PrismLattice"],
	"elementalist_meteor_core": ["Starfall"],
}
...
## switch every Elementalist driver reads in `_ready()`, and `combat_feedback` is the
...
const CAPTURE_SEED := 394320260909
...
func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-3943 Elementalist certification capture skipped (headless); run windowed for PNG evidence.")
		quit(0)
		return
	seed(CAPTURE_SEED)
	_source = {
		"ref": OS.get_environment("ELEMENTALIST_CERT_SOURCE_REF").strip_edges(),
		"commit_sha": OS.get_environment("ELEMENTALIST_CERT_SOURCE_SHA").strip_edges().to_lower(),
		"tree_sha": OS.get_environment("ELEMENTALIST_CERT_SOURCE_TREE").strip_edges().to_lower(),
	}
	for field in ["ref", "commit_sha", "tree_sha"]:
		if str(_source[field]).is_empty():
			push_error("FAN-3943 Elementalist certification capture: ELEMENTALIST_CERT_SOURCE_%s must name the checkout the captures come from" % field.to_upper())
			quit(1)
			return
	_frame_dir = OS.get_environment("ELEMENTALIST_CERT_FRAME_DIR").strip_edges()
	if not _frame_dir.is_empty():
		DirAccess.make_dir_recursive_absolute(_frame_dir)
	var manifest := _load_json(MANIFEST_PATH)
	if manifest.is_empty():
		push_error("FAN-3943 Elementalist certification capture: %s is missing or invalid" % MANIFEST_PATH)
		quit(1)
		return
	...
	if output_error != OK and output_error != ERR_ALREADY_EXISTS:
		push_error("FAN-3943 Elementalist certification capture: cannot create %s (%s)" % [OUTPUT_DIR, error_string(output_error)])
		quit(1)
		return
		...
		for weapon_id in WEAPON_IDS:
			for raw_mode in MODES:
				...
				if not failure.is_empty():
					push_error("FAN-3943 Elementalist certification capture: %s" % failure)
					quit(1)
					return
		var matrix_error := await _write_matrix_sheet(viewport)
		if matrix_error != OK:
			push_error("FAN-3943 Elementalist certification capture: matrix sheet %s failed (%s)" % [viewport["id"], error_string(matrix_error)])
			quit(1)
			return

	var write_error := _write_capture_manifest()
	if write_error != OK:
		push_error("FAN-3943 Elementalist certification capture: manifest write failed (%s)" % error_string(write_error))
		quit(1)
		return
	print("FAN-3943 Elementalist certification capture: %d samples, %d sheets written to %s" % [
...
func _capture_combination(viewport: Dictionary, weapon_id: String, mode: Dictionary) -> String:
	...
	## own settings, and before the ultimate spawns: the Elementalist driver reads
	...
	for beat in beats:
		...
		if not _frame_dir.is_empty():
			frame.save_png("%s/elementalist_%s_%s_%s_%s.png" % [
...
func _write_matrix_sheet(viewport: Dictionary) -> int:
	...
	_add_heading(host, size, "ELEMENTALIST ULTIMATES — LIVE %s CAPTURE • ACTIVE BEAT • REAL PLAYER, HAZARDS AND HUD" % str(viewport["id"]).to_upper())
	...
	var path := "%s/elementalist_certification_%s.png" % [OUTPUT_DIR, str(viewport["id"])]
...
func _render_sheet(host: Node2D, size: Vector2i, path: String) -> int:
	...
	if result == OK:
		print("FAN-3943 Elementalist certification capture saved: %s" % path)
	return result
...
func _write_capture_manifest() -> int:
	var payload := {
		...
		"capture": {
			...
			"capture_script": "tests/ultimates/presentation/elementalist_certification_live_capture.gd",
			"focused_test": "tests/ultimates/presentation/elementalist_certification_capture_test.gd",
			"seed": CAPTURE_SEED,
			"fixed_fps": 60,
			"godot_version": "%s.%s" % [
			...
			"beat_source": "docs/design/references/weapon_ultimates/elementalist/manifest.json#weapons[].timing_seconds",
			"captured_at": Time.get_datetime_string_from_system(true, true),
		},
		"commands": {
			"live_capture": "ELEMENTALIST_CERT_SOURCE_REF=<ref> ELEMENTALIST_CERT_SOURCE_SHA=<sha> ELEMENTALIST_CERT_SOURCE_TREE=<tree> python3 tools/godot_gate.py --path . --windowed --fixed-fps 60 --script res://tests/ultimates/presentation/elementalist_certification_live_capture.gd",
			"focused_test": "python3 tools/godot_gate.py --headless --path . --script res://tests/ultimates/presentation/elementalist_certification_capture_test.gd",
			"class_timelines": "python3 tools/godot_gate.py --headless --path . --script res://tests/ultimates/presentation/elementalist_ultimate_timelines.gd",
