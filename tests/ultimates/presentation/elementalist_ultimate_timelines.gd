...
func _initialize() -> void:
	...
	if not errors.is_empty():
		_finish(errors)
		return
	_expect(str(manifest.get("class_id", "")) == "elementalist", "manifest must be class-local to Elementalist", errors)
	...
	_expect(packages.size() == 3, "manifest must contain exactly three Elementalist weapon packages", errors)
...
func _check_scene(weapon_id: String, package: Dictionary, errors: Array[String]) -> void:
	...
	_expect(str(instance.get_meta("ultimate_id", "")) == "elementalist/%s" % weapon_id, "%s scene must retain its exact profile key" % weapon_id, errors)
...
func _finish(errors: Array[String]) -> void:
	for error in errors:
		push_error("Elementalist ultimate timeline: %s" % error)
	quit(1)
