...
## Snapshot every formation sprite the scene currently owns. `overlay_layer`
## (WrenchSigil) is owned by `_apply_v2_overlay()`, not the combat formation,
## and must not satisfy a resume-motion assertion while the formation itself
## is frozen.
func _scene_pose(scene: Node2D) -> String:
	var parts: Array[String] = []
	for child in scene.get_children():
		var sprite := child as Sprite2D
		if sprite == null or bool(sprite.get_meta("overlay_layer", false)):
			continue
