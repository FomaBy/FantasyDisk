...
const WEAPON_IDS := ["engineer_sentry_wrench", "engineer_repair_drone", "engineer_pressure_mines"]
const SCENES := {
...
## The three accepted FAN-3702 packs share one animation identity
## (`ultimate_flipbook`); each weapon binds its own SpriteFrames pack, so
## distinctness is carried by the pack path, never the animation name.
const FLIPBOOK_ANIMATION := &"ultimate_flipbook"
const FLIPBOOKS := {
	"engineer_sentry_wrench": {
		"node": "SentryNest",
		"path": "res://assets/sprites/effects/engineer/sentry_wrench/sentry_wrench_spriteframes.tres",
		"times": [0.0, 0.45, 0.8, 1.15, 1.6, 2.05, 2.55, 3.1, 3.45],
		"beats": [
			"weapon_ultimate.executor.engineer.engineer_sentry_wrench.volley_hits:0",
			"weapon_ultimate.executor.engineer.engineer_sentry_wrench.volley_hits:1",
		],
	},
	"engineer_repair_drone": {
		"node": "DroneSwarm",
		"path": "res://assets/sprites/effects/engineer/repair_drone/repair_drone_spriteframes.tres",
		"times": [0.0, 0.35, 0.7, 1.2, 1.75, 2.3, 2.85, 3.35, 3.75],
		"beats": [
			"weapon_ultimate.executor.engineer.engineer_repair_drone.ram:0",
			"weapon_ultimate.executor.engineer.engineer_repair_drone.ram:1",
		],
	},
	"engineer_pressure_mines": {
		"node": "MineField",
		"path": "res://assets/sprites/effects/engineer/pressure_mines/pressure_mines_spriteframes.tres",
		"times": [0.0, 0.45, 0.9, 1.3, 1.7, 2.1, 2.55, 3.1, 3.35],
		"beats": [
			"weapon_ultimate.executor.engineer.engineer_pressure_mines.smart",
			"weapon_ultimate.executor.engineer.engineer_pressure_mines.finale",
		],
	},
}
const PACKS := [
	{"weapon_id": "engineer_sentry_wrench", "scene": SCENES["engineer_sentry_wrench"], "time": 2.2, "position": Vector2(0.18, 0.55), "title": "SENTRY WRENCH — SENTRY NEST", "color": Color(0.55, 0.95, 0.85), "required_nodes": ["SentryNest"]},
	{"weapon_id": "engineer_repair_drone", "scene": SCENES["engineer_repair_drone"], "time": 2.3, "position": Vector2(0.50, 0.55), "title": "REPAIR DRONE — HELIX SWARM", "color": Color(1.0, 0.82, 0.45), "required_nodes": ["DroneSwarm"]},
	{"weapon_id": "engineer_pressure_mines", "scene": SCENES["engineer_pressure_mines"], "time": 1.85, "position": Vector2(0.82, 0.55), "title": "PRESSURE MINES — MINE LATTICE", "color": Color(0.95, 0.62, 0.4), "required_nodes": ["MineField"]},
]
const CAPTURES := [
	{"name": "648p", "file": "engineer_ultimate_timelines_648p.png", "size": Vector2i(1152, 648)},
	{"name": "720p", "file": "engineer_ultimate_timelines_720p.png", "size": Vector2i(1280, 720)},
	{"name": "1080p", "file": "engineer_ultimate_timelines_1080p.png", "size": Vector2i(1920, 1080)},
	{"name": "2k", "file": "engineer_ultimate_timelines_2k.png", "size": Vector2i(2560, 1440)},
]
...
const SHEET_TITLE := "ENGINEER WEAPON ULTIMATES — DISTINCT PRESENTATION TIMELINES"
...
func _initialize() -> void:
	...
	if not errors.is_empty():
		_finish(errors)
		return
	_expect(str(manifest.get("class_id", "")) == "engineer", "manifest must be class-local to Engineer", errors)
	...
	_expect(packages.size() == WEAPON_IDS.size(), "manifest must contain exactly three Engineer packages", errors)
	...
	if not errors.is_empty():
		_finish(errors)
		return
	print("Engineer ultimate timelines passed (three exact flipbooks, frozen phases, per-victim beat impacts, lifecycle, and crowd budgets).")
	quit(0)
...
func _check_provenance(manifest: Dictionary, errors: Array[String]) -> void:
	...
	_expect(assets == WEAPON_IDS, "provenance must list exactly the three admitted Engineer packs", errors)
	_expect(str(provenance.get("integrated_art_candidate", "")) == "8a763a5d469d5944744ab4964a889e44adc8e971", "provenance must pin the accepted art candidate", errors)
...
func _check_scene(weapon_id: String, package: Dictionary, errors: Array[String]) -> void:
	...
	_expect(str(instance.get_meta("ultimate_id", "")) == "engineer/%s" % weapon_id, "%s scene must retain its exact profile key" % weapon_id, errors)
...
func _check_scene_flipbook(weapon_id: String, package: Dictionary, instance: Node2D, timeline: AnimationPlayer, errors: Array[String]) -> void:
	...
	_expect(sprite.animation == FLIPBOOK_ANIMATION, "%s must bind the admitted animation identity" % weapon_id, errors)
	if frames != null:
		_expect(frames.get_frame_count(FLIPBOOK_ANIMATION) == 9, "%s flipbook must expose all nine frames" % weapon_id, errors)
		_expect(not frames.get_animation_loop(FLIPBOOK_ANIMATION), "%s flipbook must not loop through a seam" % weapon_id, errors)
		var texture_paths := {}
		for frame in 9:
			var texture := frames.get_frame_texture(FLIPBOOK_ANIMATION, frame)
	...
	_expect(StringName(str(flipbook.get("animation", ""))) == FLIPBOOK_ANIMATION, "%s manifest must pin the admitted animation identity" % weapon_id, errors)
...
## The authored scene is the Engineer trio's live visual channel: every
...
func _check_v2_package(weapon_id: String, package: Dictionary, errors: Array[String]) -> void:
	...
	_expect(str(identity.get("cast_pose_id", "")).begins_with("cast_pose.engineer."),
		"%s must declare an Engineer cast pose" % weapon_id, errors)
	...
	_expect(str(identity.get("class_palette_id", "")).begins_with("class_palette.engineer."),
		"%s must declare an Engineer palette" % weapon_id, errors)
...
func _check_distinction(packages: Dictionary, errors: Array[String]) -> void:
	for field in ["silhouette", "motion_path", "impact_language", "timing_seconds"]:
		...
		_expect(values.size() == WEAPON_IDS.size() and not values.has(""), "all Engineer weapons must have unique %s" % field, errors)
	var flipbook_paths := {}
	for weapon_id in WEAPON_IDS:
		flipbook_paths[str((FLIPBOOKS[weapon_id] as Dictionary)["path"])] = true
	_expect(flipbook_paths.size() == WEAPON_IDS.size(), "all Engineer weapons must bind distinct flipbook packs", errors)
...
func _finish(errors: Array[String]) -> void:
	for error in errors:
		push_error("Engineer ultimate timeline: %s" % error)
	quit(1)
