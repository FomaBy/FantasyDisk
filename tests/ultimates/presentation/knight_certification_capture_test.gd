...
## FAN-3943 focused gate for the Knight live certification capture package.
##
...
const Capture := preload("res://tests/ultimates/presentation/knight_certification_live_capture.gd")

const CLASS_ID := "knight"
const PROFILE_PATH := "res://data/ultimates/schema/v1/classes/knight.json"
const CLASS_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/knight/manifest.json"
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/knight/certification_capture_manifest.json"
const POINTER_FIXTURE_PATH := "user://fan3943_knight_capture_pointer.png"
...
func _check_negative_probes(manifest: Dictionary, profile: Dictionary, class_manifest: Dictionary, errors: Array[String]) -> void:
	...
	((wrong_scene.get("samples", []) as Array)[0] as Dictionary)["presentation_scene"] = "res://scenes/vfx/ultimates/knight/NotShipped.tscn"
	...
	_expect(png_violations("res://docs/design/reference-assets-lfs/ultimate-certification/knight/absent.png", Vector2i(8, 8)).has("missing"), "a missing PNG must fail closed", errors)
...
func _finish(errors: Array[String]) -> void:
	if errors.is_empty():
		print("FAN-3943 Knight certification capture package: PASS")
		quit(0)
		return
	for error in errors:
		push_error("FAN-3943 Knight certification capture package: %s" % error)
	quit(1)
