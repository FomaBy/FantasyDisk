...
## Headless integrity and live-composition gate for FAN-3944's windowed Priest
...
const LEGACY_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/priest/manifest.json"
const CAPTURE_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/priest/certification_capture_manifest.json"
const LIVE_CAPTURE_SCRIPT := "tests/ultimates/presentation/priest_certification_live_capture.gd"
...
const WEAPON_IDS: Array[String] = ["priest_reliquary", "priest_censer", "priest_chime"]
...
const CAPTURES := [
	{"id": "648p-release", "viewport_id": "648p", "phase": "release", "path": CAPTURE_ROOT + "/priest_certification_648p_release.png", "size": Vector2i(1152, 648)},
	{"id": "648p-active", "viewport_id": "648p", "phase": "active", "path": CAPTURE_ROOT + "/priest_certification_648p.png", "size": Vector2i(1152, 648)},
	{"id": "648p-recovery", "viewport_id": "648p", "phase": "recovery", "path": CAPTURE_ROOT + "/priest_certification_648p_recovery.png", "size": Vector2i(1152, 648)},
	{"id": "720p-release", "viewport_id": "720p", "phase": "release", "path": CAPTURE_ROOT + "/priest_certification_720p_release.png", "size": Vector2i(1280, 720)},
	{"id": "720p-active", "viewport_id": "720p", "phase": "active", "path": CAPTURE_ROOT + "/priest_certification_720p.png", "size": Vector2i(1280, 720)},
	{"id": "720p-recovery", "viewport_id": "720p", "phase": "recovery", "path": CAPTURE_ROOT + "/priest_certification_720p_recovery.png", "size": Vector2i(1280, 720)},
	{"id": "1080p-release", "viewport_id": "1080p", "phase": "release", "path": CAPTURE_ROOT + "/priest_certification_1080p_release.png", "size": Vector2i(1920, 1080)},
	{"id": "1080p-active", "viewport_id": "1080p", "phase": "active", "path": CAPTURE_ROOT + "/priest_certification_1080p.png", "size": Vector2i(1920, 1080)},
	{"id": "1080p-recovery", "viewport_id": "1080p", "phase": "recovery", "path": CAPTURE_ROOT + "/priest_certification_1080p_recovery.png", "size": Vector2i(1920, 1080)},
	{"id": "2k-release", "viewport_id": "2k", "phase": "release", "path": CAPTURE_ROOT + "/priest_certification_2k_release.png", "size": Vector2i(2560, 1440)},
	{"id": "2k-active", "viewport_id": "2k", "phase": "active", "path": CAPTURE_ROOT + "/priest_certification_2k.png", "size": Vector2i(2560, 1440)},
	{"id": "2k-recovery", "viewport_id": "2k", "phase": "recovery", "path": CAPTURE_ROOT + "/priest_certification_2k_recovery.png", "size": Vector2i(2560, 1440)},
]
...
const PACKS := [
	{
		"weapon_id": "priest_reliquary",
		"label": "RELIQUARY",
		"scene_path": "scenes/vfx/ultimates/priest/PriestReliquarySanctumJudgment.tscn",
		"scene": preload("res://scenes/vfx/ultimates/priest/PriestReliquarySanctumJudgment.tscn"),
		"crowd_cap": 22,
		"color": Color(1.0, 0.82, 0.38),
		"beats": {"release": 0.90, "active": 1.35, "recovery": 3.45},
		"required_nodes": ["BackdropVeil", "Shrine", "RingJudgment", "Pillar"],
	},
	{
		"weapon_id": "priest_censer",
		"label": "CENSER",
		"scene_path": "scenes/vfx/ultimates/priest/PriestCenserUnbreakableVow.tscn",
		"scene": preload("res://scenes/vfx/ultimates/priest/PriestCenserUnbreakableVow.tscn"),
		"crowd_cap": 20,
		"color": Color(0.92, 0.72, 0.40),
		"beats": {"release": 0.80, "active": 1.40, "recovery": 3.15},
		"required_nodes": ["BackdropVeil", "Orbit", "WardSmoke", "SlamWave"],
	},
	{
		"weapon_id": "priest_chime",
		"label": "CHIME",
		"scene_path": "scenes/vfx/ultimates/priest/PriestChimeThreeBellsOfDawn.tscn",
		"scene": preload("res://scenes/vfx/ultimates/priest/PriestChimeThreeBellsOfDawn.tscn"),
		"crowd_cap": 18,
		"color": Color(0.88, 0.94, 1.0),
		"beats": {"release": 0.75, "active": 1.50, "recovery": 2.85},
		"required_nodes": ["BackdropVeil", "Bell", "TollSilver", "DawnGuard"],
	},
]
...
func _check_legacy_handoff(legacy: Dictionary, errors: Array[String]) -> void:
	_expect(str(legacy.get("class_id", "")) == "priest", "legacy Priest manifest must stay class-local", errors)
	var evidence := legacy.get("evidence", {}) as Dictionary
	var certification := evidence.get("certification_capture", {}) as Dictionary
	_expect(
		str(certification.get("manifest", "")) == "docs/design/references/weapon_ultimates/priest/certification_capture_manifest.json",
	...
	_expect(
		str(certification.get("focused_test", "")) == "tests/ultimates/presentation/priest_certification_capture_test.gd",
...
func _check_negative_probes(errors: Array[String]) -> void:
	...
	var pointer_path := "user://fan3944_priest_lfs_pointer.png"
...
func _check_timeline_hold(scene: Node2D, expected_seconds: float, context: String, errors: Array[String]) -> void:
	...
	_expect(is_equal_approx(timeline.current_animation_position, expected_seconds * timeline.speed_scale),
...
func _check_real_runtime_cell(pack: Dictionary, mode: Dictionary, phase: String, errors: Array[String]) -> void:
	...
	world.name = "PriestCertificationRuntimeFixture"
	...
	player.call("configure_character", "priest", str(pack["weapon_id"]))
	await process_frame
	...
	if scene != null:
		...
		if phase != "release":
			_check_real_victim_impact(scene, enemies, str(pack["weapon_id"]), context, errors)
	PlayerHost.reset(player)
	world.queue_free()
	current_scene = null
	await process_frame
...
func _check_real_victim_impact(scene: Node2D, enemies: Array[Node2D], weapon_id: String, context: String, errors: Array[String]) -> void:
	var impacts: ImpactPlayer = null
	var world := scene.get_parent()
	for child in world.find_children("*", "Node", true, false) if world != null else []:
		if child is ImpactPlayer:
			impacts = child as ImpactPlayer
			break
	## Priest's frozen executors own their victim-impact players rather than the
	## presentation scene. If an active impact node is still retained, validate
	## it; the class timeline gate separately proves every canonical executor
	## creates the shared impact player on a real hit.
	if impacts != null:
	...
	if weapon_id != "priest_censer":
		_expect(damaged > 0, "%s must apply the real executor path to actual capture enemies" % context, errors)


func _check_real_hud(world: Node2D, player: Node2D, weapon_id: String, context: String, errors: Array[String]) -> void:
	var hud_root := Control.new()
	hud_root.name = "PriestCertificationHudFixture"
	...
	_expect(str(selection.get("class_id", "")) == "priest" and str(selection.get("weapon_id", "")) == weapon_id,
...
static func manifest_violations(manifest: Dictionary, profile: Dictionary) -> Array[String]:
	...
	if str(manifest.get("class_id", "")) != "priest":
		errors.append("class_id must be priest")
	if str(manifest.get("capture_script", "")) != LIVE_CAPTURE_SCRIPT:
		errors.append("capture_script must name the windowed Priest renderer")
	if str(manifest.get("focused_test", "")) != "tests/ultimates/presentation/priest_certification_capture_test.gd":
	...
	if profile_weapons != WEAPON_IDS:
		errors.append("Priest profile must still expose %s, got %s" % [WEAPON_IDS, profile_weapons])
	...
	if _string_array(configuration.get("scene_paths")) != _pack_scene_paths():
		errors.append("runtime_configuration.scene_paths must enumerate the shipped Priest trio")
...
static func seek_scene(scene: Node2D, seconds: float) -> void:
	...
	timeline.seek(seconds * timeline.speed_scale, true)
	timeline.pause()
...
func _finish(errors: Array[String]) -> void:
	if errors.is_empty():
		print("Priest certification capture gate: 4 viewports x 3 weapons x 4 modes x release/active/recovery verified")
		quit(0)
		return
	for error in errors:
		push_error("Priest certification capture gate: %s" % error)
	quit(1)
