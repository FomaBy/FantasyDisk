...
## Windowed runtime renderer for FAN-3944's Priest certification package.
##
...
const Spec := preload("res://tests/ultimates/presentation/priest_certification_capture_test.gd")
...
func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		...
		print("FAN-3944 Priest certification capture skipped: headless runs never create certification PNG evidence.")
		quit(0)
		return
	var registry = Registry.new(PD.WEAPONS_BY_CLASS)
	if not registry.is_valid():
		push_error("FAN-3944 Priest certification capture cannot start: weapon registry is invalid")
		quit(1)
		return
	if PlayerScene == null or EnemySpitterScene == null:
		push_error("FAN-3944 Priest certification capture cannot load the shipped Player/Enemy runtime scenes")
		quit(1)
		return
	...
	for raw_capture in Spec.CAPTURES:
		...
		if result != OK:
			push_error("FAN-3944 Priest certification capture failed: %s" % error_string(result))
			quit(1)
			return
...
func _capture_sheet(_registry, capture: Dictionary) -> int:
	...
	for weapon_index in Spec.PACKS.size():
		for mode_index in Spec.MODES.size():
			...
			if bool(viewport.get_meta("fan3944_capture_failed", false)):
				...
				viewport.queue_free()
				await process_frame
				push_error("FAN-3944 Priest certification live cell failed: %s" % reason)
	...
	if result == OK:
		print("FAN-3944 Priest certification capture saved: %s (%dx%d, %s)" % [output, size.x, size.y, phase])
	return result
...
func _arena_viewport(arena_size: Vector2i, weapon_index: int, mode_index: int, phase: String) -> SubViewport:
	...
	world.name = "PriestCertificationWorld"
...
func _attach_shipped_hud(viewport: SubViewport, player: Node2D, arena_size: Vector2i) -> void:
	...
	hud_root.name = "PriestCertificationHudRoot"
...
func _suppress_standard_weapon_feedback(viewport: SubViewport) -> void:
	...
	for raw_child in viewport.get_children():
		...
		if child == null or child is ColorRect \
				or child.name == &"PriestCertificationWorld" or child.name == &"ElitePoisonZone":
			continue
		child.process_mode = Node.PROCESS_MODE_DISABLED
		if child is CanvasItem:
			(child as CanvasItem).visible = false
...
func _read_viewport(viewport: SubViewport, label: String) -> Image:
	...
	if image == null or image.is_empty():
		push_error("FAN-3944 Priest certification capture rendered an empty viewport: %s" % label)
		return null
