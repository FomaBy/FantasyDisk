...
const CAPTURE_SPEC := preload("res://tests/ultimates/presentation/priest_ultimate_timelines.gd")


func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-1484 Priest ultimate contact capture skipped (headless); run windowed for PNGs.")
		quit(0)
		return
	for capture in CAPTURE_SPEC.CAPTURES:
		...
		await process_frame
		if error != OK:
			push_error("Priest ultimate contact capture failed: %s" % error_string(error))
			quit(1)
			return
		print("Priest ultimate contact capture saved: %s" % output)
	quit(0)
