...
const CAPTURE_SOURCE_REF := "dev"
const CAPTURE_SOURCE_COMMIT_SHA := "51450cdee8b010cc80c20c1bf4e9120df166c750"
const CAPTURE_SOURCE_TREE_SHA := "503a1f4144353fbf44444ee55890e984ab54d189"
...
func _check_manifest(errors: Array[String]) -> void:
	...
	_expect(
		str(live.get("source_ref", "")) == CAPTURE_SOURCE_REF,
		"manifest live_capture.source_ref must pin %s" % CAPTURE_SOURCE_REF,
		errors
	)
	_expect(
		str(live.get("source_commit_sha", "")) == CAPTURE_SOURCE_COMMIT_SHA,
		"manifest live_capture.source_commit_sha must pin the capture base %s" % CAPTURE_SOURCE_COMMIT_SHA,
		errors
	)
	_expect(
		str(live.get("source_tree_sha", "")) == CAPTURE_SOURCE_TREE_SHA,
		"manifest live_capture.source_tree_sha must pin the capture base tree %s" % CAPTURE_SOURCE_TREE_SHA,
		errors
	)
	_expect(
...
static func evidence_violations(captures: Array) -> Array[String]:
	...
	for raw_capture in captures:
		...
		for violation in png_ihdr_violations(path, expected):
			errors.append("contact sheet PNG/IHDR invalid: %s (%s)" % [path, violation])
...
## Decode the PNG envelope directly, not only through Image, so an invalid
## header or mismatched IHDR is rejected before the renderer can accept it.
