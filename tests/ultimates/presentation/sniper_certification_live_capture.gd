...
## FAN-3940 live certification capture for the three canonical Sniper weapon
## ultimates.
##
## Every frame comes from a real run: `scenes/Main.tscn` is instantiated,
## `_start_combat()` builds the shipped arena with the shipped combat HUD and a
...
## Sniper timelines declare. All 144 samples are measured at their native
## resolution and written to the capture manifest. The four committed sheets —
## one per viewport, twelve weapon x mode cells at the active beat — are the
## human index of that record.
##
...
##
##     SNIPER_CERT_SOURCE_REF=<ref> SNIPER_CERT_SOURCE_SHA=<sha> \
##     SNIPER_CERT_SOURCE_TREE=<tree> FSD_GODOT_EXCLUSIVE=1 \
...
##
## The three source variables pin the checkout the captures were taken from and
## are required: the manifest must name a commit that already exists, never the
## commit that will later carry the manifest itself. `SNIPER_CERT_FRAME_DIR`
## optionally receives every native full-resolution frame, so a reviewer can
...
const MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/sniper/manifest.json"
...
const OUTPUT_DIR := "res://docs/design/reference-assets-lfs/ultimate-certification/sniper"
...
## sheet per supported viewport, and CI hydrates only the LFS paths a class
## manifest lists under `evidence.contact_sheets`. A fifth sheet would reach the
## gate as an unsmudged pointer, so the package commits four native-size sheets
...
## Fixed hazard geometry. Rings keep the shipped enemies inside every viewport
## and off the cast origin, so the same layout reads at 648p and at 2K.
const HAZARD_RING_RADII := [230.0, 300.0, 370.0]
const HAZARD_SLOTS_PER_RING := 8
const HAZARD_PARKING := Vector2(6000.0, 6000.0)
const HAZARD_HEALTH := 100000.0
const CAPTURE_SEED := 394020260909
...
func _initialize() -> void:
	...
	_source = {
		...
		"tree_sha": OS.get_environment("SNIPER_CERT_SOURCE_TREE").strip_edges().to_lower(),
	}
...
func _capture_combination(viewport: Dictionary, weapon_id: String, mode: Dictionary) -> String:
	...
	var label := "%s/%s/%s" % [weapon_id, mode_id, str(viewport["id"])]
	...
	if root.size != size:
		return await _abandon(main, "%s window is %s, expected %s" % [label, str(root.size), str(size)])

	var player := main.get("current_player") as Node2D
	if player == null:
		return await _abandon(main, "%s produced no live player" % label)
	...
	if hazards.size() < hazard_count:
		return await _abandon(main, "%s placed %d of %d hazards" % [label, hazards.size(), hazard_count])
	...
	if baseline == null or baseline.get_size() != size:
		return await _abandon(main, "%s baseline frame is %s, expected %s" % [
			label, "null" if baseline == null else str(baseline.get_size()), str(size)])
	baseline.convert(Image.FORMAT_RGB8)

	var status := PlayerHost.activate(player)
	if status != PlayerHost.ACTIVATION_STARTED:
		return await _abandon(main, "%s did not start the ultimate (status %d, %s)" % [
			label, status, PlayerHost.activation_failure(player)])
	...
	for beat in beats:
		...
		if frame == null or frame.get_size() != size:
			return await _abandon(main, "%s %s frame is %s, expected %s" % [
				label, str(beat["phase"]),
...
func _abandon(main: Node, failure: String) -> String:
	main.queue_free()
	await process_frame
	return failure
...
func _prepare_hazards(main: Node, player: Node2D, wanted: int) -> Array[Node2D]:
	...
	while live.size() < wanted and guard < wanted * 3:
		var angle := float(guard) * TAU / float(HAZARD_SLOTS_PER_RING)
		var radius := float(HAZARD_RING_RADII[guard % HAZARD_RING_RADII.size()])
		if combat.call("_spawn_random_enemy", main.get("enemy_scene"), player.global_position + Vector2(radius, 0.0).rotated(angle), true, 0.0) == null:
			break
		guard += 1
		live = _live_hazards()
	await process_frame
	...
	for index in live.size():
		...
		if index >= wanted:
			...
			continue
		var ring := index / HAZARD_SLOTS_PER_RING
		var slot := index % HAZARD_SLOTS_PER_RING
		var radius := float(HAZARD_RING_RADII[ring % HAZARD_RING_RADII.size()]) + 26.0 * float(ring / HAZARD_RING_RADII.size())
		var angle := float(slot) * TAU / float(HAZARD_SLOTS_PER_RING) + 0.19625 * float(ring)
...
func _measure(
	...
	## The arena-wide backdrop treatment is the declared `presence.backdrop`, not
	## drawn effect area, so it is measured on its own. What remains is the box
	## the class manifest bounds with `quality.max_viewport_coverage_ratio`.
	var effect_box := _effect_screen_box(presentation, false)
	var backdrop_box := _effect_screen_box(presentation, true)
	...
	for hazard in hazards:
		if not is_instance_valid(hazard):
			continue
		if Rect2(Vector2.ZERO, Vector2(size)).has_point(_frame_point(hazard.get_global_transform_with_canvas().origin)):
			visible_hazards += 1
	var hud_overlap := 0.0
	...
	return {
		...
		"effect_nodes_drawn": _drawn_node_count(presentation),
	}
...
func _effect_screen_box(presentation: Node2D, backdrop_only: bool) -> Rect2:
	if presentation == null:
		return Rect2()
	var box := Rect2()
	var seeded := false
	for node in _drawn_nodes(presentation):
...
func _drawn_nodes(presentation: Node2D) -> Array[CanvasItem]:
	var found: Array[CanvasItem] = []
	if presentation == null:
		return found
	var pending: Array[Node] = [presentation]
	while not pending.is_empty():
		var node: Node = pending.pop_back()
		for child in node.get_children():
			pending.append(child)
		var item := node as CanvasItem
		if item == null or item == presentation:
			continue
		if item.visible and item.modulate.a > 0.01 and item.self_modulate.a > 0.01 and item.is_visible_in_tree():
			found.append(item)
	return found


func _drawn_node_count(presentation: Node2D) -> int:
	return _drawn_nodes(presentation).size()
...
func _screen_rect(item: CanvasItem) -> Rect2:
	...
	if item is Sprite2D:
		...
		var points := (item as Line2D).points
		if points.is_empty():
			return Rect2()
		local = Rect2(points[0], Vector2.ZERO)
		for point in points:
			local = local.expand(point)
		local = local.grow((item as Line2D).width * 0.5)
...
func _presentation_root(main: Node) -> Node2D:
	...
	while not pending.is_empty():
		...
		if script != null and script.resource_path == PRESENTATION_SCRIPT_PATH:
			return node as Node2D
	return null


## Sniper's certified beats live in the shipped weapon timelines, which is what
## the shared contact-sheet contract declares as this class's evidence source.
## The frame-local node each beat must still be drawing comes from the same
## file's visual contract, keyed by the phase folder it sits in.
func _beats_for(weapon_id: String) -> Array:
	var definition := _load_json("%s/%s.timeline.json" % [TIMELINE_DIR, weapon_id])
	var timing := (definition.get("manifest", {}) as Dictionary).get("timing", {}) as Dictionary
	if timing.is_empty():
		return []
	var required := (definition.get("visual_contract", {}) as Dictionary).get("required_nodes", []) as Array
	## A live cast tears its presentation node down at the declared `cancel`, so
	## no sample is taken after that frame. Both times are published: nothing is
	## silently retimed.
	var last_drawn_frame := float(timing.get("cancel", 0.0)) - 2.0 * FIXED_STEP
	var ordered: Array = []
	for beat_id in BEAT_IDS:
		if not timing.has(beat_id):
			return []
		var declared_time := float(timing[beat_id])
		var beat_nodes: Array = []
		for raw_name in required:
			if str(raw_name).begins_with("PhaseNodes/%s/" % beat_id.capitalize()):
				beat_nodes.append(str(raw_name))
		ordered.append({
			"phase": beat_id,
			"required_nodes": beat_nodes,
			"declared_time": declared_time,
			"sample_time": minf(declared_time, last_drawn_frame) if last_drawn_frame > 0.0 else declared_time,
		})
	return ordered if ordered.size() == BEAT_IDS.size() else []


func _crowd_cap(weapon_id: String) -> int:
	var performance := (_weapon_manifest.get(weapon_id, {}) as Dictionary).get("performance", {}) as Dictionary
	return maxi(1, int(performance.get("crowd_cap", 24)))
...
func _store_panel(viewport: Dictionary, weapon_id: String, mode_id: String, beat_id: String, frame: Image) -> void:
	...
	panel.resize(maxi(1, int(cell.size.x)), maxi(1, int(cell.size.y)), Image.INTERPOLATE_LANCZOS)
	_sheet_panels["%s/%s" % [weapon_id, mode_id]] = panel
...
func _write_matrix_sheet(viewport: Dictionary) -> int:
	...
	_add_heading(host, size, "SNIPER ULTIMATES — LIVE %s CAPTURE • ACTIVE BEAT • REAL PLAYER, HAZARDS AND HUD" % str(viewport["id"]).to_upper())
	...
	var path := "%s/sniper_certification_%s.png" % [OUTPUT_DIR, str(viewport["id"])]
...
func _render_sheet(host: Node2D, size: Vector2i, path: String) -> int:
	...
	if result == OK:
		print("%s Sniper certification capture saved: %s" % [ISSUE, path])
	return result
...
func _add_backdrop(host: Node2D, size: Vector2i) -> void:
	...
	background.color = Color(0.024, 0.030, 0.042, 1.0)
...
func _add_heading(host: Node2D, size: Vector2i, text: String) -> void:
	...
	heading.add_theme_color_override("font_color", Color(0.90, 0.93, 1.0))
	heading.z_index = 60
...
func _add_cell(host: Node2D, size: Vector2i, cell: Rect2, panel: Image, caption: String) -> void:
	...
	frame_polygon.color = Color(0.07, 0.09, 0.12, 1.0)
...
func _write_capture_manifest() -> int:
	var payload := {
		...
		"capture": {
			...
			"beat_source": "scenes/vfx/ultimates/sniper/<weapon>.timeline.json, the evidence source contact_sheet_beats_contract.gd declares for sniper",
			"captured_at": Time.get_datetime_string_from_system(true, true),
		},
		"commands": {
			"live_capture": "SNIPER_CERT_SOURCE_REF=<ref> SNIPER_CERT_SOURCE_SHA=<sha> SNIPER_CERT_SOURCE_TREE=<tree> FSD_GODOT_EXCLUSIVE=1 python3 tools/godot_gate.py --path . --windowed --fixed-fps 60 --script res://%s" % CAPTURE_SCRIPT_PATH,
			"focused_test": "python3 tools/godot_gate.py --headless --path . --script res://%s" % CAPTURE_TEST_PATH,
