...
## FAN-3941 — headless gate and shared spec for the Soldier certification
## capture package.
##
## The package proves the three canonical Soldier ultimates in the four
...
## pictures. The renderer lives in soldier_certification_live_capture.gd and
## preloads this file; nothing here renders.

const PD := preload("res://scripts/progression_data.gd")
const Registry := preload("res://scripts/ultimates/registry/weapon_ultimate_registry.gd")
const SoldierSpec := preload("res://tests/ultimates/presentation/soldier_ultimate_timelines.gd")
...
const MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/soldier/certification_capture_manifest.json"
const READABILITY_REPORT := "docs/design/references/weapon_ultimates/soldier/certification_readability_report.md"
const CLASS_MANIFEST_PATH := "res://docs/design/references/weapon_ultimates/soldier/manifest.json"
const PROFILE_PATH := "res://data/ultimates/schema/v1/classes/soldier.json"
const ADOPTION_SHARD_PATH := "res://data/ultimates/classes/soldier/presentation_adoption.json"
...
const CAPTURE_COMMAND := "FSD_GODOT_EXCLUSIVE=1 python3 tools/godot_gate.py --path . --script res://tests/ultimates/presentation/soldier_certification_live_capture.gd"
const TEST_COMMAND := "python3 tools/godot_gate.py --headless --path . --script res://tests/ultimates/presentation/soldier_certification_capture_test.gd"
...
const PLAYER_SPRITE := "res://assets/sprites/characters/full_frame/soldier_pixellab/soldier_idle_south.png"
...
## settings toggle main.gd mirrors onto the tree root. The Soldier scenes
## author no arena-wide surface, so `veil` is recorded for parity with the
## other class packages and there is nothing for the photosensitivity-safe
## variant to drop: it is the shipped scene with motion off, which the gate
## proves by counting full-screen surfaces (zero) and by reading every
## authored alpha track for its rise rate.
const MODES := [
...
const WEAPONS := [
	{
		...
		"scene_path": "scenes/vfx/ultimates/soldier/SoldierRifleSuppressiveOrder.tscn",
		"scene": preload("res://scenes/vfx/ultimates/soldier/SoldierRifleSuppressiveOrder.tscn"),
		"label": "RIFLE",
		"swatch": Color(0.82, 0.96, 0.62),
	},
	{
		"weapon_id": "soldier_grenade",
		"scene_path": "scenes/vfx/ultimates/soldier/SoldierGrenadeSevenSeconds.tscn",
		"scene": preload("res://scenes/vfx/ultimates/soldier/SoldierGrenadeSevenSeconds.tscn"),
		"label": "GRENADE",
		"swatch": Color(1.0, 0.62, 0.24),
	},
	{
		"weapon_id": "soldier_bayonet",
		"scene_path": "scenes/vfx/ultimates/soldier/SoldierBayonetLastCharge.tscn",
		"scene": preload("res://scenes/vfx/ultimates/soldier/SoldierBayonetLastCharge.tscn"),
		"label": "BAYONET",
		"swatch": Color(0.74, 0.86, 1.0),
	},
]
const WEAPON_IDS: Array[String] = ["soldier_rifle", "soldier_grenade", "soldier_bayonet"]
const PHASE_ORDER: Array[String] = ["windup", "release", "active", "recovery", "cancel"]
const TIMELINE_NODE := "Timeline"
const TIMELINE_ANIMATION := &"ultimate"
...
## Ceiling any arena-wide tint would have to respect; Soldier authors none.
const MAX_OVERLAY_ALPHA := 0.35
const LFS_POINTER_PREFIX := "version https://git-lfs.github.com/spec/v1"


## The class manifest is the timing and budget source for Soldier: the
## soldier_ultimate_timelines gate proves it agrees with the scenes.
...
func _check_declaration(errors: Array[String]) -> void:
	...
	_expect(weapon_ids() == WEAPON_IDS, "spec must cover the canonical Soldier trio in order", errors)
...
## The phase a time falls in, by the declared timing.
...
## The declared phase timing is the beat source; the class manifest and the
## scenes agree on it (soldier_ultimate_timelines.gd).
...
static func identity_violations(manifest: Dictionary, profile: Dictionary) -> Array[String]:
	...
	if str(manifest.get("class_id", "")) != CLASS_ID:
		errors.append("manifest must stay Soldier-local")
...
## The class shard is empty: Soldier claims no exemption from any gate.
static func adoption_violations() -> Array[String]:
	...
	if str(shard.get("class_id", "")) != CLASS_ID:
		errors.append("adoption shard must be Soldier-local")
...
static func timeline_player(scene: Node2D) -> AnimationPlayer:
	return scene.get_node_or_null(TIMELINE_NODE) as AnimationPlayer
...
## Drives the authored timeline to `seconds` and holds it there: the Soldier
## scenes are AnimationPlayer timelines, so a seek with update is exact and a
## pause keeps the position while the frame is drawn.
static func seek_scene(scene: Node2D, _registry, seconds: float) -> void:
	var timeline := timeline_player(scene)
	if timeline == null:
		return
	timeline.stop()
	timeline.play(TIMELINE_ANIMATION)
	timeline.seek(seconds, true)
	timeline.pause()


## The Soldier scenes author no arena-wide surface, so the photosensitivity-safe
...
## Drawn bounds of the authored effect in viewport pixels, through the same
## visibility and geometry rules the Soldier timelines gate applies.
static func content_bounds(scene: Node2D) -> Rect2:
	return SoldierSpec.transformed_capture_bounds(SoldierSpec.capture_content_bounds(scene), scene)
...
static func formation_signature(scene: Node2D) -> String:
	...
	for child in scene.get_children():
		...
		if node == null:
			continue
		parts.append("%s:%.3f:%.3f:%.3f:%.3f:%.3f:%s" % [
			str(node.name), node.position.x, node.position.y, node.scale.x, node.rotation, node.modulate.a, str(node.visible),
		])
	return "|".join(parts)
...
func _check_live_composition(registry, errors: Array[String]) -> void:
	for weapon_id in weapon_ids():
		for beat_id in BEAT_IDS:
			...
			var phase := phase_at(weapon_id, seconds)
			...
			var held := timeline_player(repeat)
			_expect(held != null and not held.is_playing(), "%s seek must leave the timeline paused at its beat" % weapon_id, errors)
			repeat.free()
	reset_mode(self)


## No arena-wide surface exists anywhere in the cast, and no authored element
## strobes: every `modulate:a` track of the shipped timeline is read for the
## rate at which it rises through half its own peak inside the animation
## length, against the WCAG 2.3.1 general threshold the shared contract owns.
func _check_photosensitivity(errors: Array[String]) -> void:
	for weapon_id in weapon_ids():
		...
		_expect(int(report["tracks"]) > 0, "%s must author alpha tracks to measure" % weapon_id, errors)
		_expect(float(report["max_element_rise_hz"]) <= Contract.MAX_FLASH_HZ, "%s %s alpha rises at %.2f Hz over %.1f Hz" % [weapon_id, str(report["fastest_node"]), float(report["max_element_rise_hz"]), Contract.MAX_FLASH_HZ], errors)


## Full-screen surfaces (always zero for Soldier) and the fastest alpha rise
## rate of any authored node over the animation length, read from the shipped
## animation tracks themselves.
static func backdrop_report(weapon_id: String) -> Dictionary:
	...
	var timeline := scene.get_node_or_null(TIMELINE_NODE) as AnimationPlayer
	var flicker := {}
	var tracks := 0
	var max_hz := 0.0
	var fastest := ""
	var length := 0.0
	if timeline != null and timeline.has_animation(TIMELINE_ANIMATION):
		var animation := timeline.get_animation(TIMELINE_ANIMATION)
		length = animation.length
		for track in animation.get_track_count():
			var path := animation.track_get_path(track)
			if str(path.get_concatenated_subnames()) != "modulate:a":
				continue
			tracks += 1
			...
			while elapsed <= length:
				var value := float(animation.value_track_interpolate(track, elapsed))
			...
			for value in samples:
				if value > trigger and not above:
					...
					above = false
			var hz := float(rises) / maxf(length, 0.0001)
			var node := str(path.get_name(path.get_name_count() - 1))
			flicker[node] = {"rises": rises, "hz": snappedf(hz, 0.01), "peak_alpha": snappedf(peak, 0.01)}
			if hz > max_hz:
				max_hz = hz
				fastest = node
	scene.free()
	return {
		...
		"max_element_rise_hz": snappedf(max_hz, 0.01),
		"fastest_node": fastest,
		"element_flicker": flicker,
	}
...
func _check_negatives(manifest: Dictionary, class_manifest: Dictionary, profile: Dictionary, errors: Array[String]) -> void:
	...
	var pointer_path := "user://fan3941_soldier_pointer_probe.png"
...
func _finish(errors: Array[String]) -> void:
	if errors.is_empty():
		print("Soldier certification capture gate: %d frames (%d weapons x %d modes x %d viewports at the active beat, plus release/recovery at %s) verified with readability probes, coverage measurement, quality declaration, empty adoption shard and negatives." % [
			expected_entries().size(), WEAPONS.size(), MODES.size(), VIEWPORTS.size(), BEAT_VIEWPORT,
		])
		quit(0)
		return
	for error in errors:
		push_error("Soldier certification capture gate: %s" % error)
	quit(1)
