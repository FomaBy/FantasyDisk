...
## FAN-3941 — windowed certification capture for the Soldier ultimate trio.
##
...
## shipped scene with its authored AnimationPlayer timeline sought to the beat
## and held, at the on-screen scale the game uses (2560x1440 logical canvas,
...
## weapon's own victim-impact flipbook on the crowd through the scene's own
## present() route. The Soldier scenes author no arena-wide surface, so the
## photosensitivity-safe variant is the shipped scene with motion off and the
## gate proves the surface count is zero.
##
...
##     --script res://tests/ultimates/presentation/soldier_certification_live_capture.gd

const SPEC := preload("res://tests/ultimates/presentation/soldier_certification_capture_test.gd")
...
func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-3941 Soldier certification capture skipped (headless); run windowed for evidence.")
		quit(0)
		return
	seed(SPEC.CAPTURE_SEED)
	_registry = Registry.new(PD.WEAPONS_BY_CLASS)
	if not _registry.is_valid():
		push_error("Soldier certification capture: weapon registry is invalid")
		quit(1)
		return
	...
	if made != OK:
		push_error("Soldier certification capture: cannot create %s (%s)" % [directory, error_string(made)])
		quit(1)
		return
	...
	for entry in SPEC.expected_entries():
		...
		print("Soldier certification capture saved: %s (%dx%d, coverage %.4f)" % [
	...
	for weapon_id in SPEC.weapon_ids():
		...
		print("Soldier envelope sweep %s: peak coverage %.4f at %.2fs over %d samples" % [
	...
	if file == null:
		push_error("Soldier certification capture: cannot write %s" % SPEC.MANIFEST_PATH)
		quit(1)
		return
	file.store_string(JSON.stringify(manifest, "  ", false) + "\n")
	file.close()
	print("Soldier certification capture manifest written: %s (%d frames)" % [SPEC.MANIFEST_PATH, captures.size()])
	quit(0)


func _fail() -> void:
	for error in _errors:
		push_error("Soldier certification capture: %s" % error)
	SPEC.reset_mode(self)
	quit(1)
...
func _capture(entry: Dictionary) -> Dictionary:
	...
	if not crowd_nodes.is_empty():
		_play_impacts(scene, crowd_nodes, size)
	...
	return {
		...
		"phase": str(SPEC.phase_at(weapon_id, seconds).get("name", "")),
		"width": size.x,
		"height": size.y,
		"path": path,
...
## from a scene-only render every SWEEP_STEP_SECONDS with the authored
## timeline sought to each sample time.
func _envelope_sweep(weapon_id: String) -> Dictionary:
	var size := SPEC.viewport_size(SPEC.BEAT_VIEWPORT)
	var duration := SPEC.timeline_seconds(weapon_id)
	...
	while elapsed < duration:
		SPEC.seek_scene(scene, _registry, elapsed)
		await process_frame
		await RenderingServer.frame_post_draw
...
func _manifest(captures: Array[Dictionary], weapons: Array[Dictionary]) -> Dictionary:
	...
	return {
		...
		"source": {
			...
			"note": "Integrated origin/dev revision whose shipped Soldier scenes and assets were rendered; the capture tooling, frames and this manifest are added by FAN-3941 on top of it and change no production scene.",
		},
		"engine": {
		...
		"capture": {
			...
			"seek_method": "AnimationPlayer.play(ultimate) + seek(beat, true) + pause()",
...
## The weapon's own victim-impact flipbook, routed through the scene's
## present() exactly as the executor routes live victims, advanced into its
## first burst and held. The bursts are top-level sprites, so they take the
## on-screen scale explicitly.
func _play_impacts(scene: Node2D, victims: Array[Node2D], size: Vector2i) -> void:
	scene.call("present", "%s.certification" % SPEC.CLASS_ID, {"victims": victims})
	for child in scene.get_children():
		if child.get_script() != ImpactPlayer:
			continue
		child.extra_hit_flash = false
		child.advance(IMPACT_ADVANCE_SECONDS)
		child.set_paused(true)
		for grandchild in child.get_children():
			var burst := grandchild as AnimatedSprite2D
			if burst != null:
				burst.scale *= SPEC.on_screen_scale(size)
		return
	_errors.append("%s present() routed no victim through the impact player" % scene.name)
