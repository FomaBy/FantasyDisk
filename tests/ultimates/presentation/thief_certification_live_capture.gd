...
## FAN-3941 — windowed certification capture for the Thief ultimate trio.
##
...
## weapon's own victim-impact flipbook on the crowd. The Thief scenes
## author no arena-wide surface, so the photosensitivity-safe variant is the
## shipped scene with motion off and the gate proves the surface count is zero.
##
...
func _initialize() -> void:
	if DisplayServer.get_name() == "headless":
		print("FAN-3941 Thief certification capture skipped (headless); run windowed for evidence.")
		quit(0)
		return
	seed(SPEC.CAPTURE_SEED)
	_registry = Registry.new(PD.WEAPONS_BY_CLASS)
	if not _registry.is_valid():
		push_error("Thief certification capture: weapon registry is invalid")
		quit(1)
		return
	...
	if made != OK:
		push_error("Thief certification capture: cannot create %s (%s)" % [directory, error_string(made)])
		quit(1)
		return
	...
	for entry in SPEC.expected_entries():
		...
		print("Thief certification capture saved: %s (%dx%d, coverage %.4f)" % [
	...
	for weapon_id in SPEC.weapon_ids():
		...
		print("Thief envelope sweep %s: peak coverage %.4f at %.2fs over %d samples" % [
	...
	if file == null:
		push_error("Thief certification capture: cannot write %s" % SPEC.MANIFEST_PATH)
		quit(1)
		return
	file.store_string(JSON.stringify(manifest, "  ", false) + "\n")
	file.close()
	print("Thief certification capture manifest written: %s (%d frames)" % [SPEC.MANIFEST_PATH, captures.size()])
	quit(0)


func _fail() -> void:
	for error in _errors:
		push_error("Thief certification capture: %s" % error)
	SPEC.reset_mode(self)
	quit(1)
...
func _manifest(captures: Array[Dictionary], weapons: Array[Dictionary]) -> Dictionary:
	...
	return {
		...
		"source": {
			...
			"note": "Integrated origin/dev revision whose shipped Thief scenes, pack and assets were rendered; the capture tooling, frames and this manifest are added by FAN-3941 on top of it and change no production scene.",
		},
		"engine": {
