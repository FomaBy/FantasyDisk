...
func _live_ownership_errors(
	...
	var errors: Array[String] = []
	if spawned_count <= 0 and presentation_count <= 0 and tween_count <= 0:
		errors.append(
			"%s must own a live effect channel (spawn, presentation, or tracked tween)" % label
		)
	return errors
...
func _assert_contract_falsifications() -> void:
	_check(
		not _live_ownership_errors("falsification/ownerless", 0, 0, 0).is_empty(),
		"ownerless activation must fail closed"
	)
	_check(
		_live_ownership_errors("falsification/spawn", 1, 0, 0).is_empty(),
		"spawn ownership must satisfy the live-effect contract"
	)
	_check(
		_live_ownership_errors("falsification/presentation", 0, 1, 0).is_empty(),
		"presentation ownership must satisfy the live-effect contract"
	)
	_check(
		_live_ownership_errors("falsification/tween", 0, 0, 1).is_empty(),
		"tracked-tween ownership must satisfy the live-effect contract"
	)
...
func _start_case(state: Dictionary) -> void:
	...
	# choreography packages (guitarist) own only their tracked tween plus
	# presentation handles that appear mid-run. All channels count; owning
	# none of them is the failure.
	for error in _live_ownership_errors(
		label,
		(state["spawned"] as Array).size(),
		(activation.presentation_for_tests() as Array).size(),
		(state["tweens"] as Array).size()
	):
		_check(false, error)
...
func _wait_for_natural_completion(states: Array[Dictionary]) -> void:
	...
	while Time.get_ticks_msec() <= global_deadline:
		...
		# on the same wall clock the activation tweens run on (track_tween sets
		# ignore_time_scale). Without this tick no declared-duration status could
		# ever expire, and a designed self-expiring control would read as a leak.
		var wall_delta := float(now - last_tick_ms) / 1000.0
		last_tick_ms = now
		for state in states:
			for enemy in state["enemies"]:
				StatusEffects.tick(enemy, wall_delta)
		for state in states:
