...
class RuntimeCollector extends RefCounted:
	...
	func on_damage(_target: Node2D, _attempted: float, applied: float, feedback: Dictionary) -> void:
		if applied <= 0.0:
			return
		var mechanic := str(feedback.get("ultimate_mechanic", ""))
		if mechanic == "":
...
func _measure_class(class_id: String) -> Dictionary:
	...
	for scenario_value in Pack.PLAYABLE_SCENARIOS:
		...
		print("FAN-1515 %s %s activations=%d ultimate_damage=%.2f" % [class_id, scenario_id, int((result[scenario_id] as Dictionary).get("activation_count", 0)), float(((result[scenario_id] as Dictionary).get("ultimate_source", {}) as Dictionary).get("damage", 0.0))])
	return result
...
func _measure_arm(class_id: String, scenario_id: String) -> Dictionary:
	...
	return {
		...
		"ultimate_source": {"damage": snappedf(collector.ultimate_damage, 0.0001), "hits": collector.ultimate_hits, "by_mechanic": collector.by_mechanic},
...
func _finish() -> void:
	if _errors.is_empty():
		print("FAN-1515 ultimate/Atlas attribution probe passed.")
		quit(0)
		return
