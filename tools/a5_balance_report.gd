...
const FINAL_EXECUTION_CENSER_SPACING := 24.0
const FINAL_EXECUTION_CENSER_PRESSURE_INTERVAL_FRAMES := 120
...
func _run_final_execution_probe() -> void:
	...
	for row_value in rows:
		...
		if bool(row.get("observed", false)):
			observed += 1
		print("FAN-2224 %-45s %-36s %-16s %-24s %-18s activations=%d events=%d dispatches=%d hp=%.2f trace=%s deaths=%d markers=%d owner=%s weapon=%s" % [
			...
			float(payoff.get("applied_hp_total", 0.0)), str((row.get("telemetry", {}) as Dictionary).get("trace_digest_sha256", "")), int(payoff.get("target_deaths", 0)),
...
func _spawn_final_execution_target(profile: Dictionary, index: int, collector: A5TelemetryCollector, witness: A5FinalExecutionWitness, player: Node2D) -> Node2D:
	var position := final_execution_target_position(profile, index)
...
# The censer's production counter chooses a nearest target and later applies a
# capped ward pulse. Keep every rank distinct while retaining the ten-target
# production fixture.
static func final_execution_target_position(profile: Dictionary, index: int) -> Vector2:
	...
	var layout := str(profile.get("layout", "pack"))
	if layout == "staggered_pack":
		return position + Vector2(float(index) * FINAL_EXECUTION_CENSER_SPACING, float(index * index))
	if layout == "ring":
...
func _advance_final_execution(seconds: float, dummies: Array, anchors: Array, collector: A5TelemetryCollector, witness: A5FinalExecutionWitness, probe_phase: String, profile: Dictionary, player: Node2D) -> Dictionary:
	...
	var stimulus := str(profile.get("stimulus", "autofire"))
	var pressure_interval := final_execution_incoming_interval(profile)
	var elapsed := 0.0
	var frames := 0
	while elapsed < seconds and frames < FINAL_EXECUTION_MAX_FRAMES:
		...
		elif stimulus == "incoming_pressure" and frames % pressure_interval == 0:
...
# The Censer consumer has a real-clock cooldown. Its fixture must not present
# another hit near that threshold, because a fixed-frame simulation may run at
# slightly different wall-clock speed on a busy machine.
static func final_execution_incoming_interval(profile: Dictionary) -> int:
	return FINAL_EXECUTION_CENSER_PRESSURE_INTERVAL_FRAMES if str(profile.get("layout", "")) == "staggered_pack" else FINAL_EXECUTION_INCOMING_INTERVAL_FRAMES
...
static func final_execution_profile(expected: Dictionary) -> Dictionary:
	...
	return {
		...
		"layout": "staggered_pack" if mechanic_id == "censer_absorb_retaliation" else ("ring" if FINAL_EXECUTION_RING_MODES.has(str(expected.get("mode", ""))) else "pack"),
