#!/usr/bin/env python3
"""FAN-2594 — 8-direction review sheets for the runtime hero biologist body pack.

Renders the acceptance evidence the card asks for, straight from the runtime
PNGs the game plays (assets/sprites/characters/full_frame/biologist_pixellab):

  - fan2594_biologist_8dir_contact.png  8 direction rows x (idle + move 00..05)
  - fan2594_biologist_row_<dir>.png     2x zoom of every row for the per-frame
                                         anatomy/gear/alpha review.

Deterministic and read-only; nothing is auto-fixed.

Usage: python3 tools/build_fan2594_biologist_animation_review.py
...
PACK = ROOT / "assets/sprites/characters/full_frame/biologist_pixellab"
...
# bottom-aligned with 16 px padding (footline y=496) and centred on x=256.
CROP = (170, 245, 345, 500)
BACKGROUND = (30, 30, 36, 255)


def row_frames(direction: str) -> list[str]:
    return [f"biologist_idle_{direction}.png"] + [
        f"biologist_move_{direction}_{index:02d}.png" for index in range(6)
    ]
