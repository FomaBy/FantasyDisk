#!/usr/bin/env python3
"""FAN-2596 — 8-direction review sheets for the runtime hero dark_mage body pack.

Renders the acceptance evidence the card asks for, straight from the runtime
PNGs the game plays (assets/sprites/characters/full_frame/dark_mage_pixellab):

  - fan2596_dark_mage_8dir_contact.png  8 direction rows x (idle + move 00..05)
  - fan2596_dark_mage_row_<dir>.png     2x zoom of every direction row

Deterministic and read-only; nothing is auto-fixed.

Usage: python3 tools/build_fan2596_dark_mage_animation_review.py
...
PACK = ROOT / "assets/sprites/characters/full_frame/dark_mage_pixellab"
...
# Tight window on the 512x512 runtime canvas: visible art is 246 px tall,
# bottom-aligned with 32 px padding (alpha bboxes y 234..479, x 141..370).
CROP = (136, 229, 376, 485)
BACKGROUND = (30, 30, 36, 255)


def row_frames(direction: str) -> list[str]:
    return [f"dark_mage_idle_{direction}.png"] + [
        f"dark_mage_move_{direction}_{index:02d}.png" for index in range(6)
    ]
