#!/usr/bin/env python3
"""Build the FAN-2612 stone_bruiser 8-direction PixelLab pack.

Parses a saved plain-text `get_character()` dump from the PixelLab MCP
server (same format tools/build_fan2628_mini_rot_hound_pack.py consumes),
...
CHARACTER_ID = "stone_bruiser"
CELL_SIZE = 512
TARGET_VISIBLE_HEIGHT = 245
BOTTOM_PADDING = 32
USER_AGENT = "FantasyDisk-FAN2612/1.0"

SOURCE_DIR = ROOT / "assets/sprites/enemies/pixellab/stone_bruiser"
RUNTIME_DIR = ROOT / "assets/sprites/enemies/full_frame/stone_bruiser_8dir"
SPRITEFRAMES_PATH = ROOT / "assets/sprites/enemies/full_frame/stone_bruiser_spriteframes.tres"
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: build_fan2612_stone_bruiser_pack.py <dump.txt>", file=sys.stderr)
        return 2
    ...
    for direction in DIRECTIONS:
        s = suffix(direction)
        src = SOURCE_DIR / f"stone_bruiser_idle_{s}.png"
        dest = RUNTIME_DIR / f"stone_bruiser_idle_{s}.png"
    ...
    for state, timing in STATE_TIMING.items():
        ...
        for direction in DIRECTIONS:
            ...
            for index, url in enumerate(urls):
                src = SOURCE_DIR / f"stone_bruiser_{state}_{s}_{index:02d}.png"
                dest = RUNTIME_DIR / f"stone_bruiser_{state}_{s}_{index:02d}.png"
                download(url, src)
    ...
    manifest = {
        ...
        "pixellab_character_id": "acc9d3d5-8c44-448f-be81-6583bb91b2c0",
        ...
        "states": {
            ...
            "move": {"frame_count": 6, "template_animation_id": "walking-6-frames", "fps": 9.0, "action_description": "lumbering forward slowly, heavy stone footsteps"},
            "attack": {"frame_count": 6, "mode": "v3", "action_description": "heavy overhead smash with both boulder fists", "fps": 10.0},
            "hit": {"frame_count": 6, "mode": "v3", "action_description": "recoiling backward from a heavy blow, cracks flashing across the stone body", "fps": 10.0},
            "death": {"frame_count": 8, "mode": "v3", "action_description": "crumbling and collapsing into a pile of rubble", "fps": 10.0},
        },
        "notes": (
            "FAN-2612: dedicated 8-direction pack replacing the single authored "
            "horizontal view + flip for standard_monster/stone_bruiser. "
            "8 idle rotations + 8x6 move frames + 8x6 attack frames + 8x6 hit "
            "frames + 8x8 death frames, all normalized to 245px visible height "
