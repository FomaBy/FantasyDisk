#!/usr/bin/env python3
"""Build the FAN-2615 venom_spitter 8-direction PixelLab pack.
...
CHARACTER_ID = "venom_spitter"
CELL_SIZE = 512
TARGET_VISIBLE_HEIGHT = 245
BOTTOM_PADDING = 32
USER_AGENT = "FantasyDisk-FAN2615/1.0"

SOURCE_DIR = ROOT / "assets/sprites/enemies/pixellab/venom_spitter"
RUNTIME_DIR = ROOT / "assets/sprites/enemies/full_frame/venom_spitter_8dir"
SPRITEFRAMES_PATH = ROOT / "assets/sprites/enemies/full_frame/venom_spitter_spriteframes.tres"

# State header names in the dump are prefixed with the character id
# (venom_spitter_move, venom_spitter_attack, ...); map them to the
# fleet-standard state keys used by STATE_TIMING / the registry contract.
STATE_HEADER_PREFIX = "venom_spitter_"
...
def parse_dump(text: str) -> tuple[dict[str, str], dict[str, dict[str, list[str]]]]:
    ...
    for line in text.splitlines():
        ...
        if header:
            ...
            current_state = raw_name[len(STATE_HEADER_PREFIX):] if raw_name.startswith(STATE_HEADER_PREFIX) else raw_name
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: build_fan2615_venom_spitter_pack.py <dump.txt>", file=sys.stderr)
        return 2
    ...
    for direction in DIRECTIONS:
        s = suffix(direction)
        src = SOURCE_DIR / f"venom_spitter_idle_{s}.png"
        dest = RUNTIME_DIR / f"venom_spitter_idle_{s}.png"
    ...
    for state, timing in STATE_TIMING.items():
        ...
        for direction in DIRECTIONS:
            ...
            for index, url in enumerate(urls):
                src = SOURCE_DIR / f"venom_spitter_{state}_{s}_{index:02d}.png"
                dest = RUNTIME_DIR / f"venom_spitter_{state}_{s}_{index:02d}.png"
                download(url, src)
    ...
    manifest = {
        ...
        "pixellab_character_id": "fda221e7-d8b0-40a2-9f4c-a86e790036ea",
        ...
        "states": {
            ...
            "move": {"frame_count": 6, "template_animation_id": "walk-6-frames", "fps": 9.0, "action_description": "creeping forward low to the ground on stubby legs, tail dragging"},
            "attack": {"frame_count": 6, "mode": "v3", "keep_first_frame": False, "action_description": "lunging forward and spitting a burst of venom from its wide jaws, head snapping back then thrusting out", "fps": 12.0},
            "hit": {"frame_count": 4, "mode": "v3", "keep_first_frame": False, "action_description": "flinching backward in pain from a hit, recoiling with a jolt, body tensing", "fps": 10.0},
            "death": {"frame_count": 6, "mode": "v3", "keep_first_frame": False, "action_description": "slumping down flat and motionless, body sinking low to the ground, head drooping down, defeated", "fps": 10.0},
        },
        "notes": (
            "FAN-2615: dedicated 8-direction pack replacing the single authored "
            "horizontal view + flip for standard_monster/venom_spitter. "
