#!/usr/bin/env python3
"""Build the FAN-2617 small_biter 8-direction PixelLab pack.

Parses a saved plain-text `get_character()` dump from the PixelLab MCP
server (the format the tool emits when a response is too large to inline),
...
tools/build_fan2628_mini_rot_hound_pack.py / tools/update_pixellab_character_animations.py),
...
CHARACTER_ID = "small_biter"
CELL_SIZE = 512
TARGET_VISIBLE_HEIGHT = 245
BOTTOM_PADDING = 32
USER_AGENT = "FantasyDisk-FAN2617/1.0"

SOURCE_DIR = ROOT / "assets/sprites/enemies/pixellab/small_biter"
RUNTIME_DIR = ROOT / "assets/sprites/enemies/full_frame/small_biter"
SPRITEFRAMES_PATH = ROOT / "assets/sprites/enemies/full_frame/small_biter_spriteframes.tres"
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: build_fan2617_small_biter_pack.py <dump.txt>", file=sys.stderr)
        return 2
    ...
    for direction in DIRECTIONS:
        s = suffix(direction)
        src = SOURCE_DIR / f"small_biter_idle_{s}.png"
        dest = RUNTIME_DIR / f"small_biter_idle_{s}.png"
    ...
    for state, timing in STATE_TIMING.items():
        ...
        for direction in DIRECTIONS:
            ...
            # v3 custom animations default to keep_first_frame=true, which
            # prepends the static rotation pose as frame 0. Drop it so the
            # stored row matches the fleet's contracted frame count (6 for
            # attack/death, 4 for hit), same as the bone_caller/ash_marksman
            # precedent (tests/animation_smoke_test.gd hardcodes those counts).
            if state != "move" and len(urls) > 1:
                urls = urls[1:]
            rids = []
            reports = []
            for index, url in enumerate(urls):
                src = SOURCE_DIR / f"small_biter_{state}_{s}_{index:02d}.png"
                dest = RUNTIME_DIR / f"small_biter_{state}_{s}_{index:02d}.png"
                download(url, src)
    ...
    manifest = {
        ...
        "pixellab_character_id": "82c57fa2-24d0-4b48-8857-589738999078",
        "generated_at": "2026-08-19",
        "source": "PixelLab MCP",
        "mode": "standard (idle rotations) + template walk-6-frames (move) + v3 custom (attack/hit/death)",
        "body_type": "quadruped (dog template)",
        "view": "low top-down",
        ...
        "states": {
            ...
            "move": {"frame_count": 6, "template_animation_id": "walk-6-frames", "fps": 9.0},
            "attack": {"frame_count": 6, "mode": "v3", "action_description": "lunging forward and snapping jaws in a vicious bite attack, quick aggressive lunge", "fps": 12.0},
            "hit": {"frame_count": 4, "mode": "v3", "action_description": "flinching backward in pain from a hit, recoiling with bones rattling", "fps": 10.0},
            "death": {"frame_count": 6, "mode": "v3", "action_description": "collapsing and dying, bones crumbling apart and falling still to the ground", "fps": 10.0},
        },
        "notes": (
            "FAN-2617: replaces the single-facing small_biter pack (one authored "
            "horizontal view + flip) with a full FAN-2519 explicit-eight-direction "
            "animated pack for standard_monster/small_biter. 8 idle rotations + "
            "8x6 move/attack/death frames + 8x4 hit frames, all normalized to "
            "245px visible height on a transparent 512x512 canvas, footline "
            "pinned at bottom_padding=32 (fleet standard_monster convention, "
            "matching bone_caller/ash_marksman). No flying/hover behavior: "
            "small_biter is a ground-crawling quadruped, not a winged actor."
        ),
    }
