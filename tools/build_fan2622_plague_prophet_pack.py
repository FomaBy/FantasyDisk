#!/usr/bin/env python3
"""Build the FAN-2622 plague_prophet 8-direction pack from tracked PNG input.

Run without arguments from the repository root. The source PNGs under
``assets/sprites/elites/pixellab/plague_prophet`` are the immutable, reviewed
input; the builder never calls PixelLab or the network. It normalizes those
frames into the fleet-standard 512x512 transparent runtime canvas (245px
visible height, 32px bottom padding), then writes the SpriteFrames resource,
manifest, and alpha report.
"""
...
# Maps a runtime state name to the tracked source blocks it draws frames from,
# in order. Most states are 1:1; skill_poison_volley merges two.
STATE_SOURCE_BLOCKS = {
    "move": ["move"],
...
# The reviewed source pack already omits the five duplicated shoot_00..04
# frames. Keeping the original source indices makes that decision auditable.
...
def tracked_source(block: str, direction: str, index: int | None = None) -> Path:
    suffix_name = suffix(direction)
    stem = f"plague_prophet_{block}_{suffix_name}"
    if index is not None:
        stem += f"_{index:02d}"
    return SOURCE_DIR / f"{stem}.png"


def validate_tracked_inputs() -> None:
    expected: set[str] = set()
    for direction in DIRECTIONS:
        expected.add(tracked_source("idle", direction).name)
        for block, indices in SOURCE_FRAME_INDICES.items():
            expected.update(tracked_source(block, direction, index).name for index in indices)
    ...
    if missing or extra:
        details = []
        if missing:
            details.append(f"missing tracked inputs: {missing}")
        if extra:
            details.append(f"unexpected tracked inputs: {extra}")
...
def main() -> int:
    ...
    for direction in DIRECTIONS:
        s = suffix(direction)
        src = tracked_source("idle", direction)
    ...
    # each state's tracked source block(s) (see STATE_SOURCE_BLOCKS) in order.
    for state, source_blocks in STATE_SOURCE_BLOCKS.items():
        ...
        for direction in DIRECTIONS:
            ...
            for block in source_blocks:
                for source_index in SOURCE_FRAME_INDICES[block]:
                    src = tracked_source(block, direction, source_index)
