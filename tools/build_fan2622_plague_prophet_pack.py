...
def main() -> int:
    ...
    manifest = {
        ...
        "states": {
            ...
            "hit": {"frame_count": 4, "mode": "v3", "action_description": "flinching backward in pain from a hit, robe rippling, recoiling", "fps": 10.0},
            "death": {"frame_count": 6, "mode": "v3", "action_description": "collapsing and dying, robe crumpling to the ground, staff falling from hand, body slumping", "fps": 10.0},
        },
        "notes": (
            "FAN-2622: replaces the single authored horizontal view + flip with a "
            "dedicated 8-direction pack. 8 idle rotations + 8x8 move + 8x8 "
            "skill_poison_volley + 8x4 hit + 8x6 death frames, all normalized to "
