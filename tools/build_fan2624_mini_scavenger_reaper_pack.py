#!/usr/bin/env python3
"""Build the FAN-2624 mini_scavenger_reaper 8-direction PixelLab pack.

Same pipeline as tools/build_fan2628_mini_rot_hound_pack.py: parses a saved
`get_character()` dump from the PixelLab MCP server, downloads every
rotation/animation frame it references, normalizes them into the
fleet-standard 512x512 transparent runtime canvas (245px visible height,
...
CHARACTER_ID = "mini_scavenger_reaper"
PIXELLAB_CHARACTER_ID = "3d2c3c13-ed23-4784-add8-2e446a567a8b"
CELL_SIZE = 512
TARGET_VISIBLE_HEIGHT = 245
BOTTOM_PADDING = 32
USER_AGENT = "FantasyDisk-FAN2624/1.0"

SOURCE_DIR = ROOT / "assets/sprites/elites/pixellab/mini_scavenger_reaper"
RUNTIME_DIR = ROOT / "assets/sprites/elites/full_frame/mini_scavenger_reaper"
SPRITEFRAMES_PATH = ROOT / "assets/sprites/elites/full_frame/mini_scavenger_reaper_spriteframes.tres"
...
STATE_TIMING = {
    ...
    "skill_reaping_dash": {"loop": False, "speed": 12.0},
    "skill_bleed_finish": {"loop": False, "speed": 12.0},
}
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: build_fan2624_mini_scavenger_reaper_pack.py <dump.txt>", file=sys.stderr)
        return 2
    ...
    for direction in DIRECTIONS:
        s = suffix(direction)
        src = SOURCE_DIR / f"mini_scavenger_reaper_idle_{s}.png"
        dest = RUNTIME_DIR / f"mini_scavenger_reaper_idle_{s}.png"
    ...
    # move/attack/hit/death/skill_reaping_dash/skill_bleed_finish: multi-frame per direction
    for state, timing in STATE_TIMING.items():
        ...
        for direction in DIRECTIONS:
            ...
            for index, url in enumerate(urls):
                src = SOURCE_DIR / f"mini_scavenger_reaper_{state}_{s}_{index:02d}.png"
                dest = RUNTIME_DIR / f"mini_scavenger_reaper_{state}_{s}_{index:02d}.png"
                download(url, src)
        ...
        if state in ("attack", "skill_reaping_dash", "skill_bleed_finish"):
            alias = "attack_primary" if state == "attack" else "attack_%s" % state.removeprefix("skill_")
            for direction in DIRECTIONS:
                s = suffix(direction)
                base_name = f"{state}_{s}"
                animation_blocks.append(frame_block(
                    [resources[str(RUNTIME_DIR / f"mini_scavenger_reaper_{state}_{s}_{i:02d}.png")] for i in range(len(animations[state][direction]))],
                    timing["speed"], timing["loop"], f"{alias}_{s}",
                ))
    ...
    manifest = {
        ...
        "source": "PixelLab MCP",
        "mode": "v3 (reference-image identity match, idle rotations) + template walk-6-frames (move) + v3 custom (attack/hit/death/skill_reaping_dash/skill_bleed_finish)",
        "body_type": "humanoid",
        "view": "low top-down",
        ...
        "states": {
            ...
            "attack": {"mode": "v3", "action_description": "reaper scythe-claw slashing attack, quick lunging strike", "fps": 12.0, "alias": "attack_primary"},
            "hit": {"mode": "v3", "action_description": "flinching backward in pain from a hit, recoiling", "fps": 10.0},
            "death": {"mode": "v3", "action_description": "collapsing and dying, crumbling to the ground", "fps": 10.0},
            "skill_reaping_dash": {"mode": "v3", "action_description": "fast arcing dash attack, scything through the air", "fps": 12.0, "alias": "attack_reaping_dash"},
            "skill_bleed_finish": {"mode": "v3", "action_description": "finishing blow on a wounded target, downward reaping strike", "fps": 12.0, "alias": "attack_bleed_finish"},
        },
        "notes": (
            "FAN-2624 explicit-eight-direction pack for mini_scavenger_reaper, "
            "generated from the existing runtime south sprite as v3 identity reference. "
            "8 idle rotations + per-state x8-direction move/attack/hit/death/skill frames, "
            "all normalized to 245px visible height on a transparent 512x512 canvas, "
            "footline pinned at bottom_padding=32."
        ),
    }
