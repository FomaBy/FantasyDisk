#!/usr/bin/env python3
"""Build the FAN-2628 mini_rot_hound 8-direction PixelLab pack.
...
CHARACTER_ID = "mini_rot_hound"
CELL_SIZE = 512
TARGET_VISIBLE_HEIGHT = 245
BOTTOM_PADDING = 32
USER_AGENT = "FantasyDisk-FAN2628/1.0"

SOURCE_DIR = ROOT / "assets/sprites/elites/pixellab/mini_rot_hound"
RUNTIME_DIR = ROOT / "assets/sprites/elites/full_frame/mini_rot_hound"
SPRITEFRAMES_PATH = ROOT / "assets/sprites/elites/full_frame/mini_rot_hound_spriteframes.tres"
...
STATE_TIMING = {
    ...
    "skill_shadow_strike": {"loop": False, "speed": 12.0},
}
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: build_fan2628_mini_rot_hound_pack.py <dump.txt>", file=sys.stderr)
        return 2
    ...
    for direction in DIRECTIONS:
        s = suffix(direction)
        src = SOURCE_DIR / f"mini_rot_hound_idle_{s}.png"
        dest = RUNTIME_DIR / f"mini_rot_hound_idle_{s}.png"
    ...
    # move/attack/hit/death/skill_shadow_strike: multi-frame per direction
    for state, timing in STATE_TIMING.items():
        ...
        for direction in DIRECTIONS:
            ...
            for index, url in enumerate(urls):
                src = SOURCE_DIR / f"mini_rot_hound_{state}_{s}_{index:02d}.png"
                dest = RUNTIME_DIR / f"mini_rot_hound_{state}_{s}_{index:02d}.png"
                download(url, src)
    ...
    manifest = {
        ...
        "pixellab_character_id": "73b7080a-741b-4f80-8699-28b0674149ee",
        "generated_at": "2026-08-17",
        "source": "PixelLab MCP",
        "mode": "standard (idle) + template walk-6-frames (move) + v3 custom (attack/hit/death/skill_shadow_strike)",
        ...
        "states": {
            ...
            "attack": {"frame_count": 6, "mode": "v3", "action_description": "lunging bite attack, jaws snapping forward aggressively", "fps": 12.0},
            ...
            "death": {"frame_count": 6, "mode": "v3", "action_description": "collapsing and dying, crumbling and falling apart to the ground, rotting body giving out", "fps": 10.0},
            "skill_shadow_strike": {"frame_count": 6, "mode": "v3", "action_description": "explosive shadow pounce lunge attack, dashing forward with a dark trailing energy, teeth bared for a vicious strike", "fps": 12.0, "note": "runtime alias for the shared night_stalker elite behavior attack_id=shadow_strike"},
        },
        "notes": (
            "First FAN-2519 explicit-eight-direction pack for a mini-elite. "
            "8 idle rotations + 8x6 move/attack/death frames + 8x4 hit frames + "
            "8x6 skill_shadow_strike frames, all normalized to 245px visible height "
