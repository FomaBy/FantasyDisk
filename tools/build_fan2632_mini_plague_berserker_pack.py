#!/usr/bin/env python3
"""Build the FAN-2632 mini_plague_berserker 8-direction PixelLab pack.
...
tools/update_pixellab_character_animations.py), and writes the FAN-2519
explicit-eight-direction SpriteFrames resource plus a manifest/alpha report.

Adapted from tools/build_fan2628_mini_rot_hound_pack.py (FAN-2519/FAN-2628).
"""
...
CHARACTER_ID = "mini_plague_berserker"
PIXELLAB_CHARACTER_ID = "9e63a280-524e-492b-9624-8ddbb2744c07"
CELL_SIZE = 512
TARGET_VISIBLE_HEIGHT = 245
BOTTOM_PADDING = 32
USER_AGENT = "FantasyDisk-FAN2632/1.0"

SOURCE_DIR = ROOT / "assets/sprites/elites/pixellab/mini_plague_berserker"
RUNTIME_DIR = ROOT / "assets/sprites/elites/full_frame/mini_plague_berserker"
SPRITEFRAMES_PATH = ROOT / "assets/sprites/elites/full_frame/mini_plague_berserker_spriteframes.tres"
...
STATE_TIMING = {
    ...
    "skill_poison_volley": {"loop": False, "speed": 12.0},
}
...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: build_fan2632_mini_plague_berserker_pack.py <dump.txt>", file=sys.stderr)
        return 2
    ...
    for direction in DIRECTIONS:
        s = suffix(direction)
        src = SOURCE_DIR / f"mini_plague_berserker_idle_{s}.png"
        dest = RUNTIME_DIR / f"mini_plague_berserker_idle_{s}.png"
    ...
    # move/attack/hit/death/skill_poison_volley: multi-frame per direction
    for state, timing in STATE_TIMING.items():
        ...
        for direction in DIRECTIONS:
            ...
            for index, url in enumerate(urls):
                src = SOURCE_DIR / f"mini_plague_berserker_{state}_{s}_{index:02d}.png"
                dest = RUNTIME_DIR / f"mini_plague_berserker_{state}_{s}_{index:02d}.png"
                download(url, src)
    ...
    manifest = {
        ...
        "source": "PixelLab MCP",
        "mode": "standard (idle) + template walking-6-frames (move) + v3 custom (attack/hit/death/skill_poison_volley)",
        "body_type": "humanoid",
        "view": "low top-down",
        ...
        "states": {
            ...
            "attack": {"frame_count": 6, "mode": "v3", "action_description": "aggressive clawing lunge attack, arms swiping forward violently", "fps": 12.0},
            ...
            "death": {"frame_count": 6, "mode": "v3", "action_description": "collapsing and dying, bursting with poisonous fumes, plague-ridden body giving out", "fps": 10.0},
            "skill_poison_volley": {
                "frame_count": 6,
                "mode": "v3",
                "action_description": "lobbing poison vials overhead in an aggressive throwing motion, poison volley cast",
                "fps": 12.0,
                "note": "runtime alias for the shared plague_prophet elite behavior attack_id=poison_volley",
            },
        },
        "notes": (
            "FAN-2632: dedicated explicit-eight-direction pack for mini_plague_berserker, "
            "replacing the plague_prophet fallback_of reuse. 8 idle rotations + 8x6 "
            "move/attack/death/skill_poison_volley frames + 8x4 hit frames, all normalized "
