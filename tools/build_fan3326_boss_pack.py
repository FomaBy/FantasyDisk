...
def download_source(actor_id: str, state: str, report_group: dict[str, Any], target_frames: int, source_kind: str, bearer: str, call_id: int, log=print) -> tuple[dict[str, list[Path]], int]:
    ...
    for direction in DIRECTIONS:
        urls = report_group["directions"].get(direction, [])
        if source_kind == "object":
            urls = expand_urls(urls, target_frames)
...
def rebuild_manifest(path: Path) -> bool:
    ...
    for state, spec in manifest["states"].items():
        rows[state] = {}
        for direction in DIRECTIONS:
            entries = [entry for entry in manifest["frames"] if entry["state"] == state and entry["direction"] == direction]
