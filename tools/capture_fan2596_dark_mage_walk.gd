...
## FAN-2596 — live captures of the runtime hero dark_mage walk loops.
...
## Run: Godot --path . --script res://tools/capture_fan2596_dark_mage_walk.gd -- --size 1280x720
## Output: build/qa/fan2596_dark_mage/dark_mage_walk_<W>x<H>_page<n>.png

const FRAMES_PATH := "res://assets/sprites/characters/dark_mage_spriteframes.tres"
const OUT_DIR := "res://build/qa/fan2596_dark_mage"
const DIRECTIONS := [
...
# Visible art occupies y ~234..479 of the 512 px runtime canvas; lift the sprite
# so that band, not the padded canvas, is centred in its cell.
const ART_CENTRE_OFFSET := 100.0
...
func _initialize() -> void:
	...
	if frames == null:
		push_error("capture_fan2596_dark_mage_walk: cannot load %s" % FRAMES_PATH)
		quit(1)
		return
	if DisplayServer.get_name() == "headless":
		push_error("capture_fan2596_dark_mage_walk: needs a real renderer, run without --headless")
		quit(1)
		return
...
func _capture_page(frames: SpriteFrames, page: int, size: Vector2i, rows_per_page: int) -> bool:
	...
	var path := "%s/dark_mage_walk_%dx%d_page%d.png" % [
		OUT_DIR, size.x, size.y, page + 1]
	var error := viewport.get_texture().get_image().save_png(ProjectSettings.globalize_path(path))
	viewport.queue_free()
	if error != OK:
		push_error("capture_fan2596_dark_mage_walk: save failed (%d) for %s" % [error, path])
		return false
	print("captured: ", path)
	return true
