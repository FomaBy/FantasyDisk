...
## FAN-2597 — live captures of the runtime hero doctor walk loops.
...
## Run: Godot --path . --script res://tools/capture_fan2597_doctor_walk.gd -- --size 1280x720
## Output: build/qa/fan2597_doctor/doctor_walk_<W>x<H>_page<n>.png

const FRAMES_PATH := "res://assets/sprites/characters/doctor_spriteframes.tres"
const OUT_DIR := "res://build/qa/fan2597_doctor"
const DIRECTIONS := [
...
# Visible art occupies y 236..479 of the 512 px runtime canvas; lift the sprite
...
func _initialize() -> void:
	...
	if frames == null:
		push_error("capture_fan2597_doctor_walk: cannot load %s" % FRAMES_PATH)
		quit(1)
		return
	if DisplayServer.get_name() == "headless":
		push_error("capture_fan2597_doctor_walk: needs a real renderer, run without --headless")
		quit(1)
		return
