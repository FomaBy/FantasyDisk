...
## FAN-2598 — live captures of the runtime hero druid walk loops.
...
## Run: Godot --path . --script res://tools/capture_fan2598_druid_walk.gd -- --size 1280x720
## Output: build/qa/fan2598_druid/druid_walk_<W>x<H>_page<n>.png

const FRAMES_PATH := "res://assets/sprites/characters/druid_spriteframes.tres"
const OUT_DIR := "res://build/qa/fan2598_druid"
const DIRECTIONS := [
...
# Visible art occupies y ~243..487 of the 512 px runtime canvas; lift the sprite
# so that band, not the padded canvas, is centred in its cell.
const ART_CENTRE_OFFSET := 102.0
...
func _initialize() -> void:
	...
	if frames == null:
		push_error("capture_fan2598_druid_walk: cannot load %s" % FRAMES_PATH)
		quit(1)
		return
	if DisplayServer.get_name() == "headless":
		push_error("capture_fan2598_druid_walk: needs a real renderer, run without --headless")
		quit(1)
		return
