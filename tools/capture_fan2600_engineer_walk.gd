...
## FAN-2600 — live 1080p capture of the runtime hero engineer walk loops.
...
## Run: Godot --path . --script res://tools/capture_fan2600_engineer_walk.gd
## Output: build/qa/fan2600_engineer/engineer_walk_1920x1080_page<n>.png

const FRAMES_PATH := "res://assets/sprites/characters/engineer_spriteframes.tres"
const OUT_DIR := "res://build/qa/fan2600_engineer"
...
# The engineer runtime canvas is still the raw 248px source x2 (SCRUM-428):
# visible art spans y 124..376 of the 512 px canvas, so its band centre (~250)
# almost matches the canvas centre — a small lift keeps rows centred.
const ART_CENTRE_OFFSET := -6.0
...
func _initialize() -> void:
	...
	if frames == null:
		push_error("capture_fan2600_engineer_walk: cannot load %s" % FRAMES_PATH)
		quit(1)
		return
	if DisplayServer.get_name() == "headless":
		push_error("capture_fan2600_engineer_walk: needs a real renderer, run without --headless")
		quit(1)
		return
...
func _capture_page(frames: SpriteFrames, page: int) -> bool:
	...
	var path := "%s/engineer_walk_%dx%d_page%d.png" % [
	...
	viewport.queue_free()
	if error != OK:
		push_error("capture_fan2600_engineer_walk: save failed (%d) for %s" % [error, path])
		return false
	print("captured: ", path)
	return true
