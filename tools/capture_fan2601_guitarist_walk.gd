...
## FAN-2601 — live captures of the runtime hero guitarist walk loops.
...
## Viewport size comes from user args: `-- --size 1920x1080` (default) or
## `-- --size 1280x720` (two rows per page).
...
## Run: Godot --path . --script res://tools/capture_fan2601_guitarist_walk.gd -- --size 1280x720
## Output: build/qa/fan2601_guitarist/guitarist_walk_<W>x<H>_page<n>.png

const FRAMES_PATH := "res://assets/sprites/characters/guitarist_spriteframes.tres"
const OUT_DIR := "res://build/qa/fan2601_guitarist"
const DIRECTIONS := [
...
# Visible art occupies y 154..401 of the 512 px runtime canvas; lift the sprite
# so that band, not the padded canvas, is centred in its cell.
const ART_CENTRE_OFFSET := 22.0
...
func _initialize() -> void:
	...
	if frames == null:
		push_error("capture_fan2601_guitarist: cannot load %s" % FRAMES_PATH)
		quit(1)
		return
	if DisplayServer.get_name() == "headless":
		push_error("capture_fan2601_guitarist: needs a real renderer, run without --headless")
		quit(1)
		return
	...
	for index in user_args.size():
		var arg := str(user_args[index])
		var value := ""
		if arg.begins_with("--size="):
			value = arg.trim_prefix("--size=")
		elif arg == "--size" and index + 1 < user_args.size():
			value = str(user_args[index + 1])
		if value != "":
			var parts := value.split("x")
			if parts.size() == 2:
				size = Vector2i(int(parts[0]), int(parts[1]))
	var rows_per_page := 4 if size.y >= 1080 else 2

	DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path(OUT_DIR))
	var page_count := int(ceil(DIRECTIONS.size() / float(rows_per_page)))
	for page in page_count:
		if not await _capture_page(frames, page, size, rows_per_page):
			quit(1)
			return
	quit(0)


func _capture_page(frames: SpriteFrames, page: int, size: Vector2i, rows_per_page: int) -> bool:
	...
	var path := "%s/guitarist_walk_%dx%d_page%d.png" % [
		OUT_DIR, size.x, size.y, page + 1]
	var error := viewport.get_texture().get_image().save_png(ProjectSettings.globalize_path(path))
	viewport.queue_free()
	if error != OK:
		push_error("capture_fan2601_guitarist: save failed (%d) for %s" % [error, path])
		return false
	print("captured: ", path)
	return true
