...
## SCRUM-805 v5: снимает все 3 вкладки настроек на 1920×1080 и 2560×1440.
## Run (windowed для PNG): Godot --path . --script res://tools/capture_settings_v5.gd
## Output: build/qa/scrum805_v5/settings_v5_<tab>_<WxH>.png + rects.md
...
const DUMP_NODES := [
	"SettingsV2Modal", "SettingsV2MainModalFrame", "SettingsV2Title",
...
func _initialize() -> void:
	var qa_dir := ProjectSettings.globalize_path("res://build/qa/scrum805_v5")
	DirAccess.make_dir_recursive_absolute(qa_dir)
	var dump_lines := PackedStringArray()
	dump_lines.append("# SCRUM-805 Settings v5 Capture")
	dump_lines.append("")
	for viewport_size in VIEWPORT_SIZES:
		await _capture_at_size(viewport_size, qa_dir, dump_lines)
	var file := FileAccess.open("%s/settings_v5_rects.md" % qa_dir, FileAccess.WRITE)
	if file != null:
		file.store_string("\n".join(dump_lines))
		file.close()
	print("Settings v5 capture done.")
	quit(0)
...
func _capture_at_size(viewport_size: Vector2i, qa_dir: String, dump_lines: PackedStringArray) -> void:
	...
	for tab_index in range(3):
		...
		for _i in range(6):
			await process_frame
		var output_path := "%s/settings_v5_%s_%dx%d.png" % [qa_dir, TAB_SLUGS[tab_index], viewport_size.x, viewport_size.y]
