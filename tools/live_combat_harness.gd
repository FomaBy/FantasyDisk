...
const KNOWN_ARTIFACTS := {
	"robot_reactor_core": "Вращающийся 4-вентильный веер: ровно 4 выброса с шагом 90° от МИРОВОЙ фазы (старт 0°=восток), паттерн доворачивается на +6° по часовой после КАЖДОЙ атаки (полный цикл 15 атак = 90°), без самонаведения на ближайшего врага. Пер-вентильный урон = ролл × REACTOR_VENT_DAMAGE_RATIO (0.42) — за каст по кругу летит ≈1.68× ролла, урон НЕ делится на 4. FAN-1893: extra_projectile вентилями не потребляется (real_projectile_count 0; прежний width-бонус +14%/снаряд удалён), направлений всегда ровно 4. Стационарный односторонний кластер болванок ловит лишь ~1 лопасть из 4 и вдобавок «уезжает» из-под неё по мере вращения → живой замер систематически недооценивает пропускную способность; в реальном бою (рой вокруг героя) веер заметает все стороны. Формульный balance_report держит robot в бюджете профиля.",
}
